...
const PeriodicInspections = {
            ...
            const dataRows = dayRecords.map(r => {
                ...
                <tr class="dsc-data-row" data-record-id="${Utils.escapeHTML(r.id)}" style="transition: background-color 0.15s ease;">
                    ...
                    <td class="text-left">
                        ...
                        <button type="button" class="btn-icon ml-2" style="background:#25D366;color:#fff;" onclick="PeriodicInspections.shareDailySafetyChecklistWhatsApp('${Utils.escapeHTML(r.id)}')" title="${t('module.periodic.dsc.action.whatsapp', 'مشاركة واتساب')}"><i class="fab fa-whatsapp"></i></button>
        ...
        modal.innerHTML = `
            ...
            <div class="modal-content dsc-view-box">
                ...
                <div class="dsc-view-footer">
                    <button type="button" class="btn-primary" style="background:linear-gradient(135deg,#25D366,#128C7E);border:none;" onclick="PeriodicInspections.shareDailySafetyChecklistWhatsApp('${Utils.escapeHTML(record.id)}')"><i class="fab fa-whatsapp ml-2"></i>${t('module.periodic.dsc.action.whatsapp', 'مشاركة واتساب')}</button>
     ...
     */
    buildDailySafetyChecklistWhatsAppText(record) {
        const r = record || {};
        const serial = this.getDailySafetyCheckListSerialNumber(r);
        const fieldMap = { q16: 'q15Reading', q17: 'q16', q18: 'q17' };
        const compliant = [];
        const nonCompliant = [];
        const naItems = [];
        const lines = (this.DAILY_SAFETY_CHECKLIST_QUESTIONS || []).map((q, idx) => {
            const key = fieldMap[q.key] || q.key;
            const status = r[key] != null ? String(r[key]).trim() : '';
            const n = idx + 1;
            const label = this._getDailySafetyQuestionLabel(q);
            if (q.key === 'q16' || q.isReading) {
                return '📊 *' + n + '.* ' + label + '\n    ' + (status || 'غير مسجّلة');
            }
            let emoji = '✅';
            if (status === 'غير مطابق') {
        ...
        return [
            '🛡️ *تقرير المرور اليومي لمسؤول السلامة*',
            '*الشركة العالمية للإنتاج والتصنيع الزراعي (ICAPP)*',
            '━━━━━━━━━━━━━━━━━━━━',
            '📌 *رقم التقرير:* `' + (serial || '-') + '`',
            '📅 *التاريخ:* ' + (r.date ? (typeof Utils !== 'undefined' && Utils.formatDate ? Utils.formatDate(r.date) : String(r.date).slice(0, 10)) : '-'),
            '🏭 *المصنع / الموقع:* ' + (r.siteName || '-'),
            '🕐 *الوردية:* ' + this._formatDailyShiftLabel(r.shift || '-'),
            '👤 *مسؤول السلامة:* ' + (r.inspectorName || '-'),
            '━━━━━━━━━━━━━━━━━━━━',
            '📊 *الملخص:* ✅ ' + compliant.length + ' مطابق  |  ❌ ' + nonCompliant.length + ' غير مطابق  |  ⚪ ' + naItems.length + ' لا ينطبق',
            nonCompliant.length ? ('⚠️ *بنود غير المطابقة:* ' + nonCompliant.join('، ')) : '',
            '━━━━━━━━━━━━━━━━━━━━',
            '📋 *نتائج بنود المرور (18 بنداً):*',
            '',
            ...lines,
            '',
            '━━━━━━━━━━━━━━━━━━━━',
            '📝 *الملاحظات والإجراءات المتخذة:*',
    ...
    shareDailySafetyChecklistWhatsApp(recordId) {
        const record = this.getDailySafetyCheckListRecords().find((r) => String(r.id) === String(recordId));
        if (!record) {
            if (typeof Notification !== 'undefined') Notification.error(this._t('module.periodic.dsc.recordNotFound', 'السجل غير موجود'));
            return;
        }
        const text = this.buildDailySafetyChecklistWhatsAppText(record);
