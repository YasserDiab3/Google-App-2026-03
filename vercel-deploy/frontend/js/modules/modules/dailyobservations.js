...
const DailyObservations = {
    ...
    isObservationPhotoAttachment(att) {
        if (!att) return false;
        if (att.__listOnly && !(att.fileId || att.driveId || att.url)) return false;
    ...
    filterDailyObservationsForCurrentUserScope(rows) {
        const list = Array.isArray(rows) ? rows : [];
        if (this.canViewAllObservationsWorkflow()) return list;
        ...
        // قراءة حقول خام فقط — بدون normalizeRecord لكل صف (كان يجمّد الواجهة عند آلاف الصفوف)
        return list.filter((obs) => {
            if (!obs) return false;
            const stage = String(obs.workflowStage || '').trim();
            const resp = norm(obs.responsibleDepartment);
    ...
    _invalidateObsNormCache() {
        this._obsNormCache = { key: '', list: null };
        this._dailyObsDataStamp = Date.now();
    },
    ...
    _bindObservationListCloseActions(container) {
        ...
        container.addEventListener('click', (e) => {
            ...
            if (exportSelBtn) {
                e.preventDefault();
                void this.showExportPptModal({ useSelectedOnly: true });
                return;
            }
    ...
    _dailyObsDataStamp: 0,
    _obsNormCache: { key: '', list: null },
    _obsListPage: 1,
    _obsListRetryCount: 0,
    _obsSelectedCloseIds: null,
    _obsListRenderToken: 0,

    _slimAttachmentMeta(att) {
        if (!att) return null;
        if (typeof att === 'string') {
            const s = String(att).trim();
            if (!s || s.startsWith('data:')) return null;
            const fileId = this.resolveObservationDriveFileId(s);
            const urlMatch = s.match(/https?:\/\/[^\s]+/i);
            const url = urlMatch ? urlMatch[0].trim() : (/^https?:\/\//i.test(s) ? s : '');
            if (!fileId && !url) return null;
            const nameMatch = s.match(/^(.+?)\s*-\s*https?:\/\//i);
            return {
                name: nameMatch ? nameMatch[1].trim().slice(0, 120) : 'صورة',
                type: 'image',
        ...
        const fileId = String(att.fileId || att.driveId || '').trim() || this.resolveObservationDriveFileId(att);
        let url = String(att.url || att.directLink || att.shareableLink || att.driveUrl || att.link || '').trim();
        if (url.startsWith('data:')) url = '';
        const data = String(att.data || '').trim();
        if (!url && /^https?:\/\//i.test(data)) url = data;
        if (!fileId && !url && data && !data.startsWith('data:')) {
            const embedded = this.resolveObservationDriveFileId(data);
            if (embedded) {
                return {
                    name: String(att.name || att.fileName || 'صورة').slice(0, 120),
                    type: String(att.type || att.mimeType || 'image').slice(0, 80),
                    fileId: embedded,
    ...
    /**
     * تطبيع حقل attachments من الشيت: JSON أو نص متعدد الأسطر "name - url"
     */
    _coerceAttachmentsArray(raw) {
        if (!raw && raw !== 0) return [];
        if (Array.isArray(raw)) return raw.filter((x) => x != null && x !== '');
    ...
    slimObservationForList(row) {
        ...
        const rawAtts = this._coerceAttachmentsArray(row.attachments || row.files || row.images);
        const slimAtts = rawAtts.map((a) => this._slimAttachmentMeta(a)).filter(Boolean);
        const attCount = Math.max(Number(row.attachmentCount) || 0, slimAtts.length, rawAtts.length);
        const afterRaw = this._coerceAttachmentsArray(row.afterExecutionImages);
        const slimAfter = afterRaw.map((a) => this._slimAttachmentMeta(a)).filter(Boolean);
        const afterCount = Math.max(Number(row.afterImageCount) || 0, slimAfter.length, afterRaw.length);
        return {
            ...row,
            attachmentCount: attCount,
            afterImageCount: afterCount,
            attachments: slimAtts.length ? slimAtts : (attCount > 0 ? [{ __listOnly: true }] : []),
            afterExecutionImages: slimAfter,
            timeLog: [],
            updates: [],
            comments: []
        };
    ...
    async ensureDailyObservationsDataLoaded({ force = false } = {}) {
        ...
        const run = async () => {
            ...
            try {
                ...
                const ctx = typeof this.buildObservationsRequestContext === 'function' ? this.buildObservationsRequestContext() : null;
                // مهلة أطول: ورقة كبيرة (~3.5k صف) — لا يحجب الواجهة (جلب خلفي)
                ...
                if (Array.isArray(data)) {
                    ...
                    for (let i = 0; i < data.length; i += CHUNK) {
                        for (let j = i; j < Math.min(i + CHUNK, data.length); j++) {
                            slim.push(this.slimObservationForList(data[j]));
                        }
                        await this.yieldToMain();
                    }
    ...
    async load() {
        ...
        // DataManager اختياري — لا ننتظره (كان يصل حتى ~2ث قبل أول رسم)
        const dataManagerAvailable = typeof window !== 'undefined' &&
            (typeof window.DataManager !== 'undefined' || typeof DataManager !== 'undefined');
        ...
        // ✅ عند فتح المديول: سجل الملاحظات أولاً دائماً إن وُجدت الصلاحية
        ...
        // لا نُعيد slim لكل الصفوف هنا — كان يجمّد الخيط عند آلاف الصفوف
        ...
        try {
            ...
            // إصلاح لمرة واحدة: إصدارات سابقة مسحت نص المرفقات عند الـ slim — أعد الجلب من الخادم
            try {
                const repairKey = 'daily_obs_attachments_repair_v1';
                if (!localStorage.getItem(repairKey)) {
                    localStorage.setItem(repairKey, '1');
                    lastSync = null;
                    try { localStorage.removeItem('daily_observations_last_sync'); } catch (_e) {}
                }
            } catch (_e) {}
            ...
            // هيكل خفيف فوراً: بدون normalize لكل الصفوف قبل الرسم
            const registryShell = `
            ...
            let listContent = (firstTab === 'observations-registry' || canRegistry)
                ? (firstTab === 'observations-registry' ? registryShell : obsLazyPlaceholder)
                : '';
            let analysisContent = '';
            let top10Content = '';
            if (canTop10) {
                top10Content = firstTab === 'top-10-observations' ? obsLazyPlaceholder : obsLazyPlaceholder;
            }
            if (canAnalysis) {
            ...
            try { this.setupTabs(); } catch (e) { /* ignore */ }

            // ملء السجل بعد أول إطار — بدون حجب الرسم
            await this.yieldToMain();
            if (firstTab === 'observations-registry') {
                try {
                    const filled = await this.renderList();
                    const tab = document.getElementById('tab-observations-registry');
                    if (tab && filled) tab.innerHTML = filled;
                    this.setupEventListeners();
                    await this.yieldToMain();
                    await this.loadObservationsList({ resetPage: true });
                } catch (listErr) {
                    Utils?.safeWarn?.('⚠️ بناء سجل الملاحظات:', listErr);
                }
                try { this.runObservationDueDateReminders(); } catch (remErr) {
                    Utils.safeWarn('⚠️ تنبيهات مواعيد الملاحظات:', remErr);
                }
            } else if (firstTab === 'top-10-observations') {
                void this.loadTop10Observations();
            } else if (firstTab === 'data-analysis' && isAdmin) {
                try {
                    const tab = document.getElementById('tab-data-analysis');
                    if (tab) {
                        tab.removeAttribute('data-obs-lazy');
                        tab.innerHTML = await this.renderDataAnalysis();
                    }
                } catch (e) { /* ignore */ }
            }
        } catch (error) {
    ...
    async renderList() {
        // هيكل الفلاتر خفيف — قيم الخيارات تُملأ لاحقاً عبر updateFilterOptions
        const { t, isRTL } = this.getTranslations();
        const iconMarginClass = isRTL ? 'ml-1' : 'mr-1';
        const titleIconMargin = isRTL ? 'ml-2' : 'mr-2';
        const allOpt = `<option value="">${t('filter.all')}</option>`;
    ...
    _buildInitialTableHtml(_observations, isRTL, t) {
        const localCount = Array.isArray(AppState?.appData?.dailyObservations) ? AppState.appData.dailyObservations.length : 0;
        if (localCount > 0) {
            return `<div class="empty-state" style="padding:24px;direction: ${isRTL ? 'rtl' : 'ltr'};"><i class="fas fa-spinner fa-spin text-indigo-400 text-2xl mb-2"></i><p class="text-gray-500">${Utils.escapeHTML(this._t('module.dailyobs.loading.list', 'جاري عرض سجل الملاحظات…'))}</p></div>`;
        }
        return `<div class="empty-state" style="direction: ${isRTL ? 'rtl' : 'ltr'}; text-align: ${isRTL ? 'right' : 'left'};"><p class="text-gray-500">${Utils.escapeHTML(t && typeof t === 'function' ? t('empty.noObservations') : this._t('module.dailyobs.empty.noObservations', 'لا توجد ملاحظات'))}</p></div>`;
    },

    _renderObservationAttachmentsCell(obs) {
        const atts = Array.isArray(obs?.attachments) ? obs.attachments : [];
        const real = atts.filter((a) => a && !a.__listOnly && (a.fileId || a.driveId || a.url || this.getObservationAttachmentSrc(a)));
        const count = Math.max(Number(obs?.attachmentCount) || 0, real.length, atts.filter((a) => a && !a.__listOnly).length);
        if (!count && !real.length) return '-';
        const first = real[0] || null;
        const fileId = first ? this.resolveObservationDriveFileId(first) : '';
        const title = Utils.escapeHTML(this._tf('module.dailyobs.registry.attachments.count', { n: count }, `${count} ملف`));
        if (fileId) {
            const placeholder = (typeof Utils !== 'undefined' && Utils.IMG_DRIVE_PLACEHOLDER_GIF)
                ? Utils.IMG_DRIVE_PLACEHOLDER_GIF
                : 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
            return `<div style="display:flex;align-items:center;justify-content:center;gap:6px;" title="${title}">
                <img src="${Utils.escapeHTML(placeholder)}" data-drive-proxy-id="${Utils.escapeHTML(fileId)}" alt="" style="width:36px;height:36px;object-fit:cover;border-radius:8px;border:1px solid #e2e8f0;background:#f8fafc;">
                <span style="font-size:12px;font-weight:700;color:#2563eb;">${count}</span>
            </div>`;
        }
        return `<i class="fas fa-paperclip text-blue-500" title="${title}"></i>`;
    },

    _renderObservationTableRow(obs, tbl) {
        const oid = Utils.escapeHTML(String(obs.id || ''));
        const showSelect = !!(tbl && tbl.showSelect);
        const canClose = !!(tbl && tbl.showBulkClose) && this.canCloseObservationQuick(obs);
        ...
        const selectCell = showSelect
            ? `<td style="width:44px;text-align:center;vertical-align:middle;">
                    <input type="checkbox" class="obs-row-select" data-oid="${oid}" ${selected ? 'checked' : ''} title="${Utils.escapeHTML(tbl && tbl.select ? tbl.select : 'تحديد')}" style="width:18px;height:18px;cursor:pointer;accent-color:#4f46e5;vertical-align:middle;">
               </td>`
            : '';
        return `
            <tr data-oid="${oid}" class="${closed ? 'obs-row-closed' : ''}">
                ...
                <td>${this._renderObservationAttachmentsCell(obs)}</td>
                <td>
    ...
    _obsNormCacheKey(raw) {
        const uid = AppState?.currentUser?.id || AppState?.currentUser?.email || '';
        return `${Array.isArray(raw) ? raw.length : 0}|${this._dailyObsDataStamp || 0}|${uid}`;
    },
    ...
    _getVisibleObservationsNormalized() {
        const observationsRaw = typeof this.getDailyObservationsVisibleToCurrentUser === 'function'
            ? this.getDailyObservationsVisibleToCurrentUser()
            : (Array.isArray(AppState.appData.dailyObservations) ? AppState.appData.dailyObservations : []);
        const key = this._obsNormCacheKey(observationsRaw);
        if (this._obsNormCache && this._obsNormCache.key === key && Array.isArray(this._obsNormCache.list)) {
            return this._obsNormCache.list;
        }
        const list = observationsRaw.map((item) => this.normalizeRecord(item));
        this._obsNormCache = { key, list };
        return list;
    },

    async _getVisibleObservationsNormalizedAsync() {
        const observationsRaw = typeof this.getDailyObservationsVisibleToCurrentUser === 'function'
            ? this.getDailyObservationsVisibleToCurrentUser()
            : (Array.isArray(AppState.appData.dailyObservations) ? AppState.appData.dailyObservations : []);
        const key = this._obsNormCacheKey(observationsRaw);
        ...
        const list = [];
        const CHUNK = 250;
        for (let i = 0; i < observationsRaw.length; i += CHUNK) {
            for (let j = i; j < Math.min(i + CHUNK, observationsRaw.length); j++) {
                list.push(this.normalizeRecord(observationsRaw[j]));
            }
            await this.yieldToMain();
        }
        this._obsNormCache = { key, list };
    ...
    async loadObservationsList(arg) {
        ...
        if (!container) {
            ...
            if (this._obsListRetryCount > 40) return;
        ...
        if (observationsRaw.length === 0) {
            ...
            try { this.renderStatsCards([]); } catch (_e) {}
            return;
        }

        const renderToken = (this._obsListRenderToken = (this._obsListRenderToken || 0) + 1);
        const observations = await this._getVisibleObservationsNormalizedAsync();
        if (renderToken !== this._obsListRenderToken) return;
        ...
        const tbl = {
            ...
            showSelect: true,
            showBulkClose: this.canUseObservationListClose()
        };
        ...
        // تطبيق الفلاتر
        const filters = this.getFilters();
        const filteredObservations = this.filterItems(sortedObservations, filters);
        
        // تحديث شارات العد على الفلاتر النشطة
        this.updateFilterBadges(sortedObservations, filteredObservations, filters);
        this.renderActiveFilterChips(filters, filteredObservations.length, sortedObservations.length);
        ...
        const selectedCount = this._ensureObsSelectedSet().size;
        container.innerHTML = `
            <div id="obs-bulk-close-bar" style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:8px;margin-bottom:10px;padding:10px 12px;background:#eef2ff;border:1px solid #c7d2fe;border-radius:12px;">
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    ...
                    ${tbl.showBulkClose ? `<button type="button" id="obs-bulk-close-btn" class="btn-primary btn-sm" disabled style="background:#4f46e5;border:none;">
        ...
        // فلاتر KPI بعد أول رسم للجدول — يقلل التهنيج
        await this.yieldToMain();
        if (renderToken !== this._obsListRenderToken) return;
        try { this.updateFilterOptions(); } catch (_e) {}
        try { this.renderStatsCards(observations); } catch (_e) {}
    },
    ...
    _pptExportPrefsKey() {
        return 'dailyobs_ppt_export_opts_v1';
    },

    _loadPptExportPrefs() {
        try {
            const raw = localStorage.getItem(this._pptExportPrefsKey());
            if (!raw) return {};
    ...
    _savePptExportPrefs(partial) {
        try {
            const next = Object.assign({}, this._loadPptExportPrefs(), partial || {});
            localStorage.setItem(this._pptExportPrefsKey(), JSON.stringify(next));
        } catch (_e) { /* ignore */ }
    },

    _isObservationOverdueForPpt(obs) {
        if (!obs) return false;
        const st = String(obs.status || '').trim().toLowerCase();
        if (st === 'مغلق' || st === 'closed') return false;
        const stage = String(obs.workflowStage || '').trim().toLowerCase();
        if (stage === 'closed' || stage === 'rejected') return false;
        const raw = String(obs.expectedCompletionDate || '').trim();
        if (!raw) return false;
    ...
    _showPptExportProgress(pct, message, meta = {}) {
        const p = Math.max(0, Math.min(100, Math.round(Number(pct) || 0)));
        const embedded = meta.embedded != null ? meta.embedded : this._pptExportEmbeddedCount;
        const budget = meta.imageBudget != null ? meta.imageBudget : this._pptExportImageBudget;
        let el = document.getElementById('dailyobs-ppt-progress-overlay');
        if (!el) {
            el = document.createElement('div');
            el.id = 'dailyobs-ppt-progress-overlay';
            el.setAttribute('data-minimized', '0');
            el.style.cssText = 'position:fixed;z-index:1000000;bottom:18px;left:18px;font-family:Cairo,Tahoma,sans-serif;pointer-events:none;';
            el.innerHTML = `
            ...
            el.querySelector('#dailyobs-ppt-progress-hide-btn')?.addEventListener('click', () => this._minimizePptExportProgress());
            el.querySelector('#dailyobs-ppt-progress-chip')?.addEventListener('click', () => this._restorePptExportProgress());
        }
        const panel = document.getElementById('dailyobs-ppt-progress-panel');
        const chip = document.getElementById('dailyobs-ppt-progress-chip');
        const minimized = el.getAttribute('data-minimized') === '1';
        if (panel) panel.style.display = minimized ? 'none' : 'block';
        if (chip) chip.style.display = minimized ? 'inline-flex' : 'none';

        const bar = document.getElementById('dailyobs-ppt-progress-bar');
        const msg = document.getElementById('dailyobs-ppt-progress-msg');
        const pctEl = document.getElementById('dailyobs-ppt-progress-pct');
        const imgEl = document.getElementById('dailyobs-ppt-progress-images');
        const chipText = document.getElementById('dailyobs-ppt-progress-chip-text');
        if (bar) bar.style.width = p + '%';
        if (pctEl) pctEl.textContent = p + '%';
        if (msg && message) msg.textContent = String(message);
        if (imgEl && budget != null) {
            imgEl.textContent = `الصور المضمّنة ${embedded != null ? embedded : 0} من الحد ${budget}`;
        }
        if (chipText) chipText.textContent = `تصدير… ${p}%`;
    },

    _minimizePptExportProgress() {
        const el = document.getElementById('dailyobs-ppt-progress-overlay');
        if (!el) return;
    ...
    _hidePptExportProgress() {
        const el = document.getElementById('dailyobs-ppt-progress-overlay');
        if (el) el.remove();
        this._setPptExportUiBusy(false);
    },

    _setPptExportUiBusy(busy) {
        this._pptExportBusy = !!busy;
        [
            'export-observations-ppt-btn',
            'obs-export-selected-ppt-btn',
            'dailyobs-ppt-export-btn',
            'dailyobs-ppt-preview-btn'
        ].forEach((id) => {
    ...
    async showExportPptModal(opts = {}) {
        const prefs = this._loadPptExportPrefs();
        const forceSelected = !!(opts && opts.useSelectedOnly);
        const selectedCount = this._ensureObsSelectedSet().size;
        const modal = document.createElement('div');
        ...
        const prefLang = prefs.language === 'en' ? 'en' : 'ar';
        const prefMax = ['all', '10', '50', '100', '200'].includes(String(prefs.maxCount)) ? String(prefs.maxCount) : 'all';
        const prefSummary = prefs.summaryOnly === true;
        const prefRegistry = prefs.useRegistryFilters === true;
        const prefSelected = forceSelected || prefs.useSelectedOnly === true;
        const prefFormat = prefs.outputFormat === 'pdf' || prefs.outputFormat === 'both' ? prefs.outputFormat : 'pptx';
        const prefMaxImages = Math.max(10, Math.min(120, parseInt(prefs.maxImages, 10) || 50));
        const prefPdfMode = ['full', 'summary', 'top10', 'match'].includes(prefs.pdfMode) ? prefs.pdfMode : 'match';
        const prefSkipNoImage = prefs.skipNoImageSlides === true;
        const prefExcel = prefs.exportExcelCompanion === true;
            ...
            <div style="max-width: 720px; width: 92%; max-height: 92vh; overflow:auto; background: #ffffff; border-radius: 24px; padding: 28px 32px; box-shadow: 0 25px 60px -15px rgba(15, 23, 42, 0.3); border: 1px solid rgba(226, 232, 240, 0.9); position: relative; font-family: Cairo, Tahoma, sans-serif;">
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 22px; padding-bottom: 16px; border-bottom: 1px solid #f1f5f9;">
                    <div style="display: flex; align-items: center; gap: 14px;">
                        ...
                        <div>
                            ...
                            <p style="font-size: 13px; color: #64748b; margin: 3px 0 0 0;">خيارات محفوظة تلقائياً — المحدد حالياً: ${selectedCount}</p>
                        </div>
                    </div>
                ...
                <div style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border: 1px solid #bae6fd; border-radius: 14px; padding: 14px 18px; margin-bottom: 18px; display: flex; align-items: center; justify-content: space-between; gap: 14px;">
                    <div style="display: flex; align-items: center; gap: 10px; font-size: 13px; color: #0369a1; font-weight: 600; line-height: 1.4;">
                        ...
                        <span>حدد الصفوف في السجل ثم «تصدير المحدد» — أو استخدم الفلاتر أدناه.</span>
                    </div>
                    ${isAdmin ? `
                ...
                <div style="display: flex; flex-direction: column; gap: 14px; margin-bottom: 20px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                        ...
                        <div>
                            ...
                            <select id="dailyobs-ppt-language" style="width: 100%; padding: 11px 14px; border: 1.5px solid #cbd5e1; border-radius: 10px; font-size: 13.5px; font-weight: 700; color: #1e40af;">
                                <option value="ar" ${prefLang === 'ar' ? 'selected' : ''}>العربية</option>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                        <div>
                            ...
                            <input id="dailyobs-ppt-report-date" type="date" value="${todayStr}" style="width: 100%; padding: 10px 14px; border: 1.5px solid #cbd5e1; border-radius: 10px; font-size: 13.5px; font-weight: 600;">
                        </div>
                        <div>
                            ...
                            <select id="dailyobs-ppt-maxcount" style="width: 100%; padding: 11px 14px; border: 1.5px solid #cbd5e1; border-radius: 10px; font-size: 13.5px; font-weight: 700;">
                                <option value="all" ${prefMax === 'all' ? 'selected' : ''}>الكل المطابق</option>
                                <option value="50" ${prefMax === '50' ? 'selected' : ''}>أحدث 50</option>
                                <option value="100" ${prefMax === '100' ? 'selected' : ''}>أحدث 100</option>
                                <option value="200" ${prefMax === '200' ? 'selected' : ''}>أحدث 200</option>
                                <option value="10" ${prefMax === '10' ? 'selected' : ''}>Top 10</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                        <div>
                            ...
                            <select id="dailyobs-ppt-format" style="width: 100%; padding: 11px 14px; border: 1.5px solid #cbd5e1; border-radius: 10px; font-size: 13.5px; font-weight: 700;">
                                <option value="pptx" ${prefFormat === 'pptx' ? 'selected' : ''}>PPTX فقط</option>
                                <option value="pdf" ${prefFormat === 'pdf' ? 'selected' : ''}>PDF فقط</option>
                                <option value="both" ${prefFormat === 'both' ? 'selected' : ''}>PPTX + PDF</option>
                            </select>
                        </div>
                        <div>
                            <label style="display: block; font-size: 13px; font-weight: 700; color: #334155; margin-bottom: 6px;">حد تضمين الصور</label>
                            <input id="dailyobs-ppt-max-images" type="number" min="10" max="120" value="${prefMaxImages}" style="width: 100%; padding: 10px 14px; border: 1.5px solid #cbd5e1; border-radius: 10px; font-size: 13.5px; font-weight: 700;">
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                        <div>
                            ...
                            <select id="dailyobs-ppt-pdf-mode" style="width: 100%; padding: 11px 14px; border: 1.5px solid #cbd5e1; border-radius: 10px; font-size: 13.5px; font-weight: 700;">
                                <option value="match" ${prefPdfMode === 'match' ? 'selected' : ''}>مثل شرائح PPT (موصى به)</option>
                                <option value="full" ${prefPdfMode === 'full' ? 'selected' : ''}>PDF كامل (حتى 40 ملاحظة)</option>
                                <option value="top10" ${prefPdfMode === 'top10' ? 'selected' : ''}>PDF أول 10 ملاحظات</option>
                                <option value="summary" ${prefPdfMode === 'summary' ? 'selected' : ''}>PDF ملخص فقط (بدون بطاقات)</option>
                            </select>
                        </div>
                        <div style="display:flex;flex-direction:column;justify-content:flex-end;gap:8px;">
                            <label style="display: flex; align-items: flex-start; gap: 8px; font-size: 12.5px; font-weight: 700; color: #334155; cursor: pointer;">
                                <input id="dailyobs-ppt-skip-no-image" type="checkbox" ${prefSkipNoImage ? 'checked' : ''} style="margin-top: 3px; width: 16px; height: 16px; accent-color: #2563eb;">
                                <span>إخفاء شرائح الملاحظات بدون صور</span>
                            </label>
                            <label style="display: flex; align-items: flex-start; gap: 8px; font-size: 12.5px; font-weight: 700; color: #334155; cursor: pointer;">
                                <input id="dailyobs-ppt-excel-companion" type="checkbox" ${prefExcel ? 'checked' : ''} style="margin-top: 3px; width: 16px; height: 16px; accent-color: #2563eb;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px 14px;">
                        <label style="display: flex; align-items: flex-start; gap: 8px; font-size: 12.5px; font-weight: 700; color: #334155; cursor: pointer;">
                            <input id="dailyobs-ppt-summary-only" type="checkbox" ${prefSummary ? 'checked' : ''} style="margin-top: 3px; width: 16px; height: 16px; accent-color: #2563eb;">
                            <span>ملخص فقط (بدون شريحة لكل ملاحظة)</span>
                        </label>
                        <label style="display: flex; align-items: flex-start; gap: 8px; font-size: 12.5px; font-weight: 700; color: #334155; cursor: pointer;">
                            <input id="dailyobs-ppt-use-registry-filters" type="checkbox" ${prefRegistry ? 'checked' : ''} style="margin-top: 3px; width: 16px; height: 16px; accent-color: #2563eb;">
                            <span>مطابقة فلتر السجل الحالي</span>
                        </label>
                        <label style="display: flex; align-items: flex-start; gap: 8px; font-size: 12.5px; font-weight: 700; color: #334155; cursor: pointer;">
                            <input id="dailyobs-ppt-use-selected" type="checkbox" ${prefSelected ? 'checked' : ''} ${selectedCount ? '' : 'disabled'} style="margin-top: 3px; width: 16px; height: 16px; accent-color: #2563eb;">
                            <span>تصدير المحدد فقط (${selectedCount})</span>
                        </label>
                    </div>
                </div>
        ...
        const collectOpts = (extra = {}) => {
            ...
            const summaryOnly = !!modal.querySelector('#dailyobs-ppt-summary-only')?.checked;
            const useRegistryFilters = !!modal.querySelector('#dailyobs-ppt-use-registry-filters')?.checked;
            const useSelectedOnly = !!modal.querySelector('#dailyobs-ppt-use-selected')?.checked;
            const outputFormat = modal.querySelector('#dailyobs-ppt-format')?.value || 'pptx';
            const maxImages = parseInt(modal.querySelector('#dailyobs-ppt-max-images')?.value, 10) || 50;
            const pdfMode = modal.querySelector('#dailyobs-ppt-pdf-mode')?.value || 'match';
            const skipNoImageSlides = !!modal.querySelector('#dailyobs-ppt-skip-no-image')?.checked;
            const exportExcelCompanion = !!modal.querySelector('#dailyobs-ppt-excel-companion')?.checked;
            this._savePptExportPrefs({
                language: lang,
                maxCount,
                summaryOnly,
                useRegistryFilters,
                ...
                pdfMode,
                skipNoImageSlides,
                exportExcelCompanion
            });
            return {
                department: dept, siteName, language: lang, reportDate, fromDate, toDate, status, maxCount,
                summaryOnly, useRegistryFilters, useSelectedOnly, outputFormat, maxImages,
                pdfMode, skipNoImageSlides, exportExcelCompanion,
                ...extra
            };
        };

        modal.querySelector('#dailyobs-ppt-preview-btn')?.addEventListener('click', async () => {
            if (this._pptExportBusy) return;
            const opts = collectOpts({ previewOnly: true, summaryOnly: true, outputFormat: 'pptx', maxCount: '10' });
            close();
            await this.exportPptReport(opts);
        });

        modal.querySelector('#dailyobs-ppt-export-btn')?.addEventListener('click', async () => {
            if (this._pptExportBusy) return;
            const opts = collectOpts();
            if (opts.useSelectedOnly && this._ensureObsSelectedSet().size === 0) {
                Notification.warning('اختر ملاحظة واحدة على الأقل من السجل');
                return;
            }
            close();
            await this.exportPptReport(opts);
    ...
    async exportPptReport({
        department = '', siteName = '', language = 'ar', reportDate = '', fromDate = '', toDate = '',
        status = 'all', maxCount = 'all', summaryOnly = false, useRegistryFilters = false,
        useSelectedOnly = false, outputFormat = 'pptx', maxImages = 50, previewOnly = false,
        pdfMode = 'match', skipNoImageSlides = false, exportExcelCompanion = false
    } = {}) {
        if (this._pptExportBusy) {
            Notification.info('تصدير قيد التشغيل — يمكنك إظهار شريط التقدّم من الزر العائم');
            this._restorePptExportProgress();
            return;
        }
        try {
            ...
            this._setPptExportUiBusy(true);
            this._pptExportImageBudget = Math.max(10, Math.min(120, parseInt(maxImages, 10) || 50));
            this._pptExportEmbeddedCount = 0;
            this._showPptExportProgress(2, 'تحضير البيانات...', {
                embedded: 0,
                imageBudget: this._pptExportImageBudget
            });
            const normalized = observationsRaw.map((item) => this.normalizeRecord(item));
            let dept = String(department || '').trim();
            let site = String(siteName || '').trim();
            ...
            if (useSelectedOnly) {
                const selected = this._ensureObsSelectedSet();
                if (!selected.size) {
                    this._hidePptExportProgress();
                    Notification.warning('لا توجد ملاحظات محددة للتصدير.');
                    return;
                }
                filtered = normalized.filter((obs) => selected.has(String(obs.id || '')));
            } else if (useRegistryFilters && typeof this.getFilters === 'function' && typeof this.filterItems === 'function') {
                const registryFilters = this.getFilters();
                filtered = this.filterItems(normalized, registryFilters);
                dept = String(registryFilters.responsible || dept || '').trim();
                site = String(registryFilters.site || site || '').trim();
                from = registryFilters.dateFrom ? new Date(registryFilters.dateFrom) : from;
                to = registryFilters.dateTo ? new Date(registryFilters.dateTo) : to;
                fromDate = registryFilters.dateFrom || fromDate;
                toDate = registryFilters.dateTo || toDate;
            } else {
                filtered = normalized.filter((obs) => {
            ...
            const sorted = [...filtered].sort((a, b) => {
                ...
                const statusScore = (st) => (st !== 'مغلق' && String(st || '').toLowerCase() !== 'closed' ? 2 : 1);
            ...
            const limit = previewOnly ? Math.min(5, sorted.length) : (isAllCount ? sorted.length : (parseInt(maxCount, 10) || sorted.length));
            // ملخص فقط / معاينة: بدون hydrate صور — لا طلبات شبكة إضافية
            const skipHydrate = summaryOnly || previewOnly;
            this._showPptExportProgress(8, skipHydrate ? 'بناء الشرائح...' : 'جلب صور الملاحظات...', {
                embedded: this._pptExportEmbeddedCount,
                imageBudget: this._pptExportImageBudget
            });
            const exportBatch = skipHydrate
                ? sorted.slice(0, limit)
                : await this.hydrateObservationBatchForExport(sorted.slice(0, limit));
            const imageBudget = this._pptExportImageBudget;
            let embeddedImageCount = 0;
            Notification.info(
                previewOnly
                    ? '🔎 معاينة غلاف PPT...'
                    : (summaryOnly
                        ? `🚀 تصدير ملخص (${exportBatch.length} ملاحظة)...`
                        : `🚀 تصدير PPTX (${exportBatch.length} ملاحظة)...`)
            );
            ...
            // Tahoma/Arial مضمّنان في ويندوز ويدعمان العربية — Cairo غير مضمّن في PPTX فيظهر مشوّهاً
            const pptFont = isEnglish ? 'Arial' : 'Tahoma';
            const pptLang = isEnglish ? 'en-US' : 'ar-SA';
            const pptTextBase = { fontFace: pptFont, lang: pptLang, rtl: !isEnglish };
            ...
            const exporterName = String(
                AppState.currentUser?.name || AppState.currentUser?.fullName || AppState.currentUser?.email || ''
            ).trim() || (isEnglish ? 'HSE User' : 'مستخدم النظام');

            const periodFromLabel = fromDate
                ? new Date(fromDate).toLocaleDateString(isEnglish ? 'en-US' : 'ar-EG')
                : '';
            const periodToLabel = toDate
                ? new Date(toDate).toLocaleDateString(isEnglish ? 'en-US' : 'ar-EG')
                : '';
            let periodLabel = '';
            if (periodFromLabel && periodToLabel) {
                periodLabel = isEnglish ? `${periodFromLabel} → ${periodToLabel}` : `${periodFromLabel} ← ${periodToLabel}`;
            } else if (periodFromLabel || periodToLabel) {
                periodLabel = periodFromLabel || periodToLabel;
            } else {
                const dates = exportBatch
                    .map((o) => (o.date ? new Date(o.date).getTime() : NaN))
                    .filter((t) => !Number.isNaN(t))
                    .sort((a, b) => a - b);
                if (dates.length) {
                    const a = new Date(dates[0]).toLocaleDateString(isEnglish ? 'en-US' : 'ar-EG');
                    const b = new Date(dates[dates.length - 1]).toLocaleDateString(isEnglish ? 'en-US' : 'ar-EG');
                    periodLabel = a === b ? a : (isEnglish ? `${a} → ${b}` : `${a} ← ${b}`);
                } else {
                    periodLabel = dateLabel;
                }
            }

            const clipPptText = (value, maxLen) => {
                const raw = String(value || '').trim();
                if (!raw) return { text: '—', overflow: '', clipped: false };
                if (raw.length <= maxLen) return { text: raw, overflow: '', clipped: false };
                return { text: raw.slice(0, maxLen).trim() + '…', overflow: raw, clipped: true };
            };
            const isObsClosed = (st) => {
                const s = String(st || '').trim().toLowerCase();
                return s === 'مغلق' || s === 'closed';
            };
            const riskTone = (risk) => {
                const s = String(risk || '').toLowerCase();
                if (s.includes('عالي') || s.includes('حرج') || s.includes('high') || s.includes('critical')) {
                    return { bg: 'FEE2E2', fg: 'B91C1C', line: 'EF4444' };
                }
                if (s.includes('متوسط') || s.includes('medium')) {
                    return { bg: 'FEF3C7', fg: 'B45309', line: 'F59E0B' };
                }
                return { bg: 'D1FAE5', fg: '047857', line: '10B981' };
            };
            const statusTone = (st) => {
                if (isObsClosed(st)) return { bg: 'D1FAE5', fg: '047857', line: '10B981' };
                const s = String(st || '').toLowerCase();
                if (s.includes('جاري') || s.includes('progress')) return { bg: 'DBEAFE', fg: '1D4ED8', line: '3B82F6' };
                if (s.includes('معلق') || s.includes('pending') || s.includes('مراجعة')) return { bg: 'FEF3C7', fg: 'B45309', line: 'F59E0B' };
                return { bg: 'FFEDD5', fg: 'C2410C', line: 'FB923C' };
            };
                ...
                try {
                    ...
                    img.onload = () => {
                        ...
                        try {
                            ...
                            resolve(canvas.toDataURL('image/jpeg', 0.62) || src);
                        } catch (_e) {
                            resolve(src);
                        }
                    };
            ...
            const finishPptxImage = async (dataUrl, cacheKey) => {
                if (embeddedImageCount >= imageBudget) return null;
                ...
                this._pptExportEmbeddedCount = embeddedImageCount;
                this._showPptExportProgress(
                    Math.max(15, Math.min(54, 15 + Math.round((embeddedImageCount / Math.max(1, imageBudget)) * 35))),
                    `تضمين الصور (${embeddedImageCount}/${imageBudget})`,
                    { embedded: embeddedImageCount, imageBudget }
                );
            ...
            const getObservationBeforeImage = (obs) => {
                if (!obs) return '';
            ...
            const getObservationAfterImage = (obs) => {
                if (!obs) return '';
            ...
            // جلب صور قبل/بعد — يُتخطى بالكامل في وضع الملخص فقط
            let logoImgObj = null;
            const obsImagesByKey = Object.create(null);
            const obsAfterImagesByKey = Object.create(null);
            if (summaryOnly || previewOnly) {
                this._showPptExportProgress(20, 'تجهيز الغلاف...');
                logoImgObj = await formatPptxImage(logoUrl);
            } else {
                this._showPptExportProgress(15, 'تحميل الشعار والصور...');
                const [logoRes, beforeEntries, afterEntries] = await Promise.all([
                    formatPptxImage(logoUrl),
                    mapWithConcurrency(exportBatch, 3, async (obs, index) => {
                        const key = observationImageKey(obs, index);
                        const img = await formatPptxImage(getObservationBeforeImage(obs));
                        this._showPptExportProgress(
                            15 + Math.round(((index + 1) / Math.max(1, exportBatch.length)) * 35),
                            `صور الملاحظات (${index + 1}/${exportBatch.length})`,
                            { embedded: this._pptExportEmbeddedCount, imageBudget }
                        );
                        return [key, clonePptxImage(img)];
                    }),
                    mapWithConcurrency(exportBatch, 3, async (obs, index) => {
                        const key = observationImageKey(obs, index);
                        const img = await formatPptxImage(getObservationAfterImage(obs));
                        return [key, clonePptxImage(img)];
                    })
                ]);
                logoImgObj = logoRes;
                beforeEntries.forEach(([key, img]) => { obsImagesByKey[key] = img; });
                afterEntries.forEach(([key, img]) => { obsAfterImagesByKey[key] = img; });
            }

            this._showPptExportProgress(55, 'تصميم الشرائح...');
            let slideSeq = 0;
            const detailSlidesEst = (summaryOnly || previewOnly) ? 0 : exportBatch.length;
            let totalSlidesEst = 4 + detailSlidesEst;
            const pptFooter = (slide, labelLeft) => {
                slideSeq += 1;
                slide.addShape(pptx.ShapeType.rect, { x: 0, y: 6.80, w: '100%', h: 0.55, fill: { color: 'F1F5F9' } });
                if (isEnglish) {
                    slide.addText(`${companyName} — HSE`, {
                        x: 4.0, y: 6.82, w: 5.5, h: 0.45, fontSize: 9, color: '64748B', align: 'right', ...pptTextBase, rtl: false
                    });
                    slide.addText(`Slide ${slideSeq} of ${totalSlidesEst}${labelLeft ? ' · ' + labelLeft : ''}`, {
                        x: 0.4, y: 6.82, w: 3.5, h: 0.45, fontSize: 9, color: '64748B', align: 'left', ...pptTextBase, rtl: false
                    });
                } else {
                    slide.addText(`${companyName} — HSE`, {
                        x: 0.4, y: 6.82, w: 5.5, h: 0.45, fontSize: 9, color: '64748B', align: 'right', ...pptTextBase
                    });
                    slide.addText(`شريحة ${slideSeq} من ${totalSlidesEst}${labelLeft ? ' · ' + labelLeft : ''}`, {
                        x: 9.4, y: 6.82, w: 3.5, h: 0.45, fontSize: 9, color: '64748B', align: 'left', ...pptTextBase, rtl: false
                    });
                }
            };
            ...
            // الشعار: يمين في العربية / يسار في الإنجليزية
            const coverLogoX = isEnglish ? 0.8 : 10.33;
            if (logoImgObj) {
                slide1.addShape(pptx.ShapeType.roundRect, { x: coverLogoX, y: 0.12, w: 2.2, h: 0.68, fill: { color: 'FFFFFF' }, line: { color: 'CBD5E1', width: 1 }, rectRadius: 0.08 });
                try {
                    slide1.addImage({ ...logoImgObj, x: coverLogoX + 0.05, y: 0.15, w: 2.1, h: 0.62, sizing: { type: 'contain', w: 2.1, h: 0.62 } });
                } catch(e) {}
            } else {
                slide1.addShape(pptx.ShapeType.roundRect, { x: coverLogoX, y: 0.12, w: 2.2, h: 0.68, fill: { color: 'FFFFFF' }, line: { color: 'DC2626', width: 1.5 }, rectRadius: 0.08 });
                slide1.addText('AMERICANA', { x: coverLogoX, y: 0.12, w: 2.2, h: 0.68, fontSize: 14, bold: true, color: 'DC2626', align: 'center', valign: 'middle', ...pptTextBase, rtl: false });
            }

            // ترويسة النظام في الشريط الكحلي (يمين عربي / يسار إنجليزي بجانب الشعار)
            slide1.addText(isEnglish ? 'HSE Management System — Safety & Health Department' : 'نظام إدارة السلامة والصحة المهنية والبيئة | إدارة السلامة والصحة المهنية', {
                x: isEnglish ? 4.0 : 0.4, y: 0.15, w: isEnglish ? 8.5 : 9.5, h: 0.60,
                fontSize: 13, bold: true, color: 'FFFFFF',
                align: isEnglish ? 'left' : 'right', valign: 'middle', ...pptTextBase
            });

            // ── بطاقة العنوان الرئيسية الكبرى في الوسط ──
            slide1.addShape(pptx.ShapeType.roundRect, {
                x: 1.5, y: 1.35, w: 10.33, h: 3.15, fill: { color: 'FFFFFF' }, line: { color: '3B82F6', width: 2 }, rectRadius: 0.15
            });
            slide1.addShape(pptx.ShapeType.roundRect, {
                x: 1.7, y: 1.50, w: 9.93, h: 0.04, fill: { color: 'D97706' }, line: { color: 'D97706' }
            });
            ...
            const exporterLine = isEnglish ? `Prepared by: ${exporterName}` : `أعدّه: ${exporterName}`;

            slide1.addText([
                { text: isEnglish ? 'Safety Observations Report\n\n' : 'تقرير ملاحظات السلامة\n\n', options: { fontSize: 30, bold: true, color: '1E3A8A', ...pptTextBase } },
                { text: `${companyName}\n`, options: { fontSize: 18, bold: true, color: '0F172A', ...pptTextBase } },
                { text: `${plantLabel}\n`, options: { fontSize: 16, bold: true, color: 'D97706', ...pptTextBase } },
                { text: exporterLine, options: { fontSize: 13, color: '64748B', ...pptTextBase } }
            ], { x: 1.6, y: 1.65, w: 10.13, h: 2.65, align: 'center', valign: 'middle', ...pptTextBase });

            // علامة داخلية خفيفة
            slide1.addText(isEnglish ? 'INTERNAL USE' : 'للاستخدام الداخلي', {
                x: isEnglish ? 10.2 : 1.6, y: 1.42, w: 1.5, h: 0.28, fontSize: 9, bold: true, color: '94A3B8', align: 'center', ...pptTextBase
            });

            // ── 4 بطاقات معلومات فرعية ──
            const subCards = [
                {
                ...
                {
                    title: isEnglish ? 'Period' : 'نطاق الفترة',
                    val: periodLabel,
                    bg: 'F5F3FF', line: 'C4B5FD', color: '6D28D9'
                },
                {
                ...
                {
                    title: isEnglish ? 'Observations' : 'عدد الملاحظات',
                    val: isEnglish ? `${exportBatch.length}` : `${exportBatch.length}`,
                    bg: 'ECFDF5', line: 'A7F3D0', color: '047857'
                }
            ];

            subCards.forEach((c, idx) => {
                const cx = isEnglish ? (0.85 + idx * 3.1) : (0.85 + (3 - idx) * 3.1);
                slide1.addShape(pptx.ShapeType.roundRect, { x: cx, y: 4.75, w: 2.95, h: 1.55, fill: { color: c.bg }, line: { color: c.line, width: 1.5 }, rectRadius: 0.1 });
                slide1.addText([
                    { text: c.title + '\n', options: { fontSize: 10, bold: true, color: '64748B', ...pptTextBase } },
                    { text: c.val, options: { fontSize: 12.5, bold: true, color: c.color, ...pptTextBase } }
                ], { x: cx, y: 4.85, w: 2.95, h: 1.35, align: 'center', valign: 'middle', ...pptTextBase });
            });

            // شريط سفلي
            slide1.addShape(pptx.ShapeType.rect, { x: 0, y: 6.95, w: '100%', h: 0.55, fill: { color: '1E3A8A' } });
            slide1.addText(
                (isEnglish ? `Slide 1 of ${totalSlidesEst} — ` : `شريحة 1 من ${totalSlidesEst} — `) +
                'Safety First — السلامة أولاً وبناء بيئة عمل آمنة ومستدامة',
                {
                    x: 0, y: 6.98, w: '100%', h: 0.48, fontSize: 11, bold: true, color: 'FBBF24', align: 'center', valign: 'middle', ...pptTextBase
                }
            );
            slideSeq = 1;

            if (previewOnly) {
                this._showPptExportProgress(90, 'حفظ معاينة الغلاف...');
                const previewName = `Daily_Observations_PREVIEW_${new Date().toISOString().slice(0, 10)}.pptx`;
                await pptx.writeFile({ fileName: previewName });
                this._hidePptExportProgress();
                Notification.success('✅ تم تنزيل معاينة غلاف PPT');
                return;
            }

            // ══════════════════════════════════════
            // الشريحة 2: نظرة عامة على التقرير
            // ══════════════════════════════════════
            ...
            slide2.addText(isEnglish ? 'Report Overview & Safety Performance Dashboard' : 'نظرة عامة على التقرير', {
                x: isEnglish ? 0.4 : 4.5, y: 0.05, w: isEnglish ? 8.0 : 8.4, h: 0.60, fontSize: 18, bold: true, color: 'FFFFFF',
                align: isEnglish ? 'left' : 'right', valign: 'middle', ...pptTextBase
            });
            slide2.addText(isEnglish ? `Total: ${exportBatch.length} | ${periodLabel}` : `إجمالي: ${exportBatch.length} | الفترة: ${periodLabel}`, {
                x: isEnglish ? 8.8 : 0.4, y: 0.05, w: 4.0, h: 0.60, fontSize: 11, bold: true, color: 'FBBF24',
                align: isEnglish ? 'right' : 'right', valign: 'middle', ...pptTextBase
            });

            // حساب المؤشرات
            const totalCount = exportBatch.length;
            const openCount = exportBatch.filter(o => !isObsClosed(o.status)).length;
            const closedCount = exportBatch.filter(o => isObsClosed(o.status)).length;
            const highRiskCount = exportBatch.filter(o => {
                const s = String(o.riskLevel || '').toLowerCase();
                return s.includes('عالي') || s.includes('حرج') || s.includes('high') || s.includes('critical');
            }).length;
            const mediumRiskCount = exportBatch.filter(o => {
                const s = String(o.riskLevel || '').toLowerCase();
                return s.includes('متوسط') || s.includes('medium');
            }).length;
            const lowRiskCount = Math.max(0, totalCount - highRiskCount - mediumRiskCount);

            const closedPct = Math.round((closedCount / Math.max(1, totalCount)) * 100);
            const openPct = Math.round((openCount / Math.max(1, totalCount)) * 100);
            const highRiskPct = Math.round((highRiskCount / Math.max(1, totalCount)) * 100);

            const typeCounts = {};
            exportBatch.forEach(o => {
                const t = String(o.observationType || (isEnglish ? 'General' : 'عام')).trim();
                typeCounts[t] = (typeCounts[t] || 0) + 1;
            });
            const topTypes = Object.entries(typeCounts).sort((a, b) => b[1] - a[1]).slice(0, 4);

            const uniqueSites = new Set(exportBatch.map(o => o.siteName || o.locationName).filter(Boolean)).size || 1;

            const overviewKpis = [
                {
                    label: isEnglish ? 'Total Observations' : 'إجمالي الملاحظات',
                    val: totalCount,
                    sub: isEnglish ? '100% of recorded' : 'إجمالي الحالات المرصودة',
                    color: '1E3A8A', bg: 'DBEAFE', border: '3B82F6'
                },
                {
                    label: isEnglish ? 'Open Actions' : 'ملاحظات مفتوحة',
                    val: openCount,
                    sub: isEnglish ? `${openPct}% pending actions` : `${openPct}% قيد المتابعة والتنفيذ`,
                    color: 'B45309', bg: 'FEF3C7', border: 'F59E0B'
                },
                {
                    label: isEnglish ? 'Closed Observations' : 'ملاحظات مغلقة',
                    val: closedCount,
                    sub: isEnglish ? `${closedPct}% resolution rate` : `${closedPct}% نسبة الإنجاز والمعالجة`,
                    color: '047857', bg: 'D1FAE5', border: '10B981'
                },
                {
                    label: isEnglish ? 'High Risk' : 'عالية الخطورة',
                    val: highRiskCount,
                    sub: isEnglish ? `${highRiskPct}% critical priority` : (highRiskCount > 0 ? `${highRiskPct}% تتطلب أولوية قصوى` : 'لا توجد ملاحظات حرجة'),
                    color: 'B91C1C', bg: 'FEE2E2', border: 'EF4444'
                }
            ];

            overviewKpis.forEach((k, idx) => {
                const kx = isEnglish ? (0.5 + idx * 3.1) : (0.5 + (3 - idx) * 3.1);
                slide2.addShape(pptx.ShapeType.roundRect, { x: kx, y: 0.90, w: 2.85, h: 1.35, fill: { color: k.bg }, line: { color: k.border, width: 1.5 }, rectRadius: 0.1 });
                slide2.addText([
                    { text: String(k.val) + '\n', options: { fontSize: 22, bold: true, color: k.color } },
                    { text: k.label + '\n', options: { fontSize: 11, bold: true, color: '1E293B', rtl: !isEnglish } },
                    { text: k.sub, options: { fontSize: 9, color: '64748B', rtl: !isEnglish } }
                ], { x: kx, y: 0.95, w: 2.85, h: 1.25, align: 'center', valign: 'middle', fontFace: pptFont });
            });
            ...
            // 1. رسم بياني لتوزيع الخطورة
            const col1X = isEnglish ? 0.5 : 8.88;
            slide2.addShape(pptx.ShapeType.roundRect, { x: col1X, y: cardY, w: cardW, h: cardH, fill: { color: 'FFFFFF' }, line: { color: 'CBD5E1', width: 1.5 }, rectRadius: 0.08 });
            slide2.addShape(pptx.ShapeType.rect, { x: col1X, y: cardY, w: cardW, h: 0.38, fill: { color: '1E3A8A' } });
            slide2.addText(isEnglish ? 'Risk Level Breakdown' : 'توزيع مستويات الخطورة', {
                x: col1X, y: cardY, w: cardW, h: 0.38, fontSize: 11, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont
            });
            try {
                slide2.addChart(pptx.charts.PIE, {
                    x: col1X + 0.15, y: cardY + 0.50, w: cardW - 0.3, h: 3.45,
                    series: [{
                        name: isEnglish ? 'Risk' : 'الخطورة',
                        labels: [
                            isEnglish ? 'High' : 'عالي',
                    ...
                    showLegend: true,
                    legendPos: 'b',
                    chartColors: ['B91C1C', 'D97706', '047857']
                });
            } catch (_chartErr) {
                slide2.addText([
                    { text: `${isEnglish ? 'High' : 'عالي'}: ${highRiskCount} (${highRiskPct}%)\n`, options: { color: 'B91C1C', bold: true, fontSize: 14 } },
                    { text: `${isEnglish ? 'Medium' : 'متوسط'}: ${mediumRiskCount}\n`, options: { color: 'B45309', bold: true, fontSize: 14 } },
                    { text: `${isEnglish ? 'Low' : 'منخفض'}: ${lowRiskCount}`, options: { color: '047857', bold: true, fontSize: 14 } }
                ], { x: col1X + 0.2, y: cardY + 1.2, w: cardW - 0.4, h: 2.2, align: 'center', valign: 'middle', fontFace: pptFont });
            }

            // 2. أعمدة لأنواع الملاحظات
            const col2X = 4.69;
            slide2.addShape(pptx.ShapeType.roundRect, { x: col2X, y: cardY, w: cardW, h: cardH, fill: { color: 'FFFFFF' }, line: { color: 'CBD5E1', width: 1.5 }, rectRadius: 0.08 });
            slide2.addShape(pptx.ShapeType.rect, { x: col2X, y: cardY, w: cardW, h: 0.38, fill: { color: '2563EB' } });
            slide2.addText(isEnglish ? 'Top Observation Categories' : 'أبرز مجالات وأنواع الملاحظات', {
                x: col2X, y: cardY, w: cardW, h: 0.38, fontSize: 11, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont
            });
            const typeLabels = topTypes.map(([t]) => (t.length > 18 ? t.slice(0, 18) + '…' : t));
            const typeValues = topTypes.map(([, c]) => c);
            while (typeLabels.length < 1) {
                typeLabels.push(isEnglish ? 'N/A' : 'لا يوجد');
                typeValues.push(0);
            }
            try {
                slide2.addChart(pptx.charts.BAR, {
                    x: col2X + 0.15, y: cardY + 0.50, w: cardW - 0.3, h: 3.45,
                    barGrouping: 'clustered',
                    series: [{ name: isEnglish ? 'Count' : 'العدد', labels: typeLabels, values: typeValues }],
                    showValue: true,
                    chartColors: ['2563EB'],
                    showLegend: false
                });
            } catch (_barErr) {
                const fallbackRows = topTypes.map(([t, c]) => `${t}: ${c}`).join('\n') || '—';
                slide2.addText(fallbackRows, {
                    x: col2X + 0.2, y: cardY + 0.6, w: cardW - 0.4, h: 3.3,
                    fontSize: 12, color: '0F172A', align: isEnglish ? 'left' : 'right', rtl: !isEnglish, fontFace: pptFont
                });
            }

            // 3. ملخص تنفيذي
            const col3X = isEnglish ? 8.88 : 0.5;
            slide2.addShape(pptx.ShapeType.roundRect, { x: col3X, y: cardY, w: cardW, h: cardH, fill: { color: 'FFFFFF' }, line: { color: 'CBD5E1', width: 1.5 }, rectRadius: 0.08 });
            slide2.addShape(pptx.ShapeType.rect, { x: col3X, y: cardY, w: cardW, h: 0.38, fill: { color: '059669' } });
            slide2.addText(isEnglish ? 'Executive Summary & Context' : 'الملخص التشغيلي وسياق الرصد', {
                x: col3X, y: cardY, w: cardW, h: 0.38, fontSize: 11, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont
            });

            const summaryNotes = isEnglish ? [
                { text: '• Scope & Coverage: ', options: { bold: true, color: '1E3A8A', fontSize: 10 } },
                { text: `${uniqueSites} sites/locations surveyed.\n\n`, options: { color: '0F172A', fontSize: 10 } },
                { text: '• Action Status: ', options: { bold: true, color: '1E3A8A', fontSize: 10 } },
                { text: `${closedCount} actions completed, ${openCount} in progress.\n\n`, options: { color: '0F172A', fontSize: 10 } },
                { text: '• Period: ', options: { bold: true, color: '1E3A8A', fontSize: 10 } },
                { text: `${periodLabel}\n\n`, options: { color: '0F172A', fontSize: 10 } },
                { text: '• Verification: ', options: { bold: true, color: '1E3A8A', fontSize: 10 } },
                { text: 'Follow up with site supervisors for documented closures.', options: { color: '0F172A', fontSize: 10 } }
            ] : [
                { text: '• نطاق التغطية الميدانية: ', options: { bold: true, color: '1E3A8A', fontSize: 10, rtl: true } },
                { text: `شمل الرصد ${uniqueSites} موقعاً / منطقة عمل.\n\n`, options: { color: '0F172A', fontSize: 10, rtl: true } },
                { text: '• حالة الإجراءات التصحيحية: ', options: { bold: true, color: '1E3A8A', fontSize: 10, rtl: true } },
                { text: `تم إغلاق ${closedCount} ملاحظة وجاري تنفيذ ${openCount}.\n\n`, options: { color: '0F172A', fontSize: 10, rtl: true } },
                { text: '• الفترة: ', options: { bold: true, color: '1E3A8A', fontSize: 10, rtl: true } },
                { text: `${periodLabel}\n\n`, options: { color: '0F172A', fontSize: 10, rtl: true } },
                { text: '• التوثيق الميداني: ', options: { bold: true, color: '1E3A8A', fontSize: 10, rtl: true } },
                { text: 'توثيق إجراءات الإغلاق والتصحيح بالصور والأدلة.', options: { color: '0F172A', fontSize: 10, rtl: true } }
            ];

            slide2.addText(summaryNotes, {
                x: col3X + 0.15, y: cardY + 0.50, w: cardW - 0.3, h: 3.45,
                align: isEnglish ? 'left' : 'right', valign: 'top', rtl: !isEnglish, fontFace: pptFont
            });

            pptFooter(slide2, isEnglish ? 'INTERNAL' : 'داخلي');

            // ══════════════════════════════════════
            // الشرائح 3..N: شريحة لكل ملاحظة (تُتخطى في ملخص فقط)
            // ══════════════════════════════════════
            if (!summaryOnly && !previewOnly) {
            for (let i = 0; i < exportBatch.length; i++) {
                ...
                const afterImgObj = clonePptxImage(obsAfterImagesByKey[observationImageKey(obs, i)]);
                if (skipNoImageSlides && !obsImgObj && !afterImgObj) {
                    totalSlidesEst = Math.max(4, totalSlidesEst - 1);
                    continue;
                }
                const obsSlide = pptx.addSlide();
                obsSlide.background = { color: 'F8FAFC' };
                ...
                const detailsClip = clipPptText(obs.details, 280);
                const actionClip = clipPptText(obs.correctiveAction, 240);
                const stTone = statusTone(obs.status);
                const rkTone = riskTone(obs.riskLevel);
                const overdue = this._isObservationOverdueForPpt(obs);
                const assignedLabel = String(obs.assignedToName || obs.assignedTo || '').trim() || '—';
                const stageLabel = this.getWorkflowStageLabel(obs.workflowStage);
                const targetRaw = String(obs.expectedCompletionDate || '—').slice(0, 10);
                const targetLabel = overdue
                    ? (isEnglish ? `${targetRaw} (OVERDUE)` : `${targetRaw} (متأخر)`)
                    : targetRaw;
                ...
                obsSlide.addText(isEnglish ? `Observation — ${obsLocation}` : `بطاقة الملاحظة — ${obsLocation}`, {
                    x: 5.4, y: 0.05, w: 7.5, h: 0.55, fontSize: 13.5, bold: true, color: 'FFFFFF',
                    align: isEnglish ? 'left' : 'right', rtl: !isEnglish, fontFace: pptFont
                });
                obsSlide.addText(String(obsNo), {
                    x: 0.3, y: 0.05, w: 2.4, h: 0.55, fontSize: 13, bold: true, color: 'FBBF24', align: 'left', fontFace: pptFont
                });

                // شارات الحالة والخطورة
                obsSlide.addShape(pptx.ShapeType.roundRect, {
                    x: 2.75, y: 0.12, w: 1.25, h: 0.40, fill: { color: stTone.bg }, line: { color: stTone.line, width: 1 }, rectRadius: 0.08
                });
                obsSlide.addText(String(obs.status || '—'), {
                    x: 2.75, y: 0.12, w: 1.25, h: 0.40, fontSize: 10, bold: true, color: stTone.fg, align: 'center', valign: 'middle', fontFace: pptFont
                });
                obsSlide.addShape(pptx.ShapeType.roundRect, {
                    x: 4.10, y: 0.12, w: 1.20, h: 0.40, fill: { color: rkTone.bg }, line: { color: rkTone.line, width: 1 }, rectRadius: 0.08
                });
                obsSlide.addText(String(obs.riskLevel || '—'), {
                    x: 4.10, y: 0.12, w: 1.20, h: 0.40, fontSize: 10, bold: true, color: rkTone.fg, align: 'center', valign: 'middle', fontFace: pptFont
                });
                ...
                obsSlide.addText(isEnglish ? 'Observation & Corrective Action' : 'وصف وتفاصيل الملاحظة والإجراء التصحيحي', {
                    x: 6.8, y: 0.78, w: 6.1, h: 0.38, fontSize: 11, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont
                });

                if (overdue) {
                    obsSlide.addShape(pptx.ShapeType.roundRect, {
                        x: 5.35, y: 0.12, w: 1.15, h: 0.40, fill: { color: 'FEE2E2' }, line: { color: 'DC2626', width: 1 }, rectRadius: 0.08
                    });
                    obsSlide.addText(isEnglish ? 'OVERDUE' : 'متأخر', {
                        x: 5.35, y: 0.12, w: 1.15, h: 0.40, fontSize: 9, bold: true, color: 'B91C1C', align: 'center', valign: 'middle', fontFace: pptFont
                    });
                }

                const detailsRows = [
                    [
                        { text: String(obsNo), options: { bold: true, color: '1E40AF', fontSize: 10, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Observation No' : 'رقم الملاحظة', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ],
                    [
                        { text: String(obsDate), options: { color: '0F172A', fontSize: 10, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Date' : 'تاريخ الرصد', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ],
                    [
                        { text: String(obsLocation), options: { color: '0F172A', fontSize: 9.5, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Location' : 'المكان / الموقع', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ],
                    [
                        { text: detailsClip.text, options: { color: '1E293B', fontSize: 9, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Description' : 'وصف الملاحظة', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ],
                    [
                        { text: actionClip.text, options: { color: '047857', bold: true, fontSize: 9, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Corrective Action' : 'الإجراء التصحيحي', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ],
                    [
                        { text: targetLabel, options: { color: overdue ? 'B91C1C' : '0F172A', bold: overdue, fontSize: 10, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Target Date' : 'تاريخ التنفيذ المقترح', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ],
                    [
                        { text: String(obs.responsibleDepartment || '—'), options: { color: '0F172A', fontSize: 9.5, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Responsible Dept' : 'الإدارة المسؤولة', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ],
                    [
                        { text: assignedLabel, options: { color: '1E40AF', bold: true, fontSize: 9.5, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Assigned To' : 'المسؤول المعيَّن', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ],
                    [
                        { text: String(stageLabel || '—'), options: { color: '0F172A', fontSize: 9, align: 'right', rtl: !isEnglish } },
                        { text: isEnglish ? 'Workflow Stage' : 'مرحلة السير', options: { bold: true, color: 'DC2626', fontSize: 10, align: 'right', fill: 'F8FAFC', rtl: !isEnglish } }
                    ]
                ];

                obsSlide.addTable(detailsRows, {
                    x: 6.8, y: 1.16, w: 6.1, h: 5.47, colW: [4.3, 1.8], border: { pt: 0.5, color: 'E2E8F0' }, fontFace: pptFont
                });

                // صور قبل / بعد
                const hasBefore = !!(obsImgObj && obsImgObj.data);
                const hasAfter = !!(afterImgObj && afterImgObj.data);
                ...
                if (hasBefore && hasAfter) {
                    ...
                    obsSlide.addText(isEnglish ? 'Before  |  After' : 'قبل التنفيذ  |  بعد الإغلاق', {
                        x: 0.4, y: 0.78, w: 6.1, h: 0.38, fontSize: 11, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont
                    });
                    obsSlide.addText(isEnglish ? 'Before' : 'قبل', {
                        x: 0.5, y: 1.20, w: 2.9, h: 0.28, fontSize: 10, bold: true, color: 'B45309', align: 'center', fontFace: pptFont
                    });
                    obsSlide.addText(isEnglish ? 'After' : 'بعد', {
                        x: 3.5, y: 1.20, w: 2.9, h: 0.28, fontSize: 10, bold: true, color: '047857', align: 'center', fontFace: pptFont
                    });
                    try {
                        obsSlide.addImage({ data: obsImgObj.data, x: 0.55, y: 1.55, w: 2.8, h: 4.85, sizing: { type: 'contain', w: 2.8, h: 4.85 } });
                    } catch (_e) {}
                    try {
                        obsSlide.addImage({ data: afterImgObj.data, x: 3.55, y: 1.55, w: 2.8, h: 4.85, sizing: { type: 'contain', w: 2.8, h: 4.85 } });
                    } catch (_e) {}
                } else {
                    const single = hasBefore ? obsImgObj : (hasAfter ? afterImgObj : null);
                    const title = hasAfter && !hasBefore
                        ? (isEnglish ? 'After Execution Photo' : 'صورة بعد التنفيذ')
                        : (isEnglish ? 'Observation Photo' : 'الصورة التوضيحية للملاحظة');
                    obsSlide.addShape(pptx.ShapeType.rect, { x: 0.4, y: 0.78, w: 6.1, h: 0.38, fill: { color: '64748B' } });
                    obsSlide.addText(title, {
                        x: 0.4, y: 0.78, w: 6.1, h: 0.38, fontSize: 11, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont
                    });
                    if (single && single.data) {
                        try {
                            obsSlide.addImage({ data: single.data, x: 0.55, y: 1.25, w: 5.8, h: 5.25, sizing: { type: 'contain', w: 5.8, h: 5.25 } });
                        } catch (imgErr) {
                            obsSlide.addText(isEnglish ? 'Photo attached in system' : 'صورة الملاحظة مرفقة بالنظام', {
                                x: 0.8, y: 1.6, w: 5.3, h: 4.5, fontSize: 13, color: '64748B', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont
                            });
                        }
                    } else {
                        const hadPhoto = this.observationHasRealImages(obs) || Number(obs.attachmentCount) > 0 || Number(obs.afterImageCount) > 0;
                        obsSlide.addText(
                            hadPhoto
                            ...
                            { x: 0.8, y: 1.6, w: 5.3, h: 4.5, fontSize: 13, color: '94A3B8', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont }
                        );
                    }
                }

                pptFooter(obsSlide, overdue ? (isEnglish ? 'OVERDUE' : 'متأخر') : (isEnglish ? 'INTERNAL' : 'داخلي'));

                // شريحة متابعة للنص الطويل
                if (detailsClip.clipped || actionClip.clipped) {
                    totalSlidesEst += 1;
                    const contSlide = pptx.addSlide();
                    contSlide.background = { color: 'F8FAFC' };
                    contSlide.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: '100%', h: 0.65, fill: { color: '1E3A8A' } });
                    contSlide.addShape(pptx.ShapeType.rect, { x: 0, y: 0.65, w: '100%', h: 0.04, fill: { color: 'D97706' } });
                    contSlide.addText(isEnglish ? `Full text — ${obsNo}` : `النص الكامل — ${obsNo}`, {
                        x: 0.5, y: 0.05, w: 12.3, h: 0.55, fontSize: 16, bold: true, color: 'FFFFFF',
                        align: isEnglish ? 'left' : 'right', rtl: !isEnglish, fontFace: pptFont
                    });
                    contSlide.addShape(pptx.ShapeType.roundRect, { x: 0.5, y: 0.90, w: 12.3, h: 5.70, fill: { color: 'FFFFFF' }, line: { color: 'CBD5E1', width: 1.5 }, rectRadius: 0.08 });
                    const fullBlocks = [];
                    if (detailsClip.clipped) {
                        fullBlocks.push(
                            { text: (isEnglish ? 'Description\n' : 'وصف الملاحظة\n'), options: { bold: true, color: '1E3A8A', fontSize: 13, rtl: !isEnglish } },
                            { text: detailsClip.overflow + '\n\n', options: { color: '0F172A', fontSize: 12, rtl: !isEnglish } }
                        );
                    }
                    if (actionClip.clipped) {
                        fullBlocks.push(
                            { text: (isEnglish ? 'Corrective Action\n' : 'الإجراء التصحيحي\n'), options: { bold: true, color: '047857', fontSize: 13, rtl: !isEnglish } },
                            { text: actionClip.overflow, options: { color: '0F172A', fontSize: 12, rtl: !isEnglish } }
                        );
                    }
                    contSlide.addText(fullBlocks, {
                        x: 0.7, y: 1.10, w: 11.9, h: 5.30,
                        align: isEnglish ? 'left' : 'right', valign: 'top', rtl: !isEnglish, fontFace: pptFont
                    });
                    pptFooter(contSlide, isEnglish ? 'Continuation' : 'متابعة');
                }
            }
            } // end !summaryOnly

            this._showPptExportProgress(82, 'شريحة الأولويات...');
            const summarySlide = pptx.addSlide();
            summarySlide.background = { color: 'F8FAFC' };

            summarySlide.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: '100%', h: 0.70, fill: { color: '1E3A8A' } });
            summarySlide.addShape(pptx.ShapeType.rect, { x: 0, y: 0.70, w: '100%', h: 0.04, fill: { color: 'D97706' } });
            summarySlide.addText(isEnglish ? 'Executive Summary & Priority Actions' : 'الملخص التنفيذي وأولويات المتابعة', {
                x: 0.8, y: 0.05, w: 11.7, h: 0.60, fontSize: 18, bold: true, color: 'FFFFFF',
                align: isEnglish ? 'left' : 'right', rtl: !isEnglish, fontFace: pptFont
            });

            const resolutionRate = Math.round((closedCount / Math.max(1, totalCount)) * 100);
            const highRiskRate = Math.round((highRiskCount / Math.max(1, totalCount)) * 100);

            const summaryKpis = [
                {
                    title: isEnglish ? 'Resolution Rate' : 'معدل إغلاق الملاحظات',
                    val: `${resolutionRate}%`,
                    sub: isEnglish ? `${closedCount} of ${totalCount} closed` : `تم إغلاق ${closedCount} من إجمالي ${totalCount}`,
                    color: '047857', bg: 'D1FAE5'
                },
                {
                    title: isEnglish ? 'High Risk Ratio' : 'نسبة الحالات عالية الخطورة',
                    val: `${highRiskRate}%`,
                    sub: isEnglish ? `${highRiskCount} critical items` : `${highRiskCount} ملاحظة تتطلب تركيزاً فورياً`,
                    color: 'B91C1C', bg: 'FEE2E2'
                },
                {
                    title: isEnglish ? 'Pending Actions' : 'الملاحظات قيد المتابعة',
                    val: `${openCount}`,
                    sub: isEnglish ? 'Open items in progress' : 'ملاحظات جاري استكمال تصحيحها',
                    color: 'B45309', bg: 'FEF3C7'
                }
            ];

            summaryKpis.forEach((sk, idx) => {
                const kx = isEnglish ? (0.6 + idx * 4.1) : (0.6 + (2 - idx) * 4.1);
                summarySlide.addShape(pptx.ShapeType.roundRect, { x: kx, y: 0.90, w: 3.85, h: 1.25, fill: { color: sk.bg }, line: { color: sk.color, width: 1.5 }, rectRadius: 0.1 });
                summarySlide.addText([
                    { text: sk.val + '\n', options: { fontSize: 22, bold: true, color: sk.color } },
                    { text: sk.title + '\n', options: { fontSize: 11, bold: true, color: '1E293B', rtl: !isEnglish } },
                    { text: sk.sub, options: { fontSize: 9.5, color: '64748B', rtl: !isEnglish } }
                ], { x: kx, y: 0.95, w: 3.85, h: 1.15, align: 'center', valign: 'middle', fontFace: pptFont });
            });

            // أعلى 5 مفتوحة عالية الخطورة
            const topOpenHigh = exportBatch
                .filter((o) => !isObsClosed(o.status))
                .filter((o) => {
                    const s = String(o.riskLevel || '').toLowerCase();
                    return s.includes('عالي') || s.includes('حرج') || s.includes('high') || s.includes('critical')
                        || s.includes('متوسط') || s.includes('medium')
                        || this._isObservationOverdueForPpt(o);
                })
                .slice(0, 5);

            summarySlide.addShape(pptx.ShapeType.roundRect, { x: 0.5, y: 2.30, w: 12.3, h: 4.30, fill: { color: 'FFFFFF' }, line: { color: 'CBD5E1', width: 1.5 }, rectRadius: 0.08 });
            summarySlide.addShape(pptx.ShapeType.rect, { x: 0.5, y: 2.30, w: 12.3, h: 0.38, fill: { color: 'B91C1C' } });
            summarySlide.addText(isEnglish ? 'Top 5 Open / High-Priority Observations' : 'أعلى 5 ملاحظات مفتوحة ذات أولوية', {
                x: 0.5, y: 2.30, w: 12.3, h: 0.38, fontSize: 12, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', rtl: !isEnglish, fontFace: pptFont
            });

            const topRows = [[
                { text: isEnglish ? 'Target' : 'الموعد', options: { bold: true, fill: 'F8FAFC', color: '1E3A8A', align: 'center', rtl: !isEnglish } },
                { text: isEnglish ? 'Assignee' : 'المعيَّن', options: { bold: true, fill: 'F8FAFC', color: '1E3A8A', align: 'center', rtl: !isEnglish } },
                { text: isEnglish ? 'Location' : 'الموقع', options: { bold: true, fill: 'F8FAFC', color: '1E3A8A', align: 'center', rtl: !isEnglish } },
                { text: isEnglish ? 'Risk' : 'الخطورة', options: { bold: true, fill: 'F8FAFC', color: '1E3A8A', align: 'center', rtl: !isEnglish } },
                { text: isEnglish ? 'ID' : 'الرقم', options: { bold: true, fill: 'F8FAFC', color: '1E3A8A', align: 'center', rtl: !isEnglish } }
            ]];
            if (topOpenHigh.length === 0) {
                topRows.push([
                    { text: '—', options: { color: '94A3B8', align: 'center' } },
                    { text: isEnglish ? 'No open high-priority items in this export set.' : 'لا توجد ملاحظات مفتوحة عالية الأولوية ضمن هذه الدفعة.', options: { color: '64748B', align: 'center', rtl: !isEnglish } },
                    { text: '—', options: { color: '94A3B8', align: 'center' } },
                    { text: '—', options: { color: '94A3B8', align: 'center' } },
                    { text: '—', options: { color: '94A3B8', align: 'center' } }
                ]);
            } else {
                topOpenHigh.forEach((o) => {
                    const od = this._isObservationOverdueForPpt(o);
                    const t = String(o.expectedCompletionDate || '—').slice(0, 10);
                    topRows.push([
                        { text: od ? `${t} !` : t, options: { fontSize: 10, align: 'center', bold: od, color: od ? 'B91C1C' : '0F172A' } },
                        { text: String(o.assignedToName || o.responsibleDepartment || '—').slice(0, 22), options: { fontSize: 10, align: 'center', color: '0F172A', rtl: !isEnglish } },
                        { text: String([o.siteName, o.locationName].filter(Boolean).join(' / ') || '—').slice(0, 28), options: { fontSize: 10, align: 'center', color: '0F172A', rtl: !isEnglish } },
                        { text: String(o.riskLevel || '—'), options: { fontSize: 10, bold: true, align: 'center', color: riskTone(o.riskLevel).fg } },
                        { text: String(o.isoCode || o.id || '—'), options: { fontSize: 10, bold: true, align: 'center', color: '1E40AF' } }
                    ]);
                });
            }
            summarySlide.addTable(topRows, {
                x: 0.7, y: 2.85, w: 11.9, h: 3.50,
                colW: [1.6, 2.6, 3.6, 1.6, 2.5], fontSize: 10, border: { pt: 0.5, color: 'E2E8F0' }, fontFace: pptFont
            });

            pptFooter(summarySlide, isEnglish ? 'INTERNAL' : 'داخلي');
            ...
            endSlide.addText(isEnglish ? 'Closing — Safety Commitment' : 'ختام — التزام السلامة', {
                x: isEnglish ? 0.5 : 0.5, y: 0.12, w: 12.3, h: 0.55, fontSize: 18, bold: true, color: 'FFFFFF',
                align: isEnglish ? 'left' : 'right', valign: 'middle', ...pptTextBase
            });
            ...
            endSlide.addText([
                { text: isEnglish ? 'Thank you for your continuous commitment to Safety Standards\n\n' : 'شكراً لالتزامكم الدائم والمستمر بمعايير السلامة والصحة المهنية\n\n', options: { fontSize: 22, bold: true, color: '1E3A8A', ...pptTextBase } },
                { text: isEnglish ? 'Safety First — Americana Group\n' : 'السلامة أولاً — مجموعة أمريكانا\n', options: { fontSize: 18, bold: true, color: 'D97706', ...pptTextBase } },
                { text: isEnglish ? 'Working Together for a Zero-Harm Environment' : 'نعمل معاً من أجل بيئة عمل خالية من الحوادث والمخاطر', options: { fontSize: 12.5, color: '64748B', ...pptTextBase } }
            ], { x: 2.1, y: 1.90, w: 9.13, h: 3.6, align: 'center', valign: 'middle', ...pptTextBase });

            endSlide.addShape(pptx.ShapeType.rect, { x: 0, y: 6.95, w: '100%', h: 0.55, fill: { color: '1E3A8A' } });
            endSlide.addText(
                (isEnglish ? `Slide ${slideSeq + 1} of ${totalSlidesEst} — ` : `شريحة ${slideSeq + 1} من ${totalSlidesEst} — `) +
                `${companyName} — HSE Management`,
                {
                    x: 0, y: 6.98, w: '100%', h: 0.48, fontSize: 11, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', ...pptTextBase
                }
            );

            this._showPptExportProgress(92, 'حفظ الملف...');
            const outputFilename = `Daily_Observations_${safeDept}_${new Date().toISOString().slice(0, 10)}.pptx`;
            const wantPptx = outputFormat !== 'pdf';
            ...
            if (wantPdf) {
                this._showPptExportProgress(96, 'توليد PDF مطابق للشرائح...');
                const effectivePdfMode = summaryOnly
                    ? 'summary'
                    : (pdfMode === 'match' ? 'full' : (pdfMode || 'full'));
                await this._exportObservationsReportPdfCompanion({
                    exportBatch,
                    isEnglish,
                    companyName,
                    periodLabel,
                    ...
                    exporterName,
                    logoDataUrl: logoImgObj?.data || '',
                    obsImagesByKey,
                    obsAfterImagesByKey,
                    skipNoImageSlides,
                    summaryOnly: !!summaryOnly
                });
                extras.push(effectivePdfMode === 'summary' ? 'PDF ملخص' : (effectivePdfMode === 'top10' ? 'PDF أول 10' : 'PDF مطابق'));
            }
            if (exportExcelCompanion) {
                this._showPptExportProgress(98, 'تصدير Excel مرافق...');
                await this._exportObservationsExcelBatch(exportBatch, { quiet: true });
                extras.push('Excel');
            }
            this._hidePptExportProgress();
            const folderId = String(
                (typeof AppState !== 'undefined' && (AppState?.config?.reportsOutputFolderId || AppState?.appData?.reportsOutputFolderId)) ||
                localStorage.getItem('REPORTS_OUTPUT_FOLDER_ID') ||
                ''
            ).trim();
            const baseMsg = summaryOnly
                ? `تم تصدير الملخص (${exportBatch.length} ملاحظة)`
                : `تم التصدير بنجاح (${exportBatch.length} ملاحظة)`;
            const imgNote = this._pptExportImageBudget
                ? ` — صور ${this._pptExportEmbeddedCount || 0}/${this._pptExportImageBudget}`
                : '';
            const msg = `${baseMsg}${extras.length ? ' · ' + extras.join(' + ') : ''}${imgNote}`;
            const notifOpts = { duration: 8000 };
            if (folderId) {
                notifOpts.actions = [{
                    label: 'فتح مجلد التقارير',
                    primary: true,
                    onClick: () => {
                        window.open(`https://drive.google.com/drive/folders/${encodeURIComponent(folderId)}`, '_blank', 'noopener');
                    }
                }];
                notifOpts.persistent = true;
            }
            Notification.success(msg, notifOpts);
        } catch (error) {
            this._hidePptExportProgress();
    ...
    async _ensureArabicPdfFontBase64() {
        if (this._arabicFontBase64) return this._arabicFontBase64;
        const fontUrls = [
        ...
        for (const url of fontUrls) {
            try {
                ...
                if (!resp.ok) continue;
                if (String(resp.headers.get('content-type') || '').includes('text/css')) {
                    const css = await resp.text();
                    const match = css.match(/url\(([^)]+\.ttf)\)/);
                    if (!match) continue;
                    const ttfResp = await fetch(match[1].replace(/['"]/g, ''), { cache: 'force-cache' });
                    if (!ttfResp.ok) continue;
                    this._arabicFontBase64 = await ttfResp.blob().then((b) => new Promise((r) => {
    ...
    async _exportObservationsReportPdfCompanion({
        exportBatch = [],
        isEnglish = false,
        companyName = '',
        periodLabel = '',
        ...
        logoDataUrl = '',
        obsImagesByKey = null,
        obsAfterImagesByKey = null,
        skipNoImageSlides = false,
        summaryOnly = false
    } = {}) {
        await this._loadAnalyticsLib(
        ...
        const { jsPDF } = window.jspdf || {};
        if (!jsPDF || typeof html2canvas !== 'function') throw new Error('PDF libs unavailable');
        ...
        const fontStack = '"Cairo","Tahoma","Segoe UI",Arial,sans-serif';
        const plantLabel = isEnglish ? `Plant : ${site || 'All Plants'}` : `المصنع : ${site || 'جميع المصانع'}`;
        const exporterLine = isEnglish ? `Prepared by: ${exporterName || 'HSE User'}` : `أعدّه: ${exporterName || 'مستخدم النظام'}`;
        const resolutionRate = Math.round((closedCount / Math.max(1, totalCount)) * 100);
        const highRiskRate = Math.round((highRiskCount / Math.max(1, totalCount)) * 100);
        const imgMap = obsImagesByKey && typeof obsImagesByKey === 'object' ? obsImagesByKey : {};
        const afterMap = obsAfterImagesByKey && typeof obsAfterImagesByKey === 'object' ? obsAfterImagesByKey : {};
        const obsKey = (obs, index) => String(obs?.id || obs?.isoCode || ('idx-' + index));
        const imgSrc = (obj) => (obj && obj.data) ? String(obj.data) : '';

        let detailRows = summaryOnly ? [] : [...exportBatch];
        if (pdfMode === 'summary') detailRows = [];
        else if (pdfMode === 'top10') detailRows = detailRows.slice(0, 10);
        else detailRows = detailRows.slice(0, 40);

        if (skipNoImageSlides) {
            detailRows = detailRows.filter((obs, i) => {
                const k = obsKey(obs, exportBatch.indexOf(obs) >= 0 ? exportBatch.indexOf(obs) : i);
                return !!(imgSrc(imgMap[k]) || imgSrc(afterMap[k]));
            });
        }

        const host = document.createElement('div');
        host.id = 'dailyobs-pdf-mirror-host';
        host.style.cssText = 'position:fixed;left:-14000px;top:0;width:1280px;z-index:-1;pointer-events:none;opacity:1;';
        host.setAttribute('dir', dir);

        const slideCss = `
            .dob-pdf-slide{width:1280px;height:720px;background:#F8FAFC;box-sizing:border-box;position:relative;overflow:hidden;font-family:${fontStack};direction:${dir};}
        ...
        const logoHtml = logoDataUrl
            ? `<img src="${esc(logoDataUrl)}" alt="logo" style="height:52px;max-width:160px;object-fit:contain;background:#fff;border-radius:8px;padding:4px 8px;">`
            : `<div style="background:#fff;color:#DC2626;font-weight:800;padding:10px 16px;border-radius:8px;">AMERICANA</div>`;

        // 1) Cover — مثل شريحة الغلاف
        mkSlide(`
            <div class="dob-pdf-hdr">
                <div style="font-size:16px;font-weight:800;text-align:${align};flex:1;">${esc(isEnglish ? 'HSE Management System — Safety & Health Department' : 'نظام إدارة السلامة والصحة المهنية والبيئة | إدارة السلامة والصحة المهنية')}</div>
                <div style="margin-${isEnglish ? 'left' : 'right'}:16px;">${logoHtml}</div>
            </div>
            ...
            <div style="padding:36px 48px 70px;">
                <div class="dob-pdf-card" style="text-align:center;min-height:250px;display:flex;flex-direction:column;justify-content:center;gap:10px;">
                    <div style="font-size:34px;" class="dob-pdf-title">${esc(isEnglish ? 'Safety Observations Report' : 'تقرير ملاحظات السلامة')}</div>
                    <div style="font-size:20px;font-weight:800;color:#0F172A;">${esc(companyName)}</div>
                    <div class="dob-pdf-amber" style="font-size:18px;">${esc(plantLabel)}</div>
                    <div class="dob-pdf-muted" style="font-size:15px;">${esc(exporterLine)}</div>
                </div>
                <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:22px;">
                    <div class="dob-pdf-kpi" style="background:#EFF6FF;border-color:#93C5FD;"><div class="dob-pdf-muted" style="font-size:12px;font-weight:700;">${esc(isEnglish ? 'Department' : 'الإدارة المختارة')}</div><div style="font-size:15px;font-weight:800;color:#1E40AF;margin-top:6px;">${esc(dept || (isEnglish ? 'All' : 'كافة الإدارات'))}</div></div>
                    <div class="dob-pdf-kpi" style="background:#F5F3FF;border-color:#C4B5FD;"><div class="dob-pdf-muted" style="font-size:12px;font-weight:700;">${esc(isEnglish ? 'Period' : 'نطاق الفترة')}</div><div style="font-size:15px;font-weight:800;color:#6D28D9;margin-top:6px;">${esc(periodLabel)}</div></div>
                    <div class="dob-pdf-kpi" style="background:#FEF3C7;border-color:#FDE68A;"><div class="dob-pdf-muted" style="font-size:12px;font-weight:700;">${esc(isEnglish ? 'Report Date' : 'تاريخ التقرير')}</div><div style="font-size:15px;font-weight:800;color:#B45309;margin-top:6px;">${esc(dateLabel)}</div></div>
                    <div class="dob-pdf-kpi" style="background:#ECFDF5;border-color:#A7F3D0;"><div class="dob-pdf-muted" style="font-size:12px;font-weight:700;">${esc(isEnglish ? 'Observations' : 'عدد الملاحظات')}</div><div style="font-size:15px;font-weight:800;color:#047857;margin-top:6px;">${esc(String(totalCount || exportBatch.length))}</div></div>
                </div>
            </div>
            <div class="dob-pdf-ftr">${esc(isEnglish ? 'Safety First' : 'السلامة أولاً — وبناء بيئة عمل آمنة ومستدامة')}</div>
        `);

        // 2) Overview — مثل شريحة النظرة العامة
        mkSlide(`
            <div class="dob-pdf-hdr">
                <div style="font-size:20px;font-weight:800;">${esc(isEnglish ? 'Report Overview & Safety Performance Dashboard' : 'نظرة عامة على التقرير')}</div>
                <div style="color:#FBBF24;font-weight:800;font-size:13px;">${esc(isEnglish ? `Total: ${totalCount}` : `إجمالي: ${totalCount}`)} | ${esc(periodLabel)}</div>
            </div>
            <div class="dob-pdf-hdr-gold"></div>
            <div style="padding:28px 36px 70px;display:grid;grid-template-columns:repeat(3,1fr);gap:16px;">
                <div class="dob-pdf-kpi" style="background:#D1FAE5;border-color:#047857;"><div style="font-size:28px;font-weight:800;color:#047857;">${resolutionRate}%</div><div style="font-weight:800;margin-top:4px;">${esc(isEnglish ? 'Resolution Rate' : 'معدل إغلاق الملاحظات')}</div><div class="dob-pdf-muted" style="font-size:12px;margin-top:4px;">${esc(isEnglish ? `${closedCount} of ${totalCount} closed` : `تم إغلاق ${closedCount} من ${totalCount}`)}</div></div>
                <div class="dob-pdf-kpi" style="background:#FEE2E2;border-color:#B91C1C;"><div style="font-size:28px;font-weight:800;color:#B91C1C;">${highRiskRate}%</div><div style="font-weight:800;margin-top:4px;">${esc(isEnglish ? 'High Risk Ratio' : 'نسبة الحالات عالية الخطورة')}</div><div class="dob-pdf-muted" style="font-size:12px;margin-top:4px;">${esc(String(highRiskCount))}</div></div>
                <div class="dob-pdf-kpi" style="background:#FEF3C7;border-color:#B45309;"><div style="font-size:28px;font-weight:800;color:#B45309;">${openCount}</div><div style="font-weight:800;margin-top:4px;">${esc(isEnglish ? 'Pending Actions' : 'الملاحظات قيد المتابعة')}</div><div class="dob-pdf-muted" style="font-size:12px;margin-top:4px;">${esc(isEnglish ? 'Open items' : 'جاري التصحيح')}</div></div>
                <div class="dob-pdf-card" style="grid-column:1/-1;margin-top:8px;">
                    <div style="font-weight:800;color:#1E3A8A;margin-bottom:10px;font-size:16px;">${esc(isEnglish ? 'Status snapshot' : 'ملخص الحالة')}</div>
                    <div style="display:flex;gap:24px;flex-wrap:wrap;font-size:14px;font-weight:700;color:#0F172A;">
                        <span>${esc(isEnglish ? 'Open' : 'مفتوح')}: ${openCount}</span>
                        <span>${esc(isEnglish ? 'Closed' : 'مغلق')}: ${closedCount}</span>
                        <span>${esc(isEnglish ? 'High risk' : 'عالي الخطورة')}: ${highRiskCount}</span>
                        <span>${esc(isEnglish ? 'Site' : 'الموقع')}: ${esc(site || (isEnglish ? 'All' : 'الكل'))}</span>
                    </div>
                </div>
            </div>
            <div class="dob-pdf-ftr">${esc(companyName)} — HSE</div>
        `);

        // 3) Observation cards — مثل بطاقات PPT
        detailRows.forEach((obs, i) => {
            const origIdx = (() => {
                const id = String(obs?.id || '');
                if (id) {
                    const j = exportBatch.findIndex((x) => String(x?.id || '') === id);
                    if (j >= 0) return j;
                }
                const j2 = exportBatch.indexOf(obs);
                return j2 >= 0 ? j2 : i;
            })();
            const k = obsKey(obs, origIdx);
            const before = imgSrc(imgMap[k]);
            const after = imgSrc(afterMap[k]);
            const loc = [obs.siteName, obs.locationName].filter(Boolean).join(' - ') || '—';
            const overdue = this._isObservationOverdueForPpt(obs);
            mkSlide(`
                <div class="dob-pdf-hdr" style="height:58px;">
                    <div style="font-size:16px;font-weight:800;">${esc(isEnglish ? `Observation — ${loc}` : `بطاقة الملاحظة — ${loc}`)}</div>
                    <div style="display:flex;gap:8px;align-items:center;">
                        ${overdue ? `<span style="background:#FEE2E2;color:#B91C1C;border:1px solid #DC2626;border-radius:8px;padding:4px 10px;font-size:12px;font-weight:800;">${esc(isEnglish ? 'OVERDUE' : 'متأخر')}</span>` : ''}
                        <span style="background:#FBBF24;color:#1E3A8A;border-radius:8px;padding:4px 10px;font-size:13px;font-weight:800;">${esc(obs.isoCode || obs.id || `OBS-${i + 1}`)}</span>
                    </div>
                </div>
                <div class="dob-pdf-hdr-gold"></div>
                <div style="padding:18px 24px 64px;display:grid;grid-template-columns:1.05fr 0.95fr;gap:16px;height:calc(720px - 62px - 48px);box-sizing:border-box;">
                    <div class="dob-pdf-card" style="padding:0;overflow:hidden;">
                        <div style="background:#2563EB;color:#fff;font-weight:800;font-size:13px;padding:10px;text-align:center;">${esc(isEnglish ? 'Observation & Corrective Action' : 'وصف وتفاصيل الملاحظة والإجراء التصحيحي')}</div>
                        <div style="padding:12px 14px;font-size:13px;line-height:1.55;text-align:${align};">
                            <div style="margin-bottom:8px;"><b style="color:#DC2626;">${esc(isEnglish ? 'Status' : 'الحالة')}:</b> ${esc(obs.status || '—')} &nbsp;|&nbsp; <b style="color:#DC2626;">${esc(isEnglish ? 'Risk' : 'الخطورة')}:</b> ${esc(obs.riskLevel || '—')}</div>
                            <div style="margin-bottom:8px;"><b style="color:#DC2626;">${esc(isEnglish ? 'Assignee' : 'المعيَّن')}:</b> ${esc(obs.assignedToName || obs.assignedTo || '—')}</div>
                            <div style="margin-bottom:8px;"><b style="color:#DC2626;">${esc(isEnglish ? 'Target' : 'الموعد')}:</b> ${esc(String(obs.expectedCompletionDate || '—').slice(0, 10))}</div>
                            <div style="margin-bottom:6px;font-weight:800;color:#1E3A8A;">${esc(isEnglish ? 'Details' : 'الوصف')}</div>
                            <div style="color:#1E293B;margin-bottom:12px;">${esc(String(obs.details || '—').slice(0, 420))}</div>
                            <div style="margin-bottom:6px;font-weight:800;color:#047857;">${esc(isEnglish ? 'Corrective Action' : 'الإجراء التصحيحي')}</div>
                            <div style="color:#047857;font-weight:700;">${esc(String(obs.correctiveAction || '—').slice(0, 360))}</div>
                        </div>
                    </div>
                    <div style="display:grid;grid-template-rows:1fr 1fr;gap:12px;">
                        <div class="dob-pdf-card" style="padding:10px;display:flex;flex-direction:column;">
                            <div style="font-weight:800;color:#B45309;font-size:12px;margin-bottom:6px;text-align:center;">${esc(isEnglish ? 'Before' : 'قبل التنفيذ')}</div>
                            ${before ? `<img src="${esc(before)}" style="flex:1;width:100%;object-fit:contain;border-radius:8px;background:#F1F5F9;">` : `<div style="flex:1;display:flex;align-items:center;justify-content:center;color:#94A3B8;font-weight:700;">${esc(isEnglish ? 'No image' : 'لا توجد صورة')}</div>`}
                        </div>
                        <div class="dob-pdf-card" style="padding:10px;display:flex;flex-direction:column;">
                            <div style="font-weight:800;color:#047857;font-size:12px;margin-bottom:6px;text-align:center;">${esc(isEnglish ? 'After' : 'بعد التنفيذ')}</div>
                            ${after ? `<img src="${esc(after)}" style="flex:1;width:100%;object-fit:contain;border-radius:8px;background:#F1F5F9;">` : `<div style="flex:1;display:flex;align-items:center;justify-content:center;color:#94A3B8;font-weight:700;">${esc(isEnglish ? 'No image' : 'لا توجد صورة')}</div>`}
                        </div>
                    </div>
                </div>
                <div class="dob-pdf-ftr">${esc(isEnglish ? `Observation ${i + 1} of ${detailRows.length}` : `ملاحظة ${i + 1} من ${detailRows.length}`)}</div>
            `);
        });

        // 4) Priorities
        const topRowsHtml = (topOpenHigh || []).slice(0, 5).map((o, idx) => {
            const od = this._isObservationOverdueForPpt(o);
            return `<tr style="border-bottom:1px solid #E2E8F0;">
                <td style="padding:8px;text-align:center;font-weight:800;color:#1E40AF;">${esc(o.isoCode || o.id || '—')}</td>
                <td style="padding:8px;text-align:center;font-weight:700;color:${od ? '#B91C1C' : '#0F172A'};">${esc(o.riskLevel || '—')}</td>
                <td style="padding:8px;text-align:center;">${esc(String(o.assignedToName || o.responsibleDepartment || '—').slice(0, 28))}</td>
                <td style="padding:8px;text-align:center;color:${od ? '#B91C1C' : '#0F172A'};font-weight:${od ? 800 : 600};">${esc(String(o.expectedCompletionDate || '—').slice(0, 10))}${od ? (isEnglish ? ' !' : ' !') : ''}</td>
            </tr>`;
        }).join('') || `<tr><td colspan="4" style="padding:16px;text-align:center;color:#64748B;">${esc(isEnglish ? 'No open high-priority items.' : 'لا توجد أولويات مفتوحة ضمن الدفعة.')}</td></tr>`;

        mkSlide(`
            <div class="dob-pdf-hdr"><div style="font-size:20px;font-weight:800;">${esc(isEnglish ? 'Executive Summary & Priority Actions' : 'الملخص التنفيذي وأولويات المتابعة')}</div></div>
            <div class="dob-pdf-hdr-gold"></div>
            <div style="padding:24px 36px 70px;">
                <div class="dob-pdf-card" style="padding:0;overflow:hidden;">
                    <div style="background:#B91C1C;color:#fff;font-weight:800;padding:10px;text-align:center;">${esc(isEnglish ? 'Top 5 Open / High-Priority Observations' : 'أعلى 5 ملاحظات مفتوحة ذات أولوية')}</div>
                    <table style="width:100%;border-collapse:collapse;font-size:13px;">
                        <thead><tr style="background:#F8FAFC;color:#1E3A8A;">
                            <th style="padding:10px;">${esc(isEnglish ? 'ID' : 'الرقم')}</th>
                            <th style="padding:10px;">${esc(isEnglish ? 'Risk' : 'الخطورة')}</th>
                            <th style="padding:10px;">${esc(isEnglish ? 'Assignee' : 'المعيَّن')}</th>
                            <th style="padding:10px;">${esc(isEnglish ? 'Target' : 'الموعد')}</th>
                        </tr></thead>
                        <tbody>${topRowsHtml}</tbody>
                    </table>
                </div>
            </div>
            <div class="dob-pdf-ftr">${esc(isEnglish ? 'INTERNAL' : 'داخلي')}</div>
        `);

        // 5) End
        mkSlide(`
            <div class="dob-pdf-hdr"><div style="font-size:20px;font-weight:800;width:100%;text-align:${align};">${esc(isEnglish ? 'Closing — Safety Commitment' : 'ختام — التزام السلامة')}</div></div>
            <div class="dob-pdf-hdr-gold"></div>
            <div style="padding:80px 120px 70px;">
                <div class="dob-pdf-card" style="text-align:center;min-height:320px;display:flex;flex-direction:column;justify-content:center;gap:14px;">
                    <div class="dob-pdf-title" style="font-size:26px;">${esc(isEnglish ? 'Thank you for your continuous commitment to Safety Standards' : 'شكراً لالتزامكم الدائم والمستمر بمعايير السلامة والصحة المهنية')}</div>
                    <div class="dob-pdf-amber" style="font-size:20px;">${esc(isEnglish ? 'Safety First — Americana Group' : 'السلامة أولاً — مجموعة أمريكانا')}</div>
                    <div class="dob-pdf-muted" style="font-size:15px;">${esc(isEnglish ? 'Working Together for a Zero-Harm Environment' : 'نعمل معاً من أجل بيئة عمل خالية من الحوادث والمخاطر')}</div>
                </div>
            </div>
            <div class="dob-pdf-ftr">${esc(companyName)} — HSE Management</div>
        `);

        document.body.appendChild(host);
        try {
            const slides = Array.from(host.querySelectorAll('.dob-pdf-slide'));
            const doc = new jsPDF({ orientation: 'landscape', unit: 'pt', format: 'a4' });
            const pageW = doc.internal.pageSize.getWidth();
            const pageH = doc.internal.pageSize.getHeight();
            for (let i = 0; i < slides.length; i++) {
                this._showPptExportProgress(96 + Math.min(3, Math.round((i / Math.max(1, slides.length)) * 3)), `PDF شريحة ${i + 1}/${slides.length}`);
            ...
            doc.save(`Daily_Observations_${safeDept}_${new Date().toISOString().slice(0, 10)}.pdf`);
        } finally {
            host.remove();
        }
    },

    async _exportObservationsExcelBatch(rows = [], opts = {}) {
        const quiet = !!(opts && opts.quiet);
        const list = Array.isArray(rows) ? rows : [];
        if (!list.length) {
            if (!quiet) Notification?.warning?.('لا توجد ملاحظات لتصدير Excel.');
            return;
        }
        if (typeof XLSX === 'undefined') {
            try {
                ...
                if (!quiet) Notification?.error?.('تعذّر تحميل مكتبة Excel');
                return;
            }
        }
        const excelData = list.map((obs) => ({
            ...
            'المعيَّن': obs.assignedToName || obs.assignedTo || '',
            ...
            'الحالة': obs.status || '',
            'مرحلة السير': this.getWorkflowStageLabel ? this.getWorkflowStageLabel(obs.workflowStage) : (obs.workflowStage || ''),
        ...
        const fileName = `Daily_Observations_Batch_${new Date().toISOString().slice(0, 10)}.xlsx`;
        XLSX.writeFile(workbook, fileName);
        if (!quiet) Notification?.success?.(`تم تصدير ${list.length} ملاحظة إلى Excel.`);
    },
    ...
    normalizeRecord(record = {}) {
        ...
        // ✅ FIX: Force attachments to be an array (JSON / نص متعدد الأسطر name - url)
        let rawAttachments = this._coerceAttachmentsArray(record.attachments || record.files || record.images);
        ...
        // ✅ FIX: Parse afterExecutionImages if it's a JSON string / multiline
        let rawAfterExecutionImages = this._coerceAttachmentsArray(record.afterExecutionImages);
        ...
        return {
            ...
            afterExecutionImages: this.normalizeAttachments(rawAfterExecutionImages), // ✅ صور بعد التنفيذ
    ...
    normalizeAttachments(rawAttachments = []) {
        const list = this._coerceAttachmentsArray(rawAttachments);
        return list
    ...
    normalizeAttachment(entry, index = 0) {
        ...
        if (typeof entry === 'object' && entry.__listOnly) {
            const slim = this._slimAttachmentMeta(entry);
            return slim || { __listOnly: true };
        }
        let data = '';
        let name = '';
        let type = '';
        ...
        if (typeof entry === 'string') {
            // نص الشيت: "image-1 - https://drive.google.com/uc?export=view&id=..."
            const match = entry.match(/^(.+?)\s*-\s*(https?:\/\/\S+)/i);
            if (match) {
                name = match[1].trim();
                data = match[2].trim();
            } else {
                const urlOnly = entry.match(/https?:\/\/\S+/i);
                data = urlOnly ? urlOnly[0].trim() : entry.trim();
                name = `مرفق-${index + 1}`;
            }
            ...
            const match = typeof rawData === 'string' ? rawData.match(/^(.+?)\s*-\s*(https?:\/\/\S+)/i) : null;
            ...
            fileId = String(entry.fileId || entry.driveId || '').trim() || this.resolveObservationDriveFileId(data || entry);
            if (!data && fileId) {
                data = (/^https?:\/\//i.test(String(entry.url || entry.directLink || '')))
                    ? String(entry.url || entry.directLink)
                    : fileId;
            }
        }
    ...
    _OBS_DUP_WINDOW_MS: 36 * 60 * 60 * 1000,
    _OBS_SUBMIT_DEDUP_MS: 8000,

    _normalizeObsText(value) {
    ...
    _obsLocationKey(siteId, placeId, locationName) {
        const site = String(siteId || '').trim();
        const place = String(placeId || '').trim();
        if (site && place && place !== '__custom__') return site + '|' + place;
        const loc = this._normalizeObsText(locationName);
        return site + '|loc:' + loc;
    },
    ...
    _obsTextSimilarity(a, b) {
        const na = this._normalizeObsText(a);
        const nb = this._normalizeObsText(b);
        if (!na || !nb) return 0;
        if (na === nb) return 1;
    ...
    _obsWithinDupWindow(obsDateIso) {
        const t = obsDateIso ? new Date(obsDateIso).getTime() : NaN;
        if (Number.isNaN(t)) return false;
        return Math.abs(Date.now() - t) <= this._OBS_DUP_WINDOW_MS;
    },

    _getActiveHazardsForLocation(siteId, placeId, locationName, excludeId) {
        const key = this._obsLocationKey(siteId, placeId, locationName);
        if (!key || key.endsWith('|') || key.endsWith('|loc:')) return [];
        const list = AppState.appData.dailyObservations || [];
        const out = [];
        for (let i = 0; i < list.length; i += 1) {
            ...
            if (!this._obsIsStillOpen(obs)) continue;
            const obsKey = this._obsLocationKey(obs.siteId, obs.placeId, obs.locationName);
            if (obsKey !== key) continue;
            const when = obs.date || obs.createdAt || obs.submittedAt;
            if (!this._obsWithinDupWindow(when)) continue;
            out.push(obs);
        }
        out.sort((a, b) => new Date(b.date || b.createdAt || 0) - new Date(a.date || a.createdAt || 0));
        return out.slice(0, 6);
    },

    _findDuplicateCandidates(payload, editId) {
        const key = this._obsLocationKey(payload.siteId, payload.placeId, payload.locationName);
        const details = payload.details || '';
        const type = String(payload.observationType || '').trim();
        const list = AppState.appData.dailyObservations || [];
        const hits = [];
        for (let i = 0; i < list.length; i += 1) {
            const obs = list[i];
            if (!obs || (editId && obs.id === editId)) continue;
            if (!this._obsIsStillOpen(obs)) continue;
            if (this._obsLocationKey(obs.siteId, obs.placeId, obs.locationName) !== key) continue;
            const when = obs.date || obs.createdAt || obs.submittedAt;
            if (!this._obsWithinDupWindow(when)) continue;
            const typeMatch = !type || !obs.observationType || String(obs.observationType).trim() === type;
            const sim = this._obsTextSimilarity(details, obs.details);
            if (!typeMatch && sim < 0.85) continue;
            if (sim < 0.5) continue;
            hits.push({ obs, sim, exact: sim >= 0.92 });
        }
        hits.sort((a, b) => b.sim - a.sim);
        return hits.slice(0, 5);
    },

    _refreshLocationHazardsBanner(form, excludeId) {
        if (!form) return;
        const banner = form.querySelector('#obs-active-hazards-banner');
        if (!banner) return;
        const siteId = form.querySelector('#observation-site')?.value || '';
        const placeSelect = form.querySelector('#observation-place');
        const placeValue = placeSelect?.value || '';
        const customName = (form.querySelector('#custom-location-input')?.value || '').trim();
        let placeId = '';
        let locationName = '';
        if (placeValue === '__custom__') {
            ...
            locationName = opt ? (opt.getAttribute('data-name') || opt.textContent.trim()) : '';
        }
        const matches = this._getActiveHazardsForLocation(siteId, placeId, locationName, excludeId);
        if (!matches.length) {
            banner.classList.add('hidden');
            banner.innerHTML = '';
            return;
        }
        const rows = matches.map((obs) => {
            const code = Utils.escapeHTML(obs.isoCode || obs.id || '');
            const shift = Utils.escapeHTML(obs.shift || '—');
            const who = Utils.escapeHTML(obs.observerName || obs.submittedBy || '—');
            const type = Utils.escapeHTML(obs.observationType || '');
            const snippet = Utils.escapeHTML(String(obs.details || '').slice(0, 80));
            const oid = Utils.escapeHTML(obs.id);
            return `<li style="margin:0 0 6px;display:flex;gap:8px;align-items:flex-start;flex-wrap:wrap;">
                <strong style="color:#9a3412;">${code}</strong>
                <span>${type}</span>
                <span style="opacity:.85;">وردية ${shift} — ${who}</span>
                <span style="flex:1;min-width:140px;opacity:.9;">${snippet}${String(obs.details || '').length > 80 ? '…' : ''}</span>
                <button type="button" class="btn-secondary btn-sm obs-hazard-open" data-oid="${oid}" style="padding:2px 8px;font-size:12px;">عرض</button>
            </li>`;
        }).join('');
        ...
        banner.innerHTML = `
            <div style="background:#fff7ed;border:1px solid #fdba74;border-radius:10px;padding:10px 12px;margin-bottom:14px;">
                <div style="font-weight:700;color:#9a3412;margin-bottom:6px;">
                    ...
                    يوجد ${matches.length} ملاحظة مفتوحة في هذا المكان خلال آخر 36 ساعة (ورديات سابقة).
                    سجّل متابعة على القائمة بدل تكرار نفس الخطر إن كان قائماً.
                </div>
                <ul style="margin:0;padding:0;list-style:none;">${rows}</ul>
            </div>`;
        banner.querySelectorAll('.obs-hazard-open').forEach((btn) => {
            btn.addEventListener('click', () => {
                const oid = btn.getAttribute('data-oid');
                if (oid && typeof this.viewObservation === 'function') void this.viewObservation(oid);
            });
        });
    },

    _askDuplicateDecision(hits) {
        return new Promise((resolve) => {
            const top = hits[0];
            const obs = top.obs;
            const code = Utils.escapeHTML(obs.isoCode || obs.id || '');
            ...
            const extra = hits.length > 1
                ? `<p style="margin:8px 0 0;font-size:13px;opacity:.85;">و${hits.length - 1} ملاحظة مشابهة أخرى في نفس المكان.</p>`
                : '';
            ...
            overlay.innerHTML = `
                <div class="modal-content" style="max-width:520px;">
                    ...
                    <div class="modal-body" style="line-height:1.7;">
                        <p>هذه الحالة مسجّلة مسبقاً برقم <strong>${code}</strong> بواسطة <strong>${who}</strong> في الوردية <strong>${shift}</strong>.</p>
            ...
            overlay.querySelectorAll('[data-dup]').forEach((btn) => {
                btn.addEventListener('click', () => finish(btn.getAttribute('data-dup')));
            });
    ...
    async showForm(data = null) {
        ...
        placeSelect.addEventListener('change', (event) => {
            ...
            if (!value) {
                ...
                customLocationInput.value = '';
                stepTwoContainer.classList.add('hidden');
                this._refreshLocationHazardsBanner(form, normalizedData?.id || null);
                return;
            }
        ...
        saveBtn.addEventListener('click', async () => {
            if (this._obsSubmitInFlight || (saveBtn && saveBtn.disabled)) {
                Notification.warning('جاري الحفظ... الرجاء الانتظار');
                return;
            }
            this._obsSubmitInFlight = true;
            let originalText = '';
            if (saveBtn) {
            ...
            try {
                ...
                this._obsSubmitInFlight = false;
                if (saveBtn && document.body.contains(modal)) {
        ...
        if (normalizedData && normalizedData.siteId) {
            ...
            if (normalizedData.placeId) {
                ...
                this.state.isCustomLocationSelected = true;
                stepTwoContainer.classList.remove('hidden');
                this._refreshLocationHazardsBanner(form, normalizedData.id);
            }
        }
    },
    ...
    async handleSubmit(form, editId = null, modal) {
        ...
        if (!editId) {
            const submitFp = [
                this._obsLocationKey(siteId, placeId, locationName),
                String(observationType || '').trim(),
                this._normalizeObsText(details)
            ].join('||');
            const nowTs = Date.now();
            if (this._lastObsSubmitFp === submitFp && (nowTs - (this._lastObsSubmitAt || 0)) < this._OBS_SUBMIT_DEDUP_MS) {
                Notification.warning('تم تسجيل هذه الملاحظة للتو. لن تُرسل مرة أخرى.');
                return;
            }
            const dupHits = this._findDuplicateCandidates({
                siteId,
                placeId,
                locationName,
                observationType,
            ...
            if (dupHits.length) {
                const choice = await this._askDuplicateDecision(dupHits);
                const existingId = dupHits[0].obs.id;
                if (choice === 'cancel') return;
                if (choice === 'view' || choice === 'followup') {
                    if (modal) {
                        modal.remove();
                        this.resetFormState();
                    }
                    if (choice === 'view') {
                        if (typeof this.viewObservation === 'function') void this.viewObservation(existingId);
                    } else if (typeof this.showAddCommentModal === 'function') {
                        this.showAddCommentModal(existingId);
                    }
                    return;
                }
            }
            this._lastObsSubmitFp = submitFp;
            this._lastObsSubmitAt = nowTs;
        }
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 900px; border-radius: 20px; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); font-family: 'Cairo', 'Segoe UI', Tahoma, Geneva, Verdana, Arial, sans-serif; text-rendering: optimizeLegibility; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;">
                ...
                <div class="modal-footer obs-detail-footer" style="display:flex;flex-wrap:nowrap;align-items:center;justify-content:flex-end;gap:8px;padding:12px 18px;background:#fff;border-top:1px solid #e2e8f0;border-radius:0 0 20px 20px;overflow-x:auto;">
                    ...
                    <button type="button" class="btn-primary obs-detail-quick-close" data-oid="${Utils.escapeHTML(String(observation.id || ''))}" style="background:#4f46e5;border:none;">
                    ...
                    <button type="button" onclick="DailyObservations.shareViaWhatsApp('${observation.id}');" class="btn-secondary" style="background:#16a34a;color:#fff;border-color:#15803d;">
                    ...
                    ${this.canDailyObservationsFullAdminUi() ? `
                    <button type="button" onclick="DailyObservations.openEditFromDetailModal('${observation.id}')" class="btn-primary">
                        <i class="fas fa-edit"></i>تعديل
                    </button>
                    <button type="button" onclick="DailyObservations.deleteObservation('${observation.id}'); this.closest('.modal-overlay').remove();" class="btn-secondary" style="background:#dc2626;color:#fff;border-color:#b91c1c;">
    ...
    async updateObservationDataFromBackend(observationId, modal) {
        try {
            ...
            if (response.success && response.data) {
                ...
                if (index !== -1) {
                    // احتفظ بروابط المرفقات المطبّعة — لا تستبدل بمصفوفة فارغة
                    const prev = AppState.appData.dailyObservations[index] || {};
                    const nextAtts = (Array.isArray(slim.attachments) && slim.attachments.length)
                        ? slim.attachments
                        : (Array.isArray(updatedObs.attachments) && updatedObs.attachments.length
                            ? updatedObs.attachments.map((a) => this._slimAttachmentMeta(a)).filter(Boolean)
                            : (prev.attachments || []));
                    AppState.appData.dailyObservations[index] = {
                        ...
                        attachmentCount: Math.max(
                            Number(slim.attachmentCount) || 0,
                            Number(updatedObs.attachmentCount) || 0,
                            nextAtts.length
                        ),
                        afterExecutionImages: (Array.isArray(slim.afterExecutionImages) && slim.afterExecutionImages.length)
                            ? slim.afterExecutionImages
                            : (updatedObs.afterExecutionImages || prev.afterExecutionImages || [])
                    };
                    try { this._invalidateObsNormCache(); } catch (_e) {}
                } else {
    ...
    _observationImmediateDisplaySrc(att) {
        const rawSrc = this.getObservationAttachmentSrc(att);
        if (/^data:image\//i.test(rawSrc) && !/^data:image\/gif/i.test(rawSrc)) return rawSrc;
        const fileId = this.resolveObservationDriveFileId(att);
        // روابط Drive تُحمَّل عبر وكيل hydrateDriveProxyImages — لا تعتمد على thumbnail المباشر (غالباً يُحجب)
        if (fileId || /drive\.google\.com|googleusercontent\.com/i.test(rawSrc)) {
            return (typeof Utils !== 'undefined' && Utils.IMG_DRIVE_PLACEHOLDER_GIF)
                ? Utils.IMG_DRIVE_PLACEHOLDER_GIF
                : 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
        }
    ...
    _observationPhotoSectionHtml(observation) {
        ...
        const real = atts.filter((a) => a && !a.__listOnly && (this.getObservationAttachmentSrc(a) || a.fileId || a.driveId));
        const waiting = real.length === 0 && !this.observationHasRealImages(observation) && Number(observation?.attachmentCount) > 0;
        const inner = waiting
    ...
    _buildObservationAttachmentsHtml(attachments) {
        ...
        const real = attachments.filter((att) => att && (!att.__listOnly || att.fileId || att.url || att.driveId));
        if (!real.length) {
            return '';
        }
        ...
        return real.map((att) => {
            ...
            const fileId = this.resolveObservationDriveFileId(att);
            const isImg = this.isObservationPhotoAttachment(att) || !!fileId;

            if (isImg && (rawSrc || fileId)) {
                let displaySrc = this._observationImmediateDisplaySrc(att);
                let driveProxyAttr = '';
                if (typeof Utils !== 'undefined' && typeof Utils.resolveDriveAwareImgDisplay === 'function') {
                    const info = Utils.resolveDriveAwareImgDisplay(rawSrc || fileId || att);
                    if (info && info.needsProxy && info.proxyFileId) {
                        displaySrc = info.displaySrc || displaySrc;
                        driveProxyAttr = Utils.driveProxyImgAttrs(info);
                    } else if (info && info.displaySrc && !info.needsProxy) {
                        displaySrc = info.displaySrc;
                    }
                }
                if (!driveProxyAttr && fileId) {
                    driveProxyAttr = ` data-drive-proxy-id="${Utils.escapeHTML(fileId)}"`;
                }
                if (!displaySrc) {
                ...
                return `
                    <div class="border border-slate-200 rounded-xl overflow-hidden bg-white flex flex-col">
                        ...
                        <div class="w-full py-2 px-3 flex items-center justify-between text-xs text-gray-600 border-t border-slate-100">
                            ...
                            <button type="button" onclick="DailyObservations.viewFullImage(this.closest('.border').querySelector('img')?.currentSrc || '${Utils.escapeHTML(rawSrc || fileId)}')" class="text-blue-600 hover:text-blue-800 font-bold flex items-center gap-1 cursor-pointer">
