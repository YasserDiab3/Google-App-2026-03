...
const DataManager = {
    ...
    purgeLocalAppData(reason) {
        ...
        // بعد تحميل إصدار جديد: ممنوع مسح الكاش المحلي (يظهر النظام فارغاً)
        try {
            if (typeof window !== 'undefined' && window.__HSE_AFTER_VERSION_UPDATE) {
                const allow = why === 'logout' || why === 'user_changed' || why === 'load_owner_mismatch' || why === 'load_payload_owner_mismatch';
                if (!allow) {
                    if (typeof Utils !== 'undefined' && Utils.safeWarn) {
                        Utils.safeWarn('🛡️ تخطي مسح البيانات بعد تحديث الإصدار:', why || 'unknown');
                    }
                    return Promise.resolve(false);
                }
            }
            if (typeof AppState !== 'undefined' && AppState.isPageRefresh && why && !/logout|user_changed|owner_mismatch|payload_owner/.test(why)) {
                if (typeof Utils !== 'undefined' && Utils.safeWarn) {
                    Utils.safeWarn('🛡️ تخطي مسح البيانات أثناء إعادة تحميل الصفحة:', why);
                }
                return Promise.resolve(false);
            }
        } catch (_e) { /* ignore */ }
    ...
    purgeIfUserChanged(newUserEmail) {
        ...
        if (prev && prev !== next) {
            ...
            // نفس المستخدم أو جلسة أولى: تبنَّ الكاش الموجود — المسح كان يفرّغ IndexedDB عند كل إعادة تحميل
            if (typeof Utils !== 'undefined' && Utils.safeLog) {
                Utils.safeLog('ℹ️ ربط كاش محلي بالمستخدم الحالي دون مسح');
            }
        }
    ...
    loadGoogleConfig() {
        try {
            ...
            const parsed = config ? JSON.parse(config) : (AppState.googleConfig || {});
            if (typeof window !== 'undefined' && typeof window.__hseEnsureGasConfig === 'function') {
                AppState.googleConfig = window.__hseEnsureGasConfig(parsed);
            } else {
                AppState.googleConfig = parsed;
            }
            try {
