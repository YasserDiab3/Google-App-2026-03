...
(function () {
    ...
    const StableLoader = {
        ...
        smokeModuleCounts() {
            ...
            const keys = [
                'users', 'employees', 'incidents', 'nearmiss', 'ptw', 'ptwRegistry',
                'training', 'clinicVisits', 'clinicContractorVisits', 'dailyObservations',
                'approvedContractors', 'contractors', 'fireEquipment', 'ppe', 'violations',
                'behaviorMonitoring', 'chemicalSafety', 'isoDocuments'
            ];
            const out = {};
            keys.forEach((k) => {
                const v = data[k];
                out[k] = Array.isArray(v) ? v.length : (v && typeof v === 'object' ? Object.keys(v).length : 0);
            });
            return out;
        },
