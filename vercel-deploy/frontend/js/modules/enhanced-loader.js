...
const EnhancedLoader = {
    ...
    init() {
        try {
            if (document.body && document.body.classList.contains('app-active')) {
                this.hide();
                return;
            }
        } catch (e) { /* ignore */ }
        ...
        this.loadingState.total = this.loadingState.total > 0 ? this.loadingState.total : 100;
    },
    ...
    hide() {
        ...
        const overlay = this.elements.overlay || document.getElementById('loading-overlay');
        if (overlay) {
            ...
            overlay.dataset.loadingVisible = '0';
        }
    },
    ...
    updateProgress(step = null, message = null) {
        ...
        if (!this.loadingState.total || this.loadingState.total <= 0) {
        ...
        if (step !== null) {
            ...
            if (Number.isFinite(n)) {
                this.loadingState.loaded = Math.max(0, Math.min(100, n));
                this.loadingState.total = 100;
            }
        }
        ...
        const total = this.loadingState.total > 0 ? this.loadingState.total : 100;
        const loaded = Number(this.loadingState.loaded);
        const rawPct = (Number.isFinite(loaded) ? loaded : 0) / total * 100;
        ...
        if (this.elements.timeText && this.loadingState.startTime) {
            ...
            this.elements.timeText.textContent = `${Number.isFinite(elapsed) ? elapsed : 0}s`;
        }

        if (this.elements.currentStepText && this.loadingState.currentStep) {
            this.elements.currentStepText.textContent = this.loadingState.currentStep;
        }
