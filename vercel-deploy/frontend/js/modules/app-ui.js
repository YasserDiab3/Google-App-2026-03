...
window.UI = {
    ...
    _showUpdateModal(newVersion, customHighlights = null) {
        ...
        const dismissedKey = 'hse_update_dismissed_version';
        ...
        try {
            ...
            if (lastSeen && typeof this._compareVersions === 'function' && this._compareVersions(v, lastSeen) <= 0) return;
            const dismissedLocal = typeof localStorage !== 'undefined' ? (localStorage.getItem(dismissedKey) || '') : '';
            if (dismissedLocal && typeof this._compareVersions === 'function' && this._compareVersions(v, dismissedLocal) <= 0) return;
            if (typeof sessionStorage !== 'undefined') {
                if (sessionStorage.getItem(sessionShownKey) === v) return;
                if (sessionStorage.getItem(dismissedKey) === v) return;
            }
        } catch (e) {}
        ...
        const versionPrefix = isEn ? 'App updated to Version:' : 'تم إجراء تحديث على التطبيق — الإصدار:';
        const notesTitle = isEn ? 'Release Highlights' : 'نبذة عن التحديث';
        const reloadText = isEn ? 'Reload Page Now' : 'تحديث الصفحة الآن';
        const laterText = isEn ? 'Later' : 'لاحقاً';
        const toastMsg = isEn 
        ...
        if (typeof Notification !== 'undefined' && Notification.info) {
            Notification.info(toastMsg, { duration: 6000 });
        }
        ...
        const onReload = () => {
            ...
            try {
                if (typeof localStorage !== 'undefined') {
                    ...
                    localStorage.setItem(dismissedKey, v);
                }
                if (typeof sessionStorage !== 'undefined') {
                    sessionStorage.setItem(storageKey, v);
                    sessionStorage.setItem(sessionShownKey, v);
                    sessionStorage.setItem(dismissedKey, v);
                    sessionStorage.removeItem('hse_hard_reload_for');
                }
            } catch (e) {}
            // إرسال SKIP_WAITING لـ SW الجديد المنتظر حتى يتفعّل → controllerchange → reload
            try {
            ...
            // علّم أن المستخدم طلب التحديث — controllerchange سيُعيد التحميل
            window.__hseUserRequestedReload = true;
            if (typeof this._hardReloadForUpdate === 'function') {
                this._hardReloadForUpdate(v, { force: true });
                return;
            }
            window.location.reload();
        };
        // «لاحقاً»: تخزين الإقرار بالرفض دائمياً للجلسة وlocalStorage لمنع تكرار الرسالة
        const onLater = () => {
            try {
                if (typeof localStorage !== 'undefined') {
                    localStorage.setItem(storageKey, v);
    ...
    /**
     * عرض نبذة عن التحديث يدوياً من أي مكان في النظام
     */
    ...
    /**
     * كسر كاش SW/المتصفح ثم إعادة تحميل — حتى لا يبقى رقم الإصدار المعروض قديماً بعد «تحديث الصفحة».
     */
    async _hardReloadForUpdate(serverVersion, options = {}) {
        const v = serverVersion ? String(serverVersion).trim() : '';
        const force = !!(options && options.force);
        try {
            // عند force من الزر: لا تمسح last_seen إن كان المستخدم بالفعل على نفس الإصدار (ثُبّت في onReload)
            if (!force && typeof localStorage !== 'undefined') {
                localStorage.removeItem('hse_last_seen_version');
            }
            if (typeof sessionStorage !== 'undefined') {
                sessionStorage.removeItem('hse_update_modal_shown_version');
                sessionStorage.setItem('hse_keep_local_data', '1');
            }
        } catch (e) {}
        if (typeof window !== 'undefined' && typeof window.__hseHardReloadForUpdate === 'function') {
            await window.__hseHardReloadForUpdate(v, { force: force });
            return;
        }
        try {
        ...
        try {
            ...
            if (v) u.searchParams.set('_v', v);
            u.searchParams.set('_bust', String(Date.now()));
            if (force) u.searchParams.set('_force', '1');
     ...
     * إن كان JS المحمّل أقدم من الخادم: كسر الكاش فوراً حتى لو lastSeen = إصدار الخادم.
    ...
    async _checkServerVersion() {
        try {
            ...
            const loaded = (typeof AppState !== 'undefined' && AppState.appVersion)
                ? String(AppState.appVersion).trim()
                : '';
            const staleClient = !!(loaded && this._compareVersions(serverVersion, loaded) > 0);
            if (staleClient) {
                // عرض المودال فقط — لا ريلود تلقائي. المستخدم يقرر بنفسه
                this._showUpdateModal(serverVersion, data.highlights || null);
                return;
            }

            // العميل محدّث — أزل قفل الريلود حتى لا يعطّل زر «تحديث الصفحة الآن» لاحقاً
            try {
                if (typeof sessionStorage !== 'undefined') sessionStorage.removeItem('hse_hard_reload_for');
            } catch (e) {}
            ...
            // إذا لم يكن هناك lastSeen مخزن: إن كان العميل محملاً بنفس إصدار الخادم نعتبره مطلعاً ونخزن الإصدار
            if (!lastSeen) {
                if (loaded && this._compareVersions(serverVersion, loaded) <= 0) {
                    try { localStorage.setItem(storageKey, loaded); } catch (e) {}
                    return;
                }
     ...
     */
    async _showUpdateMessageIfNeeded() {
        try {
            ...
            // عند أول زيارة أو تسجيل دخول جديد بدون lastSeen: نخزن الإصدار الحالي ولا نزعج المستخدم بالنافذة طالما أنه على الإصدار الحالي
            if (!lastSeen) {
            ...
            if (cmp < 0) {
                // lastSeen أحدث من JS المحمّل = كاش قديم — عرض المودال فقط بدون ريلود تلقائي
                this._showUpdateModal(lastSeen, AppState?._serverReleaseHighlights || null);
                return;
            }
            if (cmp === 0) {
                try {
    ...
    _installUiRecoveryWatchdog() {
        ...
        const tick = () => {
            try {
                ...
                const appActive = document.body && document.body.classList.contains('app-active');
                const overlayAgeMs = typeof window._hseLoadingSince === 'number'
                    ? (Date.now() - window._hseLoadingSince)
                    : Infinity;
                const overlayTimeoutMs = appActive ? 4000 : 120000;
                if (overlayVisible && mainVisible && typeof window._hseLoadingSince !== 'number') {
                    this.cleanupStaleStartupLayers();
                } else if (overlayVisible && overlayAgeMs > overlayTimeoutMs) {
                    if (typeof Loading !== 'undefined' && typeof Loading.hide === 'function') {
                        Loading.hide();
                    }
                    this.cleanupStaleStartupLayers();
                    if (typeof Utils !== 'undefined' && Utils.safeWarn) {
                        Utils.safeWarn('⚠️ تم إزالة طبقة التحميل تلقائياً بعد مهلة');
                    }
                }
            } catch (e) { /* ignore */ }
        };
    ...
    async handleSyncClick() {
        ...
        try {
            ...
            if (typeof Notification !== 'undefined') {
                Notification.info('جاري تحديث البيانات في الخلفية. البيانات الحالية ظاهرة.');
            }
    ...
    setLanguage(lang, isInitialLoad = false) {
        ...
        const translations = {
            ar: {
                ...
                'dash.safetyMetricsNote': 'ملاحظة: TRIR/AFR/FAR/FR/SR/IR تُحسب عبر HseMetrics الموحّد (YTD العام الحالي) بنفس منطق السكوركارد.',
            ...
            en: {
                ...
                'dash.afrSubtitle': 'Per 1,000,000 work hours',
                'dash.farTitle': 'Fatality Accident Rate',
                'dash.farSubtitle': 'Per 100,000,000 work hours',
                'dash.frTitle': 'LTI Frequency Rate (FR)',
                'dash.frSubtitle': 'Per 1,000,000 work hours',
                ...
                'dash.safetyMetricsNote': 'Note: TRIR/AFR/FAR/FR/SR/IR are calculated via unified HseMetrics (current year YTD), same as the scorecard.',
