...
/**
 * إصلاح تسلسل أرقام الملاحظات اليومية وتصحيح التكرارات — تظهر بأعلى قائمة Apps Script (a_...)
 */
function a_repairObservationSequence() {
    return repairObservationSequence();
}
...
function _dob_buildCoverSlide_(coverSlide, department, dateLabel, logoUrl, isEnglish) {
    ...
    try {
        ...
        coverSlide.getBackground().setSolidFill('#f8fafc');

        // شريط علوي كحلي + خط ذهبي (مطابقة هوية PptxGenJS)
        const topBar = coverSlide.insertShape(SlidesApp.ShapeType.RECTANGLE, 0, 0, 720, 58);
        topBar.getFill().setSolidFill('#1e3a8a');
        topBar.getBorder().setTransparent();
        const topGold = coverSlide.insertShape(SlidesApp.ShapeType.RECTANGLE, 0, 58, 720, 4);
        topGold.getFill().setSolidFill('#d97706');
        topGold.getBorder().setTransparent();
            ...
            try {
                ...
                if (logoBlob) {
                    ...
                    if (origW > 0 && origH > 0) {
                        ...
                        img.setWidth(origW * ratio);
                        img.setHeight(origH * ratio);
                        img.setLeft(28);
                        img.setTop(10 + (maxH - origH * ratio) / 2);
                    }
        ...
        if (!insertedLeftLogo) {
            const logoLeft = coverSlide.insertShape(SlidesApp.ShapeType.ROUND_RECTANGLE, 24, 10, 140, 38);
            logoLeft.getFill().setSolidFill('#ffffff');
            logoLeft.getBorder().setWeight(1.5);
            ...
            logoLeftTxt.getTextStyle().setFontFamily(isEnglish ? 'Arial' : 'Cairo').setFontSize(14).setBold(true).setForegroundColor('#dc2626');
            try { logoLeftTxt.getParagraphStyle().setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER); } catch(e) {}
        }

        const logoRight = coverSlide.insertShape(SlidesApp.ShapeType.RECTANGLE, 280, 12, 420, 36);
        logoRight.getFill().setTransparent();
        logoRight.getBorder().setTransparent();
        const logoRightTxt = logoRight.getText();
        logoRightTxt.setText(isEnglish ? 'HSE Management System — Safety & Health Department' : 'نظام إدارة السلامة والصحة المهنية والبيئة | إدارة السلامة والصحة المهنية');
        logoRightTxt.getTextStyle().setFontFamily(isEnglish ? 'Arial' : 'Cairo').setFontSize(11).setBold(true).setForegroundColor('#ffffff');
        ...
        const centerCard = coverSlide.insertShape(SlidesApp.ShapeType.ROUND_RECTANGLE, 70, 100, 580, 175);
        centerCard.getFill().setSolidFill('#ffffff');
        centerCard.getBorder().setWeight(2);
        centerCard.getBorder().getLineFill().setSolidFill('#3b82f6');
        ...
        try {
            ...
            if (cardParas.length >= 1) {
                cardParas[0].getRange().getTextStyle().setFontFamily(isEnglish ? 'Arial' : 'Cairo').setFontSize(26).setBold(true).setForegroundColor('#1e3a8a');
            }
            if (cardParas.length >= 2) {
                cardParas[1].getRange().getTextStyle().setFontFamily(isEnglish ? 'Arial' : 'Cairo').setFontSize(16).setBold(true).setForegroundColor('#0f172a');
            }
            if (cardParas.length >= 3) {
                cardParas[2].getRange().getTextStyle().setFontFamily(isEnglish ? 'Arial' : 'Cairo').setFontSize(12).setBold(false).setForegroundColor('#94a3b8');
            }
        } catch(pErr) {
            cardText.getTextStyle().setFontFamily(isEnglish ? 'Arial' : 'Cairo').setFontSize(20).setBold(true).setForegroundColor('#1e3a8a');
        }

        const blueBox = coverSlide.insertShape(SlidesApp.ShapeType.ROUND_RECTANGLE, isEnglish ? 70 : 400, 300, 250, 80);
        blueBox.getFill().setSolidFill('#eff6ff');
        blueBox.getBorder().setWeight(1.5);
        blueBox.getBorder().getLineFill().setSolidFill('#93c5fd');
        ...
        blueBoxTxt.getTextStyle().setFontFamily(isEnglish ? 'Arial' : 'Cairo').setFontSize(12).setBold(true).setForegroundColor('#1e40af');
        try { blueBoxTxt.getParagraphStyle().setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER); } catch(e) {}

        const bottomStripe = coverSlide.insertShape(SlidesApp.ShapeType.RECTANGLE, 0, 400, 720, 40);
        bottomStripe.getFill().setSolidFill('#1e3a8a');
        bottomStripe.getBorder().setTransparent();
        const bottomTxtShape = coverSlide.insertShape(SlidesApp.ShapeType.RECTANGLE, 40, 405, 640, 30);
        bottomTxtShape.getFill().setTransparent();
        bottomTxtShape.getBorder().setTransparent();
        const bottomTxt = bottomTxtShape.getText();
        bottomTxt.setText('Safety First — السلامة أولاً وبناء بيئة عمل آمنة ومستدامة');
        bottomTxt.getTextStyle().setFontFamily(isEnglish ? 'Arial' : 'Cairo').setFontSize(11).setBold(true).setForegroundColor('#fbbf24');
        try { bottomTxt.getParagraphStyle().setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER); } catch(e) {}
...
function createDefaultDailyObservationsPptTemplate() {
    try {
        ...
        rightHeader.getFill().setSolidFill('#2563eb');
        rightHeader.getBorder().setWeight(1);
        rightHeader.getBorder().getLineFill().setSolidFill('#1e40af');
        const rHeadTxt = rightHeader.getText();
        rHeadTxt.setText('وصف الملاحظة');
        rHeadTxt.getTextStyle().setFontFamily('Cairo').setFontSize(18).setBold(true).setForegroundColor('#ffffff');

        const leftHeader = itemTemplateSlide.insertShape(SlidesApp.ShapeType.RECTANGLE, 10, 10, 340, 38);
        leftHeader.getFill().setSolidFill('#64748b');
        leftHeader.getBorder().setWeight(1);
        leftHeader.getBorder().getLineFill().setSolidFill('#475569');
        const lHeadTxt = leftHeader.getText();
        lHeadTxt.setText('الصورة التوضيحية');
        lHeadTxt.getTextStyle().setFontFamily('Cairo').setFontSize(18).setBold(true).setForegroundColor('#ffffff');

        const rightBodyBox = itemTemplateSlide.insertShape(SlidesApp.ShapeType.RECTANGLE, 360, 52, 345, 340);
        rightBodyBox.getFill().setSolidFill('#ffffff');
        rightBodyBox.getBorder().setWeight(1.5);
        rightBodyBox.getBorder().getLineFill().setSolidFill('#cbd5e1');
        ...
        bodyTxt.setText(
            ...
            'المسؤول المعيَّن : {{ASSIGNED_TO}}\n' +
            'مرحلة السير : {{WORKFLOW_STAGE}}\n' +
            'الحالة : {{STATUS}}'
        );
    ...
    try {
        ...
        observations.forEach(function (obs, idx) {
            ...
            _dob_replaceAllTextSafe_(presentation, slide, {
                ...
                '{{ASSIGNED_TO}}': String(obs.assignedToName || obs.assignedTo || ''),
                '{{WORKFLOW_STAGE}}': String(obs.workflowStage || obs.workflowStageLabel || ''),
                '{{STATUS}}': status,
                '{{SHIFT}}': shift,
        ...
        return {
            ...
            folderUrl: outputFolderId ? ('https://drive.google.com/drive/folders/' + outputFolderId) : ''
        };
    } catch (error) {
...
function transitionObservationWorkflow(payload) {
    try {
        ...
        if (action === 'assign_responsible') {
            ...
            if (stage === 'closed' || String(obs.status || '').trim() === 'مغلق') {
                return fail('الملاحظة مغلقة بالفعل');
            }
 ...
 */
/**
 * استرجاع فائق السرعة للتحليل الإحصائي للملاحظات الميدانية (زمن استجابة < 300ms)
 * يعتمد على قراءة الأعمدة المحددة مباشرة كـ 2D Array والتخزين السحابي المؤقت CacheService
 */
/**
 ...
 */
/**
 * استرجاع التحليل الإحصائي للملاحظات الميدانية مع حصر المواقع حصرياً في (ICAPP-1, ICAPP-2, WH)
 */
function getPublicObservationsAnalytics(payload) {
    try {
        var force = payload && (payload.force === true || payload.force === 'true');
        var cacheKey = 'PUBLIC_OBS_ANALYTICS_COMPACT_V8';
        var cache = null;
        try {
            cache = CacheService.getScriptCache();
            if (cache && !force) {
        ...
        var sheet = ss.getSheetByName('DailyObservations');
        if (!sheet) return { success: false, message: 'DailyObservations sheet not found' };
        ...
        var colId = colMap['id'] !== undefined ? colMap['id'] : (colMap['كود الملاحظة'] !== undefined ? colMap['كود الملاحظة'] : 0);
        var colDate = colMap['date'] !== undefined ? colMap['date'] : (colMap['التاريخ'] !== undefined ? colMap['التاريخ'] : 1);
        var colSite = colMap['sitename'] !== undefined ? colMap['sitename'] : (colMap['site'] !== undefined ? colMap['site'] : (colMap['الموقع'] !== undefined ? colMap['الموقع'] : 3));
        var colLoc = colMap['locationname'] !== undefined ? colMap['locationname'] : (colMap['المكان'] !== undefined ? colMap['المكان'] : 4);
        var colType = colMap['observationtype'] !== undefined ? colMap['observationtype'] : (colMap['النوع'] !== undefined ? colMap['النوع'] : 5);
        var colObs = colMap['observername'] !== undefined ? colMap['observername'] : (colMap['الراصد'] !== undefined ? colMap['الراصد'] : (colMap['مسؤول السلامة'] !== undefined ? colMap['مسؤول السلامة'] : 7));
        ...
        var observerStats = {};
        var siteStats = {
            'ICAPP-1': { total: 0, open: 0, inProgress: 0, closed: 0, highRisk: 0, thisWeek: 0, thisMonth: 0, overdue: 0 },
            'ICAPP-2': { total: 0, open: 0, inProgress: 0, closed: 0, highRisk: 0, thisWeek: 0, thisMonth: 0, overdue: 0 },
            'WH': { total: 0, open: 0, inProgress: 0, closed: 0, highRisk: 0, thisWeek: 0, thisMonth: 0, overdue: 0 }
        };
        var riskStats = {};
        var criticalOpen = [];
        ...
        for (var i = 0; i < allValues.length; i++) {
            ...
            // حصر وتوحيد الموقع حصرياً في: ICAPP-1, ICAPP-2, WH
            ...
            if (rawSite.toUpperCase().indexOf('ICAPP-2') !== -1 || rawSite === '2' || rawLoc.indexOf('ICAPP-2') !== -1) {
                site = 'ICAPP-2';
            } else if (rawSite.toUpperCase().indexOf('WH') !== -1 || rawSite.indexOf('مخازن') !== -1 || rawSite.indexOf('مخزن') !== -1 || rawLoc.toUpperCase().indexOf('WH') !== -1) {
                site = 'WH';
            } else {
                site = 'ICAPP-1';
            }
            ...
            var obsId = String(row[colId] || ('OBS-' + (i + 1))).trim();
            ...
            var isMedium = (rk === 'متوسط' || rk === 'متوسطة' || rk === 'Medium');
            var isLow = (!isHigh && !isMedium);
            var rkKey = isHigh ? 'high' : (isMedium ? 'medium' : 'low');
            ...
            // 2. إحصائيات المواقع الثلاثة فقط (ICAPP-1, ICAPP-2, WH)
            siteCounts[site] = (siteCounts[site] || 0) + 1;
            var sst = siteStats[site];
            sst.total++;
            if (isOpen) sst.open++;
            if (isClosed) sst.closed++;
            if (isInProg) sst.inProgress++;
        ...
        Object.keys(siteStats).forEach(function(k) {
            var sst = siteStats[k];
            sst.closeRate = sst.total > 0 ? Math.round((sst.closed / sst.total) * 100) : 0;
        });

        Object.keys(riskStats).forEach(function(k) {
            var rst = riskStats[k];
        ...
        var plantBenchmark = validSites.map(function(sName) {
            var s = siteStats[sName] || { total: 0, open: 0, closed: 0, closeRate: 0, highRisk: 0, overdue: 0 };
        ...
        try {
            ...
            Logger.log('Cache put error: ' + cPutErr.toString());
        }
