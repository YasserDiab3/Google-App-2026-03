...
const Contractors = {
    ...
    switchTab(tab) {
        ...
        if (tab === 'approval-request' || tab === 'evaluations') {
            this.syncPendingEvaluationApprovalRequests()
                ...
                .then(() => {
                    this.refreshEvaluationApprovalRequestsSection();
                    if (tab === 'approval-request') this.refreshApprovalRequestsSection();
                })
                .catch(() => {});
        }
            ...
            try {
                ...
                if (evaluationData) {
                    ...
                    try {
                        ...
                        if (submitBtn) submitBtn.disabled = false;
                        Notification.error('تعذر إرسال طلب التقييم: ' + (syncErr?.message || 'خطأ غير معروف'));
                    } finally {
    ...
    /**
     * إعادة مزامنة طلبات التقييم العالقة محلياً (TEMP / _isPendingSync / _syncError)
     */
    async syncPendingEvaluationApprovalRequests(targetId) {
        ...
        if (synced || failed) {
            ...
            if (failed && !synced && tid) {
                const failedReq = this.findEvaluationApprovalRequest(tid);
                if (failedReq?._syncErrorMessage) {
                    Notification.error('فشل مزامنة طلب التقييم: ' + failedReq._syncErrorMessage);
                }
            }
        }
    ...
    _evaluationApprovalNeedsSync_(req) {
        if (!req) return false;
        const id = String(req.id || '').trim();
        return !!(req._isPendingSync || id.startsWith('TEMP_') || req._syncError);
    },

    buildEvaluationApprovalRowForBackend_(source, options = {}) {
        const s = source || {};
        let evalData = s.evaluationData;
        ...
        const row = {
            ...
            contractorName: s.contractorName || s.companyName || '',
        ...
        if (options.includeRequestType) row.requestType = 'evaluation';
        if (AppState?.googleConfig?.sheets?.spreadsheetId) {
            row.spreadsheetId = AppState.googleConfig.sheets.spreadsheetId;
        }
        return row;
    },

    async ensureEvaluationApprovalRequestSynced_(requestId, options = {}) {
        const maxRetries = options.maxRetries ?? 3;
        const showLoader = options.showLoader !== false;
        let request = this.findEvaluationApprovalRequest(requestId);
        if (!request) return null;
        if (!this._evaluationApprovalNeedsSync_(request)) return request;
        ...
        try {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                ...
                if (request && !this._evaluationApprovalNeedsSync_(request)) return request;
                if (attempt < maxRetries - 1) {
                    await new Promise((resolve) => setTimeout(resolve, 600 * (attempt + 1)));
                }
            }
            return this.findEvaluationApprovalRequest(requestId);
        } finally {
            if (showLoader && typeof Loading !== 'undefined' && Loading.hide) Loading.hide();
        }
    },
    ...
    getMyEvaluationApprovalRequests() {
        ...
        return (AppState.appData.contractorEvaluationApprovalRequests || [])
            .filter((req) => req && req.createdBy === currentUserId && !this._evaluationApprovalNeedsSync_(req))
    ...
    getPendingEvaluationApprovalRequests() {
        ...
        return (AppState.appData.contractorEvaluationApprovalRequests || [])
            .filter((req) => req &&
                !this._evaluationApprovalNeedsSync_(req) &&
    ...
    renderApprovalRequestSection() {
        ...
        if (isAdmin) {
            try {
                ...
                pendingEvaluationRequests = this.getPendingEvaluationApprovalRequests();
            } catch (error) {
                ...
                <div class="card-body space-y-6">
                    ...
                    ${isAdmin ? `
                        ...
                        <div class="border-t pt-4">
                            <h3 class="text-lg font-semibold text-gray-800 mb-4">
                                <i class="fas fa-clipboard-check ml-2"></i>
                                طلبات اعتماد التقييمات
                            </h3>
                            <div id="pending-evaluation-approval-admin-container">
                                ${this.renderApprovalRequestsTable(pendingEvaluationRequests, true)}
                            </div>
                        </div>
                    ` : ''}
    ...
    getMyApprovalRequests() {
        ...
        const approvalRequests = AppState.appData.contractorApprovalRequests
            .filter(req => req && req.createdBy === currentUserId && String(req.requestType || '').trim() !== 'evaluation')
        ...
        const allRequests = [
            ...approvalRequests,
            ...deletionRequests,
            ...this.getMyEvaluationApprovalRequests()
        ];
    ...
    getPendingApprovalRequests() {
        ...
        const approvalRequests = AppState.appData.contractorApprovalRequests
            .filter(req => req && 
                (req.status === 'pending' || req.status === 'under_review') &&
                req.createdBy !== currentUserId &&
                String(req.requestType || '').trim() !== 'evaluation')
            .map(req => ({ ...req, requestCategory: 'approval' }));
    ...
    async postEvaluationApprovalToBackend_(payload) {
        const row = this.buildEvaluationApprovalRowForBackend_(payload, { includeRequestType: true });
        const sheetRow = this.buildEvaluationApprovalRowForBackend_(payload, { includeRequestType: false });
        ...
        return {
            success: false,
            message: lastMessage || 'فشل حفظ طلب التقييم في قاعدة البيانات'
        };
    },

    _findEvaluationApprovalRequestIndex_(matchId) {
        const mid = String(matchId || '').trim();
        if (!mid) return -1;
        return (AppState.appData.contractorEvaluationApprovalRequests || []).findIndex((r) => {
            if (!r) return false;
            const id = String(r.id || '').trim();
            const legacy = String(r.legacyTempId || r._tempId || '').trim();
            return id === mid || legacy === mid;
        });
    },

    _markEvaluationApprovalSyncError_(matchId, message) {
        const tempIndex = this._findEvaluationApprovalRequestIndex_(matchId);
        if (tempIndex === -1) return;
        const row = AppState.appData.contractorEvaluationApprovalRequests[tempIndex];
        row._isPendingSync = false;
        row._syncError = true;
    ...
    async syncEvaluationApprovalRequestToBackend(requestData, tempId, options = {}) {
        ...
        if (this._activeSyncs[syncKey]) {
            return this._activeSyncs[syncKey];
        }
        ...
        const syncPromise = (async () => {
            try {
                ...
                const idStr = String(actualTempId || '');
                const isTempId = idStr.startsWith('TEMP_');
                if (isTempId) {
                    delete payload.id;
                ...
                const backendResult = await this.postEvaluationApprovalToBackend_(payload);
                ...
                if (backendResult?.success) {
                    ...
                    const tempIndex = this._findEvaluationApprovalRequestIndex_(actualTempId);
                    if (tempIndex !== -1) {
                        const preservedEvaluationData =
                            AppState.appData.contractorEvaluationApprovalRequests[tempIndex].evaluationData;
                        ...
                        delete AppState.appData.contractorEvaluationApprovalRequests[tempIndex]._syncErrorMessage;
                    } else {
                ...
                const failMsg = backendResult?.message || 'فشل المزامنة';
                this._markEvaluationApprovalSyncError_(actualTempId, failMsg);
                if (window.DataManager?.save) window.DataManager.save();
                Notification.error('فشل مزامنة طلب التقييم: ' + failMsg);
                throw new Error(failMsg);
            } catch (error) {
                Utils.safeError('❌ خطأ في مزامنة طلب اعتماد التقييم:', error);
                this._markEvaluationApprovalSyncError_(tempId || sourceRequest?.id, error.message || 'خطأ في المزامنة');
                if (window.DataManager?.save) window.DataManager.save();
                throw error;
            }
        })();

        this._activeSyncs[syncKey] = syncPromise;
        try {
            return await syncPromise;
        } finally {
            if (this._activeSyncs?.[syncKey] === syncPromise) {
                delete this._activeSyncs[syncKey];
            }
        }
    },
    ...
    async viewApprovalRequest(requestId, requestCategory = 'approval') {
        ...
        if (requestCategory === 'deletion') {
            ...
            if (!request || this._evaluationApprovalNeedsSync_(request)) {
                await this.ensureEvaluationApprovalRequestSynced_(rid, { showLoader: true, maxRetries: 3 });
            }
            request = this.findEvaluationApprovalRequest(rid);
            if (!request) {
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 800px;">
                ...
                <div class="modal-footer" style="margin-top: auto; flex-shrink: 0; width: 100%;">
                    ...
                    ` : ''}
                    ${isAdmin && (request._isPendingSync || (request.id && String(request.id).startsWith('TEMP_')) || request._syncError) ? `
                        <div class="flex flex-col gap-2">
                            <span class="text-sm text-blue-600"><i class="fas fa-sync fa-spin ml-1"></i> الطلب قيد المزامنة مع الخادم — انتظر ثوانٍ أو أعد المحاولة</span>
                            ${request._syncErrorMessage ? `<span class="text-sm text-red-600">${Utils.escapeHTML(request._syncErrorMessage)}</span>` : ''}
                            <button type="button" class="btn-secondary btn-sm" id="retry-eval-sync-btn">
                                <i class="fas fa-redo ml-1"></i>إعادة المزامنة
                            </button>
                        </div>
                    ` : ''}
                </div>
        ...
        const retrySyncBtn = modal.querySelector('#retry-eval-sync-btn');
        if (retrySyncBtn) {
            retrySyncBtn.addEventListener('click', async () => {
                try {
                    retrySyncBtn.disabled = true;
                    const current = this.findEvaluationApprovalRequest(requestId);
                    if (current) {
                        current._isPendingSync = true;
                        current._syncError = false;
                    }
                    await this.ensureEvaluationApprovalRequestSynced_(requestId, { showLoader: true, maxRetries: 4 });
                    modal.remove();
                    await this.viewApprovalRequest(requestId, requestCategory);
                } catch (_retryErr) {
    ...
    async saveRequestChanges(requestId, requestCategory = 'approval') {
        ...
        try {
            ...
            const isEvaluationRequest = requestCategory === 'evaluation_approval' || request.requestType === 'evaluation';
            let updateData;
            ...
            const action = requestCategory === 'deletion'
                ...
                : (requestCategory === 'evaluation_approval'
                    ? 'updateContractorEvaluationApprovalRequest'
    ...
    async approveRequest(requestId, requestCategory = 'approval') {
        ...
        if (requestCategory === 'evaluation_approval') {
            request = await this.ensureEvaluationApprovalRequestSynced_(requestId, { showLoader: true, maxRetries: 4 });
            if (!request) {
                Notification.error('طلب اعتماد التقييم غير موجود');
                return;
            }
            if (this._evaluationApprovalNeedsSync_(request)) {
                Notification.error(
        ...
        if (!request && String(requestId).startsWith('TEMP_')) {
        ...
        const confirmMsg = 'هل أنت متأكد من اعتماد هذا الطلب؟ سيتم إضافة المقاول/المورد إلى قائمة المعتمدين.';
