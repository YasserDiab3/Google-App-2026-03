...
const Training = {
    ...
    ensureData() {
        ...
        if (!Array.isArray(data.legalRegister)) data.legalRegister = [];
    ...
    renderContractorAnalyticsDashboard(model, state) {
        ...
        const optionList = (items, selected) => this._analyticsSelectOptions(items, selected);
            ...
            return `
                <div class="contractor-analytics-pivot-wrap">
                    <table class="contractor-analytics-pivot-table w-full">
                        <thead>
                            <tr>
                                <th><i class="fas ${mode === 'topic' ? 'fa-book' : 'fa-user'} ml-2"></i>${mode === 'topic' ? 'الموضوع' : 'الموظف'}</th>
                ...
                <div class="contractor-analytics-details-wrap">
                    <table class="contractor-analytics-details-table w-full">
                        ...
                        <tbody>
                            ${rows.map((r) => `
                                <tr>
                                    <td><span class="date-badge">${(r.raw?.date || r.raw?.attendanceDate) ? safe(Utils.formatDate(r.raw.date || r.raw.attendanceDate)) : '-'}</span></td>
                                    <td title="${safe(r.topic)}" style="max-width:200px;overflow:hidden;text-overflow:ellipsis;">${safe(r.topic)}</td>
                                    <td><span class="trainer-name">${safe(r.employee)}</span></td>
                                    <td>${safe(r.trainingType)}</td>
            ...
            <div class="contractor-analytics-section tx-analytics-section grid grid-cols-1 gap-4">
                <div class="contractor-analytics-slicers">
                    ...
                    <div class="contractor-analytics-slicers-grid tx-analytics-slicers-wide">
                        <div class="filter-group">
                            ...
                            <select id="attendance-analytics-employee">${optionList(model.dimensions.employees, state.employee)}</select>
                        </div>
                        <div class="filter-group">
                            <label><i class="fas fa-book"></i><span>الموضوع</span></label>
                            <select id="attendance-analytics-topic">${optionList(model.dimensions.topics, state.topic)}</select>
                        </div>
                        <div class="filter-group">
                            <label><i class="fas fa-sitemap"></i><span>الإدارة</span></label>
                            <select id="attendance-analytics-department">${optionList(model.dimensions.departments, state.department)}</select>
                        </div>
                        <div class="filter-group">
                            <label><i class="fas fa-industry"></i><span>المصنع</span></label>
                            <select id="attendance-analytics-factory">${optionList(model.dimensions.factories, state.factory)}</select>
                        </div>
                        <div class="filter-group">
                            <label><i class="fas fa-tag"></i><span>نوع التدريب</span></label>
                            <select id="attendance-analytics-trainingType">${optionList(model.dimensions.trainingTypes, state.trainingType)}</select>
                        </div>
                        <div class="filter-group">
                            <label><i class="fas fa-chalkboard-teacher"></i><span>المحاضر</span></label>
                            <select id="attendance-analytics-trainer">${optionList(model.dimensions.trainers, state.trainer)}</select>
                        </div>
                        <div class="filter-group search-full">
                            <label><i class="fas fa-search"></i><span>بحث سريع</span></label>
                            <input id="attendance-analytics-search" placeholder="ابحث عن موظف، موضوع، إدارة..." value="${safe(state.search)}">
                        </div>
                    </div>
                ...
                <div class="contractor-analytics-kpi-grid tx-analytics-kpi-4">
                    <div class="contractor-analytics-kpi-card kpi-green">
                        ...
                        <div class="kpi-value">${fmt(computed.totals.records)}</div>
                    </div>
                    <div class="contractor-analytics-kpi-card kpi-amber">
                        <div class="kpi-label"><i class="fas fa-clock"></i>الساعات</div>
                        <div class="kpi-value">${fmt(computed.totals.hours, 2)}</div>
                    ...
                    <div class="contractor-analytics-kpi-card kpi-blue">
                        ...
                        <div class="kpi-value">${fmt(computed.totals.employees)}</div>
                    </div>
                    <div class="contractor-analytics-kpi-card kpi-indigo">
                        <div class="kpi-label"><i class="fas fa-book"></i>الموضوعات</div>
                        <div class="kpi-value">${fmt(computed.totals.topics)}</div>
                ...
                <div class="contractor-analytics-tabs-bar">
                    <div class="tabs-row">
                        <div class="tabs-group">
                            <button type="button" id="attendance-analytics-tab-employee" class="contractor-analytics-tab ${tabActiveClass('employee')}">
                                <i class="fas fa-user"></i>حسب الموظف
                            </button>
                            <button type="button" id="attendance-analytics-tab-topic" class="contractor-analytics-tab ${tabActiveClass('topic')}">
                                <i class="fas fa-book"></i>حسب الموضوع
                            </button>
                            <button type="button" id="attendance-analytics-tab-details" class="contractor-analytics-tab ${tabActiveClass('details')}">
                        ...
                        <div class="contractor-analytics-sort-group">
                            <div class="contractor-analytics-sort-box">
                                ...
                                <select id="attendance-analytics-sortby">
                                    <option value="hours" ${state.sortBy === 'hours' ? 'selected' : ''}>الساعات</option>
                                    <option value="count" ${state.sortBy === 'count' ? 'selected' : ''}>عدد السجلات</option>
                                </select>
                ...
                <div class="contractor-analytics-content">
                    ${state.view === 'topic'
                        ? renderPivotTable(computed.topTopics, 'topic')
                        : state.view === 'details'
                            ? renderDetails()
                            : renderPivotTable(computed.topEmployees, 'employee')}
                </div>
            </div>
        `;
    },
    ...
    _syncSelectOptions(selectId, optionsHtml) {
        const sel = document.getElementById(selectId);
        if (!sel) return;
        const current = sel.value;
        sel.innerHTML = `<option value="">جميع الأشهر</option>${optionsHtml || ''}`;
        if (current && Array.from(sel.options).some((o) => o.value === current)) sel.value = current;
    },

    _analyticsSelectOptions(items, selected, max = 250) {
        const safe = (v) => Utils.escapeHTML(String(v ?? ''));
        const sel = String(selected ?? '').replace(/\s+/g, ' ').trim();
        const list = Array.isArray(items) ? items.slice() : [];
        const limited = list.length > max ? list.slice(0, max) : list;
        if (sel && !limited.includes(sel)) {
            const found = list.find((v) => String(v) === sel);
            if (found != null) limited.unshift(found);
        }
        return [`<option value="">الكل</option>`]
            .concat(limited.map((v) => `<option value="${safe(v)}" ${sel === String(v) ? 'selected' : ''}>${safe(v)}</option>`))
            .join('');
    },

    _analyticsPlaceholder(message) {
        return `
            <div class="tx-analytics-placeholder">
                <i class="fas fa-chart-pie"></i>
                <p>${Utils.escapeHTML(message || 'جاري تجهيز التحليل…')}</p>
            </div>
        `;
    },

    _scheduleDeferredAnalytics(tabName) {
        const run = () => {
            if (this._currentActiveTab !== tabName) return;
            if (tabName === 'contractors') {
                this.updateContractorStatsWithFilter(document.getElementById('contractor-month-filter')?.value || '');
            } else if (tabName === 'attendance') {
                this.refreshAttendanceAnalytics(document.getElementById('attendance-month-filter')?.value || '');
            }
        };
    ...
    buildAttendanceTabMarkup() {
        let siteOptions = '';
        try {
            siteOptions = (this.getSiteOptions() || []).map((site) => `
    ...
    async renderTabContent(tabName) {
        if (tabName === 'programs') {
            ...
            return this.buildContractorsTabMarkup();
        } else if (tabName === 'attendance') {
            return this.buildAttendanceTabMarkup();
    ...
    async loadTrainingList() {
        ...
        //   - vertical-align: middle لكل الخلايا → محاذاة عمودية متناسقة
        //   - alternating row colors → فصل بصري واضح بين الصفوف
        //   - white-space: nowrap على عمود الإجراءات → الأزرار في سطر واحد
        //   - word-break + min-width على عمود الاسم → النص الطويل ينساب بدلاً من التمدد
        //   - row hover effect مع transition سلس
        container.innerHTML = `
    ...
    initializeFormInteractions() {
        ...
        const updateCalculatedHours = () => {
            ...
            if (hoursDisplay && (!startVal || !endVal)) {
...
<div class="training-attendance-print-root" style="font-family:'Cairo','Segoe UI',Tahoma,sans-serif;direction:rtl;text-align:right;color:#1f2937;-webkit-print-color-adjust:exact;print-color-adjust:exact">
  <div style="border-radius:14px;overflow:hidden;box-shadow:0 4px 6px -1px rgba(0,0,0,0.08);border:1px solid #e5e7eb">
                ...
                <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                    ...
                    <div style="display:flex;gap:3px;flex-wrap:wrap;">
                        ${['30','90','180','365','0'].map((v,i) => {
                            ...
                            const active = (this._trainPeriod || '0') === v;
                            return `<button class="train-period-btn" data-period="${v}" style="padding:5px 10px;border-radius:8px;border:none;cursor:pointer;font-size:0.75rem;font-weight:600;transition:all .2s;background:${active?'#fff':'rgba(255,255,255,0.15)'};color:${active?'#312e81':'#fff'};">${labels[i]}</button>`;
                        }).join('')}
                    </div>
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;">
                    ${[
                        ...
                        <div>
                            ...
                            <select id="${f.id}" style="width:100%;padding:7px 10px;border:1.5px solid #c7d2fe;border-radius:8px;font-size:0.82rem;background:#fff;color:#374151;cursor:pointer;" onfocus="this.style.borderColor='#4f46e5'" onblur="this.style.borderColor='#c7d2fe'">
    ...
    // ── دالة قديمة مُبقاة للتوافق ──
    _renderAnalysisTabLegacy() {
    ...
    loadAttendanceRegistry() {
        ...
        this._attendanceRegistryPageSize = this._attendanceRegistryPageSize || 80;
        ...
        container.innerHTML = visibleRows.map((record, index) => {
                ...
                <tr>
                    <td colspan="14" class="text-center py-4">
                        <button type="button" id="attendance-registry-show-more" class="btn-secondary">
                            عرض المزيد (${shown} من ${filtered.length})
                        </button>
                    </td>
                </tr>
            ` : '');
    ...
    setupAttendanceRegistryListeners() {
        ...
        if (moreBtn) {
            moreBtn.onclick = () => {
                this._attendanceRegistryShown = (this._attendanceRegistryShown || 80) + 80;
                this.loadAttendanceRegistry();
            };
        }
    },
    ...
    // ══════════════════════════════════════════════════════════════
    //  لوحة تحليل التدريب — الدوال الجديدة
    // ══════════════════════════════════════════════════════════════

    async updateTrainingAnalyticsDashboard() {
        const root = document.getElementById('train-analytics-root');
        if (!root) return;
        ...
        // ── ملء الفلاتر ──
        this._tPopulateFilters(byPeriod);

        // ── تطبيق الفلاتر ──
        const t = this._tApplyFilters(byPeriod);
        const total = t.length;
        const countEl = document.getElementById('train-filter-count');
        if (countEl) countEl.textContent = `${total} برنامج`;

        // ── KPIs ──
        const completed = t.filter(x => x.status === 'مكتمل').length;
        const upcoming  = t.filter(x => x.status === 'مخطط' || x.status === 'قادم').length;
        ...
        const thisMonth = t.filter(x => {
            const d = new Date(x.date || x.startDate || '');
            const n = new Date();
            return !isNaN(d) && d.getFullYear() === n.getFullYear() && d.getMonth() === n.getMonth();
        }).length;

        const kpiEl = document.getElementById('train-kpi-strip');
        if (kpiEl) {
            const kpis = [
                { label:'إجمالي البرامج',      value:total,                          icon:'fas fa-graduation-cap',       color:'#4f46e5', bg:'#eef2ff', border:'#c7d2fe' },
                { label:'مكتملة',              value:completed,                       icon:'fas fa-check-circle',         color:'#10b981', bg:'#ecfdf5', border:'#a7f3d0' },
                { label:'مخططة/قادمة',         value:upcoming,                        icon:'fas fa-calendar-alt',         color:'#f59e0b', bg:'#fffbeb', border:'#fde68a' },
                { label:'إجمالي المتدربين',    value:totalPart.toLocaleString('en-US'),icon:'fas fa-users',               color:'#6366f1', bg:'#eef2ff', border:'#c7d2fe' },
                { label:'برامج الموظفين',      value:empT,                            icon:'fas fa-user-tie',             color:'#0ea5e9', bg:'#f0f9ff', border:'#bae6fd' },
                { label:'برامج المقاولين',     value:conT,                            icon:'fas fa-users-cog',            color:'#f97316', bg:'#fff7ed', border:'#fed7aa' },
        ...
        // ── تحميل Chart.js ──
        const loaded = await this._tEnsureChartJS();
        if (!loaded || typeof Chart === 'undefined') {
            root.insertAdjacentHTML('afterbegin', `<div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:14px 18px;margin-bottom:16px;"><i class="fas fa-exclamation-triangle" style="color:#d97706;"></i> <span style="font-size:0.85rem;color:#92400e;">تعذّر تحميل مكتبة الرسوم البيانية.</span></div>`);
            return;
        }

        // ── الرسوم البيانية ──
        const statusColors = { 'مكتمل':'rgba(16,185,129,0.85)', 'مخطط':'rgba(245,158,11,0.85)', 'جارٍ':'rgba(59,130,246,0.85)', 'قادم':'rgba(139,92,246,0.85)', 'ملغي':'rgba(239,68,68,0.85)' };
        const statusG = this._tGroupBy(t, 'status');
        this._tDrawDoughnut('train-chart-status', statusG.labels, statusG.data, statusG.labels.map(l => statusColors[l] || 'rgba(148,163,184,0.8)'));

        const typeG = this._tGroupBy(t, 'trainingType', 10);
        this._tDrawDoughnut('train-chart-type', typeG.labels, typeG.data, this._tChartColors(typeG.labels.length));
        ...
        const topT = t.slice().sort((a,b) => {
            const pa = this.getParticipantsCount ? this.getParticipantsCount(b) : (Number(b.participantsCount)||0);
            const pb = this.getParticipantsCount ? this.getParticipantsCount(a) : (Number(a.participantsCount)||0);
            return pa - pb;
        }).slice(0, 20);
        const topCount = document.getElementById('train-top-count');
        const tbody = document.getElementById('train-top-tbody');
    ...
    // ── مساعد: رسم Doughnut ──
    _tDrawDoughnut(canvasId, labels, data, colors) {
        ...
        if (!this._trainCharts) this._trainCharts = {};
        const prev = this._trainCharts[canvasId];
        if (prev) { try { prev.destroy(); } catch(e){} }
        this._trainCharts[canvasId] = new Chart(canvas, {
            type: 'doughnut',
    ...
    // ── مساعد: رسم HBar ──
    _tDrawHBar(canvasId, labels, data, color) {
        const canvas  = document.getElementById(canvasId);
        const emptyEl = document.getElementById(canvasId + '-empty');
        if (!canvas) return;
        if (!data.length) {
        ...
        this._trainCharts[canvasId] = new Chart(canvas, {
            ...
            options: {
                ...
                plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ${ctx.parsed.x} برنامج` } } },
                scales: {
    ...
    _tDrawTrend(canvasId, records) {
        ...
        this._trainCharts[canvasId] = new Chart(canvas, {
            ...
            data: {
                ...
                datasets: [
                    { label: 'عدد البرامج', data: counts, backgroundColor: counts.map(c => c === Math.max(...counts) ? 'rgba(79,70,229,0.85)' : 'rgba(79,70,229,0.5)'), borderRadius: 6, borderSkipped: false, order: 1 },
    ...
    // ── مساعد: مشاركون شهرياً ──
    _tDrawParticipants(canvasId, records) {
        const canvas  = document.getElementById(canvasId);
        const emptyEl = document.getElementById(canvasId + '-empty');
        if (!canvas) return;
        const now = new Date();
        ...
        for (let i = 11; i >= 0; i--) {
            const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
            months.push({ year: d.getFullYear(), month: d.getMonth(), label: `${arabicMonths[d.getMonth()]}` });
        }
        const counts = months.map(m => records.filter(x => {
            const d = new Date(x.date || x.startDate || '');
            return !isNaN(d.getTime()) && d.getFullYear() === m.year && d.getMonth() === m.month;
        }).reduce((s, x) => s + (this.getParticipantsCount ? this.getParticipantsCount(x) : (Number(x.participantsCount)||0)), 0));
        if (counts.reduce((a,b)=>a+b,0) === 0) {
            canvas.style.display = 'none';
            if (emptyEl) emptyEl.style.display = 'flex';
            return;
    ...
    // ── مساعد: ألوان ──
    _tChartColors(n) {
        const palette = ['rgba(79,70,229,0.8)','rgba(16,185,129,0.8)','rgba(245,158,11,0.8)','rgba(239,68,68,0.8)','rgba(59,130,246,0.8)','rgba(139,92,246,0.8)','rgba(236,72,153,0.8)','rgba(20,184,166,0.8)','rgba(249,115,22,0.8)','rgba(168,85,247,0.8)'];
        return Array.from({ length: n }, (_, i) => palette[i % palette.length]);
    },

    // ── تصدير PDF بهيدر وفوتر احترافيين ──
    async _tExportPDF() {
        const root = document.getElementById('train-analytics-root');
        if (!root) return;
        const btn = document.getElementById('train-export-pdf-btn');
        ...
        try {
            ...
            const filterPanel = document.getElementById('train-filter-panel');
    ...
    _tBindAnalyticsEvents() {
        const root = document.getElementById('train-analytics-root');
        if (!root) return;

        // أزرار الفترة
        root.querySelectorAll('.train-period-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                this._trainPeriod = btn.getAttribute('data-period');
                root.querySelectorAll('.train-period-btn').forEach(b => {
                    const active = b === btn;
                    b.style.background = active ? '#fff' : 'rgba(255,255,255,0.15)';
                    b.style.color      = active ? '#312e81' : '#fff';
                });
                this.updateTrainingAnalyticsDashboard();
            });
        });
        ...
        // تحديث
        const refreshBtn = document.getElementById('train-analytics-refresh');
        if (refreshBtn) refreshBtn.addEventListener('click', () => this.updateTrainingAnalyticsDashboard());

        // PDF
        const pdfBtn = document.getElementById('train-export-pdf-btn');
        if (pdfBtn) pdfBtn.addEventListener('click', () => this._tExportPDF());

        // فلاتر — toggle panel
        const toggleBtn   = document.getElementById('train-toggle-filters-btn');
        const filterPanel = document.getElementById('train-filter-panel');
    ...
    renderLegalTrainingTab() {
        ...
        const regStats = this.getLegalRegisterStats();
        const activeSub = this._legalRegisterSubTab || 'register';
        return `
            <div class="legal-sub-tabs">
                <button class="legal-sub-tab ${activeSub === 'register' ? 'active' : ''}" data-sub="register">
                    <i class="fas fa-balance-scale ml-2"></i>سجل التشريعات والقوانين
                </button>
                <button class="legal-sub-tab ${activeSub === 'training' ? 'active' : ''}" data-sub="training">
                    <i class="fas fa-gavel ml-2"></i>التدريبات القانونية
                </button>
            </div>

            <div id="legal-register-section" class="${activeSub === 'register' ? '' : 'hidden'}">
                    ...
                    <div class="lr-kpi-card lr-kpi-blue">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="kpi-value" id="lr-total-count">${regStats.total}</p>
                            </div>
                        </div>
                    </div>
                    <div class="lr-kpi-card lr-kpi-green">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="kpi-value" id="lr-applicable-count">${regStats.applicable}</p>
                            </div>
                        </div>
                    </div>
                    <div class="lr-kpi-card lr-kpi-amber">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="kpi-value" id="lr-amended-count">${regStats.amended}</p>
                            </div>
                        </div>
                    </div>
                    <div class="lr-kpi-card lr-kpi-red">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="kpi-value" id="lr-repealed-count">${regStats.repealed}</p>
                            </div>
                        </div>
                    </div>
                    <div class="lr-kpi-card lr-kpi-purple">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="kpi-value" id="lr-compliance-rate">${regStats.complianceRate}%</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="lr-filters-bar">
                    <div class="filter-group">
                        ...
                        <select id="lr-category-filter" class="form-input" style="max-width: 200px;">
                            <option value="">الكل</option>
                            ${this.LEGAL_REGISTER_CATEGORIES.map(c => `<option value="${c.value}">${c.label}</option>`).join('')}
                        </select>
                    </div>
                    <div class="filter-group">
                        <label>الحالة:</label>
                        <select id="lr-status-filter" class="form-input" style="max-width: 160px;">
                            <option value="">الكل</option>
                            ${this.LEGAL_REGISTER_STATUSES.map(s => `<option value="${s.value}">${s.label}</option>`).join('')}
                        </select>
                    </div>
                    <div class="filter-group">
                        ...
                        <select id="lr-priority-filter" class="form-input" style="max-width: 160px;">
                            <option value="">الكل</option>
                            ${this.LEGAL_PRIORITIES.map(p => `<option value="${p.value}">${p.label}</option>`).join('')}
                        </select>
                    </div>
            ...
            <div id="legal-training-section" class="${activeSub === 'training' ? '' : 'hidden'}">
    ...
    _bindLegalTrainingEvents() {
        ...
        const subTabs = document.querySelectorAll('.legal-sub-tab');
        subTabs.forEach(btn => {
            if (!btn.dataset.bound) {
                btn.addEventListener('click', () => {
                    const sub = btn.dataset.sub;
                    this._legalRegisterSubTab = sub;
                    subTabs.forEach(b => b.classList.toggle('active', b.dataset.sub === sub));
                    document.getElementById('legal-register-section')?.classList.toggle('hidden', sub !== 'register');
                    document.getElementById('legal-training-section')?.classList.toggle('hidden', sub !== 'training');
                    if (sub === 'register') this.loadLegalRegisterList();
                    else this.loadLegalTrainingList();
                });
                btn.dataset.bound = '1';
            }
        });
        ...
        const rows = items.map(r => {
            ...
            <tr>
                ...
                <td class="text-sm text-center">
                    <button class="lr-amd-btn" onclick="Training.showLegalAmendments('${r.id}')" title="عرض التحديثات القانونية">
                ...
                <td>
                    <div class="flex items-center gap-1">
                        <button class="btn-icon btn-sm" onclick="Training.showLegalRegisterForm('${r.id}')" title="تعديل">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn-icon btn-sm text-red-600" onclick="Training.deleteLegalRegisterRecord('${r.id}')" title="حذف">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </td>
        ...
        const html = `
            <div class="modal-overlay active" id="lr-amd-form-modal">
                <div class="modal-content" style="max-width: 640px;">
                    ...
                    <form id="lr-amd-form" onsubmit="Training.handleAmendmentSubmit(event, '${registerId}')">
