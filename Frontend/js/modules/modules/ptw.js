...
const PTW = {
    ...
    switchTab(tab) {
        ...
        // Update tab buttons — layout behaviour is handled by CSS (.ptw-tab-btn in module-tabs.css)
    ...
    async saveManualPermitEntry(modal, entryId = null) {
        try {
            ...
            const fullLocationText = Array.isArray(formData.locationEntries) && formData.locationEntries.length > 0
                ? formData.locationEntries
                    .map(entry => entry?.sublocation ? `${entry.location} - ${entry.sublocation}` : entry.location)
                    .filter(Boolean)
                    .join(' | ')
                : (formData.sublocation ? `${formData.location} - ${formData.sublocation}` : formData.location);
            ...
            if (entryId) {
                ...
                entry = {
                    ...
                    ...fullEntryData,
                    id: existingEntry.id,
                    permitId: existingEntry.permitId || Utils.generateId('PTW'),
