...
/**
 * Centralized breakpoints â€” must match css/design-tokens.css values.
 * Use these constants instead of raw pixel literals throughout the UI code.
 *   xs  < 480   : small phones
 *   sm  < 768   : phones
 *   md  < 1024  : tablets (also used as mobile threshold for sidebar overlay)
 *   lg  < 1280  : small desktops
 *   xl  < 1600  : desktops
 *   2xl >= 1600 : large desktops
 */
window.BREAKPOINTS = Object.freeze({
    ...
    lg:  1280,
    xl:  1600,
    isMobile:  () => window.innerWidth <= window.BREAKPOINTS.md,
    isTablet:  () => window.innerWidth > window.BREAKPOINTS.md && window.innerWidth <= window.BREAKPOINTS.lg,
    isDesktop: () => window.innerWidth > window.BREAKPOINTS.lg,
});
...
window.UI = {
    ...
    _backgroundSyncInterval: null, // âœ… مزامنة تلقائية دورية في الخلفية
    ...
    showLoginScreen() {
        ...
        // âœ… إصلاح: التحقق من وجود بيانات الجلسة أولاً (حتى لو لم يكن AppState.currentUser معيّناً بعد)
                ...
                <div class="modal-body space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="text-sm text-gray-800">${Utils.escapeHTML(plan.contactPerson || 'غير محدد')} ${plan.contactPhone ? 'â€¢ ' + Utils.escapeHTML(plan.contactPhone) : ''}</div>
                        </div>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">الوصف</p>
    ...
    async showMainApp() {
        ...
        if (!AppState.currentUser) {
            if (AppState.debugMode) Utils.safeLog('âš ï¸ لا يوجد مستخدم مسجل دخول - لا يمكن عرض التطبيق');
            return;
        }
    ...
    /** مدة صلاحية "شاهد السياسة" بالأيام â€” بعدها نعرض السياسة مرة أخرى (كل 10 أيام) */
    _postLoginPolicyValidityDays: 10,

    /** مفتاح localStorage لآخر مشاهدة سياسة â€” يُربط بمعرّف المستخدم لضمان ظهور السياسة مرة واحدة لكل مستخدم في هذا المتصفح */
    _policyLastSeenKey() {
        ...
        try {
            ...
            try {
                if (typeof localStorage !== 'undefined' && !localStorage.getItem(storageKey))
                    return false; // أول مرة لهذا المستخدم في هذا المتصفح â€” نعرض السياسة دائماً
            } catch (e) {}
            ...
            if (!seenAt) return false; // لا يوجد تاريخ مشاهدة صالح â€” نعرض السياسة
            if (now - seenAt >= daysMs) return false; // مرّ أكثر من 10 أيام â€” نعرض السياسة مرة أخرى
            return true; // شاهد خلال آخر 10 أيام â€” لا نعرض
    ...
    _showUpdateModal(newVersion) {
        ...
        modal.innerHTML = `
            <div class="hse-update-message-card" style="background:var(--bg-primary,#fff);color:var(--text-primary,#111);max-width:420px;width:100%;border-radius:16px;box-shadow:0 25px 50px -12px rgba(0,0,0,0.25);padding:1.5rem;text-align:right;">
                ...
                <p style="margin:0 0 0.5rem;font-size:0.9rem;color:var(--gray-600,#4b5563);">تم إجراء تحديث على التطبيق â€” الإصدار: <strong>${safeVersion}</strong></p>
    ...
    /**
     * التحقق من إصدار التطبيق على الخادم (Vercel) â€” إن وُجد إصدار أحدث مما شاهده المستخدم سابقاً، عرض إشعار التحديث.
    ...
    async _checkServerVersion() {
        try {
            ...
            if (AppState.debugMode && typeof Utils !== 'undefined' && Utils.safeWarn) Utils.safeWarn('âš ï¸ التحقق من إصدار الخادم:', e);
        }
    },

    /**
     * عرض رسالة "هناك تحديث جديد" عند تسجيل الدخول â€” يُعرض فقط إذا كان appVersion أحدث فعلاً من آخر إصدار رآه المستخدم.
     * إذا كان lastSeen في localStorage أحدث من currentVersion (بسبب بقاء JS قديم في الكاش) â†’ لا تعرض شيئاً.
     */
    _showUpdateMessageIfNeeded() {
        try {
            ...
            if (AppState.debugMode && typeof Utils !== 'undefined' && Utils.safeWarn) Utils.safeWarn('âš ï¸ خطأ في عرض رسالة التحديث:', e);
        }
    },
    ...
    /**
     * مراقب دوري: إن بقيت فئة سياسة الدخول على body دون طبقة السياسة، أو علق التحميل طويلاً â€” أصلح الواجهة.
     */
    ...
    _continueMainAppSetup() {
        ...
        // âš ï¸ لا يتم تشغيل هذا المنطق عند مجرد إعادة تحميل الصفحة (isPageRefresh)
        ...
        // âœ… إصلاح: إعادة تحديث القائمة بعد تحميل البيانات للتأكد من الصلاحيات الصحيحة
        setTimeout(() => {
            if (typeof Permissions !== 'undefined' && typeof Permissions.updateNavigation === 'function') {
                Permissions.updateNavigation();
            }
        ...
        // âœ… إعادة تحديث صورة المستخدم بعد تأخير قصير لضمان ظهورها بعد الدخول مباشرة (عند جاهزية الـ DOM)
        ...
        // التحقق من إصدار الخادم (Vercel) â€” يُهيَّأ مرة واحدة فقط طوال عمر الصفحة لمنع تراكم المؤقتات والمستمعين
        ...
        // âœ… استعادة القسم المحفوظ إذا كان موجوداً (بغض النظر عن isPageRefresh)
        ...
        // âœ… إصلاح Reload: إذا كان القسم المحفوظ غير مسموح به للمستخدم (صلاحيات تغيّرت/مستخدم محدود)
        ...
        // âœ… تحديث AppState.currentSection قبل عرض القسم لضمان التزامن
        AppState.currentSection = sectionToShow;

        // âœ… تحديث عنوان الصفحة (hash) ليعكس القسم المستعاد بعد إعادة التحميل
        try {
        ...
        (async () => {
            try {
                ...
                // âœ… إصلاح: تحميل إعدادات الشركة (بما في ذلك الشعار) إذا لم تكن محملة
                ...
                // âœ… إضافة: الاستماع لحدث اكتمال المزامنة لتحديث الموديولات تلقائياً
    ...
    /**
     * âœ… إعداد الاستماع لحدث اكتمال المزامنة
     */
    setupSyncDataCompletedListener() {
        ...
        this._syncDataCompletedHandler = (event) => {
            ...
            // âœ… إصلاح: تحديث صلاحيات المستخدم الحالي إذا تم تحديث ورقة Users
            ...
            if (includesUsersSheet && AppState.currentUser && AppState.appData.users) {
                ...
                if (updatedUser) {
                    // âœ… إصلاح: تطبيع الصلاحيات قبل التحديث
    ...
    updateCompanyLogoHeader() {
        ...
        if (header) {
            if (AppState.companyLogo) {
                header.classList.add('has-company-logo');
                ...
                if (logoImgRight) logoImgRight.classList.add('hidden');
            } else {
            ...
            const shouldApplyTopPadding = !!(header && header.style.display === 'flex' && window.innerWidth > BREAKPOINTS.md);
    ...
    updateLoginLogo() {
        ...
        if (!loginLogoImg || !defaultLogoIcon) {
            Utils.safeWarn('âš ï¸ عناصر الشعار غير موجودة في DOM');
            return;
        }
        ...
        if (!logoUrl) {
            try {
                ...
                Utils.safeWarn('âš ï¸ خطأ في قراءة الشعار من localStorage:', e);
            }
        }
        ...
        if (logoUrl && logoUrl.trim() !== '') {
            ...
            loginLogoImg.onerror = () => {
                if (!hasError) {
                    hasError = true;
                    Utils.safeWarn('âš ï¸ فشل تحميل شعار الشركة من:', logoUrl);
            ...
            loginLogoImg.onload = () => {
                ...
                loginLogoImg.classList.add('login-logo-img');
    ...
    bindSidebarEvents() {
        ...
        if (!this.sidebarKeydownBound) {
            document.addEventListener('keydown', (event) => {
                if (event.key === 'Escape' && window.innerWidth <= BREAKPOINTS.md) {
                    const sidebar = document.querySelector('.sidebar');
                    if (sidebar && sidebar.classList.contains('open')) {
                        this.toggleSidebar(false);
                    }
    ...
    updateUserProfilePhoto() {
        ...
        // âœ… استخدام صورة من appData.users أو من currentUser (بعد الدخول مباشرة)
    ...
    showHeaderActions() {
        ...
        const shouldShowHeader = (companyLogoHeader && companyLogoHeader.style.display !== 'none') || window.innerWidth > BREAKPOINTS.md;
    ...
    showSection(sectionName) {
        ...
        if (AppState.debugMode) {
            Utils.safeLog(' محاولة العثور على:', sectionId, section ? 'âœ… موجود' : ' غير موجود');
        }

        if (!section) {
            if (AppState.debugMode) {
                Utils.safeWarn('âš  القسم غير موجود في DOM، جاري البحث ي جميع الأقسام...');
                const allSections = document.querySelectorAll('.section');
                Utils.safeLog('ðŸ“‹ الأقسام الموجودة:', Array.from(allSections).map(s => s.id).join(', '));
            }
            ...
            if (mainContent) {
                if (AppState.debugMode) {
                    Utils.safeLog('ðŸ”¨ إنشاء قسم جديد:', sectionId);
                }
                ...
                if (usersSection && sectionName === 'employees') {
                    ...
                    if (AppState.debugMode) {
                        Utils.safeLog('âœ… تم إدراج قسم employees-section بعد users-section');
                    }
                } else {
                    mainContent.appendChild(section);
                    if (AppState.debugMode) {
                        Utils.safeLog('âœ… تم إضافة القسم الجديد إلى main-content');
                    }
                }
            } else {
        ...
        if (section) {
            ...
            if (AppState.debugMode) {
                Utils.safeLog('âœ… تم تعيل القسم:', sectionId);
            }
        ...
        const loadSectionWork = () => {
            // âœ… تقسيم العمل لتقليل setTimeout/idle Violations: تحميل البيانات أولاً، ثم الأيقونات لاحقاً
            setTimeout(() => {
        ...
        if (sectionName === 'dashboard') {
            ...
            // âœ… تجنّب Violation: اجعل requestIdleCallback سريعاً، وادفع العمل الثقيل خارج callback
    ...
    loadSectionData(sectionName, silent = false) {
        ...
        console.log('ðŸ“¢ loadSectionData called for:', sectionName, 'silent:', silent);
        ...
        try {
            switch (sectionName) {
                ...
                case 'clinic':
                    ...
                    // âœ… التحقق من وجود Clinic على window أولاً
                    ...
                    const retryInterval = setInterval(() => {
                        ...
                        if (attempts >= maxAttempts) {
                            ...
                            script.onload = () => {
                                ...
                                if (!silent) Utils.safeError('âŒ موديول FireEquipment لم يُحمّل بعد تحميل السكربت');
                                showFireEquipmentError('الموديول لم يُحمّل بشكل صحيح. يرجى تحديث الصفحة.', true);
                            };
                            script.onerror = () => {
                                if (!silent) Utils.safeError('âŒ فشل تحميل سكربت معدات الحريق');
                            ...
                            if (tryLoad() || attempts >= maxAttempts) {
                                ...
                                if (!tryLoad()) {
                                    ...
                                    if (!document.querySelector('script[src*="periodicinspections.js"]')) {
                                        ...
                                        script.onerror = () => {
                                            if (!silent) Utils.safeError('âŒ فشل تحميل موديول PeriodicInspections');
                                        };
                                        document.body.appendChild(script);
                                    } else if (!silent) {
                                        Utils.safeError('âŒ موديول PeriodicInspections غير متوفر - الموديول لم يُحمّل بشكل صحيح');
                                    }
                                }
                            }
                        }, 350);
                            ...
                            if (tryLoad() || attempts >= maxAttempts) {
                                ...
                                if (!tryLoad()) {
                                    ...
                                    if (!document.querySelector('script[src*="ppe.js"]')) {
                                        ...
                                        script.onload = () => { tryLoad(); };
                                        script.onerror = () => {
                                            if (!silent) Utils.safeError('âŒ فشل تحميل موديول PPE');
                                        };
                                        document.body.appendChild(script);
                                    } else if (!silent) {
                                        Utils.safeError('âŒ موديول PPE غير متوفر - الموديول لم يُحمّل بشكل صحيح');
                                    }
                                }
                            }
                        }, 350);
                            ...
                            if (tryLoad() || attempts >= maxAttempts) {
                                ...
                                if (!tryLoad()) {
                                    ...
                                    if (!document.querySelector('script[src*="violations.js"]')) {
                                        ...
                                        script.onload = () => { tryLoad(); };
                                        script.onerror = () => {
                                            if (!silent) Utils.safeError('âŒ فشل تحميل موديول Violations');
                                        };
                                        document.body.appendChild(script);
                                    } else if (!silent) {
                                        Utils.safeError('âŒ موديول Violations غير متوفر - الموديول لم يُحمّل بشكل صحيح');
                                    }
                                }
                            }
                        }, 350);
    ...
    _bindSidebarNotificationsDelegation(self) {
        ...
        const handler = (e) => {
            try {
                ...
                // âœ… إصلاح: منع انتشار الحدث لمنع التكرار
                e.preventDefault();
                e.stopPropagation();
    ...
    initNotificationsButton() {
        ...
        // âœ… إصلاح: منع الاستدعاء المتكرر اللانهائي
        ...
        try {
            ...
            // âœ… إصلاح: ربط تفويض sidebar للزر الجانبي فقط
            if (sidebarBtn) {
                this._bindSidebarNotificationsDelegation(self);
                // إزالة أي handler مباشر قديم إذا وجد
                ...
                Utils.safeLog('✅ تم ربط زر الإشعارات في القائمة الجانبية عبر التفويض');
            }

            // ربط أزرار الموبايل والهيدر مباشرة (وليس sidebar)
                ...
                try {
                    ...
                    const clickHandler = (e) => {
                        try {
                            ...
                            try {
                                const dropdownId = btn.id === 'mobile-notifications-btn' ? 'mobile-notifications-dropdown' :
        ...
        self._notificationsClickHandler = (e) => {
            try {
                // إذا كان النقر على أحد أزرار الإشعارات: لا نغلق القائمة هنا â€” نترك الزر نفسه يفتح/يغلق عبر الـ handler المباشر (لتجنب عدم استجابة النقر في القائمة الجانبية)
                if (e && e.target) {
                ...
                dropdowns.forEach(dropdownId => {
                    try {
                        ...
                        // تجاهل الأخطاء
                        Utils.safeWarn('âš ï¸ خطأ في إغلاق dropdown:', err);
                    }
                });
            } catch (error) {
                // تجاهل جميع الأخطاء من document click handler
                Utils.safeWarn('âš ï¸ خطأ في معالج النقر:', error);
            }
        };
        ...
        // âœ… إصلاح: إزالة إعادة الاستدعاء المتكرر - يكفي محاولة واحدة
    ...
    /**
     * âœ… دالة مساعدة للتعامل مع النقر على زر الإشعارات في القائمة الجانبية
    ...
    toggleNotificationsDropdown(dropdownId, listId, emptyId, closeBtnId, button) {
        try {
            Utils.safeLog('ðŸ”” toggleNotificationsDropdown - dropdownId:', dropdownId);
            const dropdown = document.getElementById(dropdownId);
            if (!dropdown) {
                Utils.safeWarn('⚠️ لم يتم العثور على dropdown:', dropdownId);
                return;
            ...
            try {
                // âœ… فحص سريع أولاً (inline style)
    ...
    async showNotificationsDropdown(dropdownId, listId, emptyId, button) {
        try {
            ...
            // âœ… عرض dropdown فوراً مع الإشعارات الحالية (لا ننتظر تحميل البيانات)
            ...
            // âœ… إصلاح: استخدام setProperty مع important لتجاوز أي CSS conflicts
            ...
            if (notifications.length === 0) {
                ...
                Utils.safeLog('â„¹ï¸ لا توجد إشعارات غير مقروءة');
            } else {
                if (empty) {
                        ...
                        if (typeof this.renderNotificationItem === 'function') {
                            list.innerHTML = notifications.map(notif => {
                                try {
                                    ...
                                    Utils.safeWarn('âš ï¸ خطأ في رسم إشعار:', err);
                                    return '';
                                }
                            }).filter(Boolean).join('');
                        } else {
                                        ...
                                        try {
                                            ...
                                            if (notificationId) {
                                                ...
                                                try {
                                                    ...
                                                    Utils.safeWarn('âš ï¸ خطأ في الحصول على قائمة الإشعارات:', err);
                                                }
                                                ...
                                                if (notification && typeof notification.onClick === 'function') {
                                                    try {
                                                        ...
                                                        Utils.safeWarn('âš ï¸ خطأ في تنفيذ onClick للإشعار:', err);
                                                    }
                                                }
                                                ...
                                                try {
                                                    ...
                                                    Utils.safeWarn('âš ï¸ خطأ في تمييز الإشعار كمقروء:', err);
                                                }
                                            }
                                        } catch (error) {
                                            Utils.safeWarn('âš ï¸ خطأ في معالجة نقر الإشعار:', error);
                                        }
                                    }, { capture: true, passive: false });
                                } catch (err) {
                                    Utils.safeWarn('âš ï¸ خطأ في ربط حدث الإشعار:', err);
                                }
                            });
                        } catch (err) {
                            Utils.safeWarn('âš ï¸ خطأ في ربط أحداث الإشعارات:', err);
                        }
                    } catch (error) {
                        Utils.safeError('âš ï¸ خطأ في عرض قائمة الإشعارات:', error);
                        if (list) {
            ...
            requestAnimationFrame(() => {
                try {
                    ...
                    Utils.safeWarn('âš ï¸ فشل تموضع قائمة الإشعارات:', e);
                }
            });

            // âœ… تحديث البيانات في الخلفية بعد عرض dropdown (لتحسين الاستجابة)
            ...
            const updateNotificationsList = () => {
                try {
                    ...
                    Utils.safeWarn('âš ï¸ خطأ في تحديث قائمة الإشعارات:', err);
                }
            };
                ...
                try {
                    ...
                    if (typeof FireEquipment !== 'undefined' && typeof FireEquipment.loadApprovalRequestsFromBackend === 'function') {
                        try {
                            ...
                            Utils.safeWarn('âš ï¸ خطأ في تحميل طلبات الموافقة عند عرض الإشعارات:', error);
                        }
                    }
     ...
     * - âœ… إصلاح: دعم خاص للـ dropdown في القائمة الجانبية (sidebar)
     */
    positionNotificationsDropdown(dropdown, button) {
        ...
        Utils.safeLog('ðŸ”” positionNotificationsDropdown - dropdown:', dropdown.id, 'button:', anchorEl.id || 'parent');
        ...
        if (isSidebarDropdown) {
            // âœ… إصلاح خاص للـ sidebar dropdown
    ...
    markAllNotificationsAsRead() {
        try {
            ...
            Utils.safeLog('âœ… تم تعليم جميع الإشعارات كمقروءة');
        } catch (e) {
    ...
    getNotificationsList() {
        ...
        try {
            ...
            // 7. تنبيهات الملاحظات اليومية (حالة السير + التأخير) â€” مربوطة بنفس نظام المقروء/غير المقروء
            try {
    ...
    /**
     * بطاقة تنبيه ملاحظة (نمط العنوان + الوصف + Â«فتح الملاحظة»)
     */
    ...
    async handleSyncClick() {
        ...
        try {
            ...
            if (typeof GoogleIntegration.syncData === 'function') {
                // âœ… تحميل تدريجي: تحميل البيانات غير المكتملة فقط
                syncResult = await GoogleIntegration.syncData({
                    ...
                    includeUsersSheet: true,
                    incremental: true // âœ… تحميل تدريجي: البيانات غير المكتملة فقط
                });
                
                // âœ… إذا نجحت المزامنة، عطّل bootstrap تلقائياً (إن كان مستخدماً)
                try {
    ...
    /**
     * âœ… بدء المزامنة التلقائية الدورية في الخلفية
     */
    startBackgroundSync() {
    ...
    /**
     * âœ… إيقاف المزامنة التلقائية الدورية
     */
    stopBackgroundSync() {
    ...
    updateUserConnectionStatus() {
        ...
        if (user) {
            ...
            // âš ï¸ أمان: لا نقوم بإنشاء/إضافة مستخدمين تلقائياً في قاعدة البيانات.
    ...
    initLanguageToggle() {
        ...
        if (!langToggle) {
            Utils.safeWarn('âš ï¸ لم يتم العثور على زر اللغة');
            return;
        }
    ...
    setLanguage(lang, isInitialLoad = false) {
        ...
        // dir attribute drives all layout via [dir="ltr"]/[dir="rtl"] CSS rules in layout.css

        // تحديث nav-item borders — قيمة لا يمكن للـ CSS الاستنتاج منها بدون dir
        const navItems = document.querySelectorAll('.nav-item');
        navItems.forEach(item => {
            if (isRTL) {
        ...
        const translations = {
            ar: {
                ...
                'dash.activeUsers': 'المستخدمين النشطيين',
                ...
                'dash.electricitySubtitle': 'كيلوواط ساعة (kWh)',
    ...
    fixSidebarForLanguage(lang, isRTL) {
        // All sidebar geometry is now in layout.css via [dir="ltr"]/[dir="rtl"] selectors.
        // JS only sets the dir attribute on <html> (already done in applyLanguage) and
        // removes the obsolete injected style block if it still exists from old code.
        const obsoleteStyle = document.getElementById('sidebar-english-fix-v2');
        if (obsoleteStyle) obsoleteStyle.remove();
...
// بمرحلة الفقاعة يستدعي toggle مرة أخرى â€” فيُفتح القائمة ثم يُغلق في نفس النقرة ويبدو الزر Â«غير مستجيب».
