...
window.Auth = {
    ...
    handleServerSessionInvalid(message, errorCode) {
        const hasUiSession = (() => {
            try {
                return !!(sessionStorage.getItem('hse_current_session') || (typeof AppState !== 'undefined' && AppState.currentUser));
            } catch (_e) {
                return !!(typeof AppState !== 'undefined' && AppState.currentUser);
            }
        })();
        // إعادة تحميل أو جلسة واجهة قائمة: لا تُخرج المستخدم — البيانات المحلية تبقى ظاهرة
        if ((typeof AppState !== 'undefined' && AppState.isPageRefresh) || hasUiSession) {
            Utils.safeWarn('⚠️ تجاهل إسقاط الجلسة:', errorCode || 'SESSION_INVALID');
            if (!this._sessionInvalidWarned) {
                this._sessionInvalidWarned = true;
                try {
                    if (typeof Notification !== 'undefined') {
                        Notification.warning('تعذر التحقق من جلسة الخادم. البيانات المحلية ظاهرة. سجّل الدخول إن لزم المزامنة الكاملة.');
                    }
                } catch (_n) { /* ignore */ }
            }
            return;
        ...
        (async () => {
            try {
                ...
                if (typeof Utils !== 'undefined' && typeof Utils.hasCloudBackendSync === 'function' && Utils.hasCloudBackendSync() && typeof GoogleIntegration !== 'undefined') {
                    ...
                    // الموظفون والقوى الشهرية أوراق مملوكة — تُجلب عند فتح موديولها لا عند الدخول
                    const prioritySheets = ['Contractors', 'ApprovedContractors'];
                    if (isAdminLogin) {
            ...
            if (sessionData) {
                try {
                    ...
                    if (user && user.email) {
                        ...
                        if (!this._hasValidServerSessionToken()) {
                            const sid = String(user.activeSessionId || user.sessionId || '').trim();
                            if (sid.startsWith('SES_') && sid.length >= 16) {
                                this._storeServerSessionToken({ sessionToken: sid });
                            }
                        }
                        if (!this._hasValidServerSessionToken()) {
                            Utils.safeWarn('⚠️ جلسة واجهة بدون token خادم — تُستعاد محلياً ثم تُزامن عند توفر الجلسة');
                        }
            ...
            if (user) {
                try {
                    ...
                    if (user && user.email) {
                        ...
                        if (!this._hasValidServerSessionToken()) {
                            Utils.safeWarn('⚠️ تذكرني بدون token خادم — تُستعاد الجلسة محلياً');
                        }
                        if (typeof window.DataManager !== 'undefined' && typeof window.DataManager.purgeIfUserChanged === 'function') {
    ...
    async loadModulesDataSequentially() {
        ...
        try {
            ...
            const processModule = async (moduleName) => {
                try {
                    if (moduleName === 'training' || moduleName === 'clinic' || moduleName === 'ptw' || moduleName === 'daily-observations') {
                        return;
                    }
