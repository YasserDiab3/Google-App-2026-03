...
(function() {
    ...
    // بعد «تحديث الصفحة»: أبقِ IndexedDB/localStorage — لا تفرّغ الواجهة
    (function markPreserveLocalAfterUpdate() {
        try {
            if (sessionStorage.getItem('hse_keep_local_data') === '1') {
                sessionStorage.removeItem('hse_keep_local_data');
                window.__HSE_AFTER_VERSION_UPDATE = true;
            }
        } catch (_) {}
    })();

    // تنظيف روابط API قديمة فقط — ممنوع مسح hse_app_data أو IndexedDB عند تغيير الإصدار
    ...
    const AppBootstrap = {
        ...
        async phaseServices() {
            ...
            if (window.__HSE_AFTER_VERSION_UPDATE && typeof AppState !== 'undefined') {
                AppState.isPageRefresh = true;
                log('⚡ تحديث إصدار — عرض البيانات المحلية فوراً دون مسح الكاش');
            }
        ...
        async phaseReady() {
            ...
            try {
                if (window.EnhancedLoader && typeof window.EnhancedLoader.hide === 'function') {
