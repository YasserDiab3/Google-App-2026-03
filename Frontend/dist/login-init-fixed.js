...
(function () {
    ...
    const LoginSyncSetup = (function () {
        ...
        function getDefaultGoogleConfig() {
            const apiUrl = (typeof getEffectiveApiUrl === 'function')
                ? getEffectiveApiUrl()
                : (typeof window !== 'undefined' && window.HSE_DEFAULT_GAS_URL) || '';
            const sheetId = (typeof window !== 'undefined' && window.HSE_DEFAULT_SPREADSHEET_ID) || '';
            const base = {
                appsScript: { enabled: true, scriptUrl: apiUrl },
                sheets: { enabled: !!sheetId, spreadsheetId: sheetId, apiKey: '' },
                maps: { enabled: false, apiKey: '' }
            };
            if (typeof window !== 'undefined' && typeof window.__hseEnsureGasConfig === 'function') {
                return window.__hseEnsureGasConfig(base);
            }
            return base;
        }
