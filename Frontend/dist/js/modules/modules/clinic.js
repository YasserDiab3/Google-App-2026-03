                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4" style="background: linear-gradient(135deg, #f8fafc 0%, #eef2ff 100%); padding: 16px; border-radius: 12px;">
                            ...
                            <div>
                                ...
                                <select id="injury-factory" class="form-input">
                                    ...
                                    ${g.map($=>`<option value="${Utils.escapeHTML($.id)}" ${f===$.id||f===$.name?"selected":""}>${Utils.escapeHTML($.name)}</option>`).join("")}
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select id="injury-type" required class="form-input">
                                    ...
                                    ${u.map($=>`<option value="${Utils.escapeHTML($)}" ${e?.injuryType===$?"selected":""}>${Utils.escapeHTML($)}</option>`).join("")}
                                </select>
                            </div>
                            <div>
...
\u0647\u0644 \u062A\u0631\u064A\u062F \u0627\u0644\u0625\u063A\u0644\u0627\u0642\u061F`))return;i.state.currentInjuryAttachments=[],n.remove()}})},showVisitForm(e=null,t=null){const i=!!e;if(!document.getElementById("clinic-section")){t&&(t.disabled=!1);return}try{this.ensureData()}catch(o){t&&(t.disabled=!1),Utils.safeError("\u062E\u0637\u0623 \u0641\u064A \u062A\u062D\u0636\u064A\u0631 \u0646\u0645\u0648\u0630\u062C \u0627\u0644\u0632\u064A\u0627\u0631\u0629:",o);return}typeof Permissions<"u"&&Permissions.ensureFormSettingsState&&Permissions.ensureFormSettingsState().catch(()=>{});const s=document.createElement("div");if(s.className="modal-overlay",s.innerHTML=`
        ...
        `,document.body.appendChild(s),t){const o=new MutationObserver(()=>{document.body.contains(s)||(t.disabled=!1,o.disconnect())});o.observe(document.body,{childList:!0,subtree:!0})}setTimeout(()=>{const o=document.getElementById("visit-person-type"),l=document.getElementById("visit-employee-code"),r=document.getElementById("visit-history-tbody"),c=()=>{if(!r)return;const L=o?.value||"employee",D=l?.value.trim();if(L!=="employee"||!D){r.innerHTML='<tr><td colspan="6" class="text-center text-gray-500 py-4">\u0644\u0627 \u062A\u0648\u062C\u062F \u0628\u064A\u0627\u0646\u0627\u062A</td></tr>';return}const b=(AppState.appData.clinicVisits||[]).filter(S=>S.personType==="employee"&&(S.employeeCode===D||S.employeeNumber===D)).sort((S,A)=>new Date(A.visitDate||A.createdAt)-new Date(S.visitDate||S.createdAt)).slice(0,10);b.length===0?r.innerHTML='<tr><td colspan="6" class="text-center text-gray-500 py-4">\u0644\u0627 \u062A\u0648\u062C\u062F \u0632\u064A\u0627\u0631\u0627\u062A \u0633\u0627\u0628\u0642\u0629</td></tr>':r.innerHTML=b.map(S=>`
                    ...
                    `).join("")};if(l&&r&&(l.addEventListener("blur",c),l.addEventListener("input",()=>{l.value.trim().length>=3&&c()})),o&&o.addEventListener("change",()=>{const L=document.querySelector("#visit-history-table")?.closest(".mt-6");L&&(L.style.display=o.value==="employee"?"block":"none"),o.value==="employee"&&c()}),e&&e.employeeCode&&c(),e?.personType==="contractor"){const L=document.getElementById("visit-contractor-name-select");L&&(Clinic.loadContractorsIntoSelect(L),(e.employeeName||e.contractorName)&&(L.value=e.employeeName||e.contractorName||""))}typeof Clinic.handlePersonTypeChange=="function"&&Clinic.handlePersonTypeChange(),typeof Clinic.setupClinicWorkplaceDatalist=="function"&&(Clinic.setupClinicWorkplaceDatalist("visit-factory","visit-employee-location","visit-employee-location-datalist"),Clinic.setupClinicWorkplaceDatalist("visit-contractor-factory","visit-work-area","visit-work-area-datalist"));const p=document.getElementById("visit-contractor-name-select"),f=document.getElementById("visit-contractor-workers-datalist");if(p&&f){const L=()=>{const D=(p.value||"").trim(),b=Clinic.getContractorWorkerSuggestions(D);f.innerHTML=b.map(S=>`<option value="${Utils.escapeHTML(S)}"></option>`).join("")};p.addEventListener("input",L),p.addEventListener("change",L)}const d=document.getElementById("visit-medication-select"),m=document.getElementById("visit-medications-list"),u=document.getElementById("visit-add-medication-btn"),g=document.getElementById("visit-medication-quantity"),y=document.getElementById("visit-medications-datalist");let h=e?.medications&&Array.isArray(e.medications)?[...e.medications]:[];const x=()=>{if(!d||!y)return;const L=this.getMedications().filter(b=>(b.remainingQuantity??b.quantity??0)<=0?!1:!h.some(C=>C.medicationId===b.id)),D={};y.innerHTML=L.map(b=>{const S=b.remainingQuantity??b.quantity??0,A=`${b.name||""} (\u0645\u062A\u0648\u0641\u0631: ${S})`,C=String(b.name||"").toLowerCase().trim();return C&&(D[C]=b.id),`<option value="${Utils.escapeHTML(A)}"></option>`}).join(""),d.dataset.nameToId=JSON.stringify(D),d.dataset.selectedId=""},T=()=>{if(m){if(h.length===0){m.innerHTML="";return}m.innerHTML=h.map((L,D)=>{const b=this.getMedications().find(S=>S.id===L.medicationId);return`
