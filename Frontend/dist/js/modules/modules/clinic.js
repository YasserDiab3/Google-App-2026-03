            ...
            <div class="content-card" style="padding:0;overflow:hidden;">
                ...
                <div class="clinic-stat clinic-stat--med">
                    ...
                    <span class="clinic-stat__pct">${W.sub}</span>
                        ...
                        <td style="padding:9px 12px;color:${X};">${Utils.escapeHTML(w.stockStatus)}</td>
                    </tr>`}).join(""));const ee=document.getElementById("clinic-med-low-stock-alert"),se=document.getElementById("clinic-med-low-stock-list");ee&&se&&(L.lowStockHighDemand.length>0?(ee.style.display="block",se.innerHTML=L.lowStockHighDemand.map(w=>`<li><strong>${Utils.escapeHTML(w.name)}</strong>: \u0645\u0646\u0635\u0631\u0641 ${w.totalQty} \u2014 \u0645\u062E\u0632\u0648\u0646 ${w.stockRemaining??"\u2014"}</li>`).join("")):(ee.style.display="none",se.innerHTML=""));const K={};d.forEach(w=>{const H=w.contractorWorkerName||w.employeeName||w.externalName||w.personName||w.name||"",z=String(H).trim(),W=String(w.contractorName||"").trim(),X=z||(W?W+" (\u0628\u062F\u0648\u0646 \u0627\u0633\u0645 \u0639\u0627\u0645\u0644)":"\u063A\u064A\u0631 \u0645\u062D\u062F\u062F");K[X]||(K[X]={count:0,dept:"",loc:""}),K[X].count++,K[X].dept||(K[X].dept=w.employeeDepartment||w.department||w.contractorName||w.contractorPosition||"\u2014"),K[X].loc||(K[X].loc=w.employeeLocation||w.workArea||w.factoryName||w.factory||"\u2014")});const ie=Object.entries(K).sort((w,H)=>H[1].count-w[1].count).slice(0,15),oe=document.getElementById("clinic-top-visitors-count"),re=document.getElementById("clinic-top-visitors-tbody");oe&&(oe.textContent=`${ie.length} \u0634\u062E\u0635`),re&&(re.innerHTML=ie.length===0?'<tr><td colspan="5" style="padding:24px;text-align:center;color:#94a3b8;">\u0644\u0627 \u062A\u0648\u062C\u062F \u0628\u064A\u0627\u0646\u0627\u062A \u0632\u064A\u0627\u0631\u0627\u062A</td></tr>':ie.map(([w,H],z)=>{const W=z%2===0?"#fff":"#fafafa",X=H.count>=5?"#dc2626":H.count>=3?"#f59e0b":"#2563eb";return`<tr style="border-bottom:1px solid #f8fafc;background:${W};" onmouseover="this.style.background='#eff6ff'" onmouseout="this.style.background='${W}'">
