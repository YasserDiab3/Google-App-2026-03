                            ...
                            <div class="ptw-s1-layout">
                                ...
                                <div>
                                    ...
                                    <select id="manual-permit-location" class="form-input transition-all focus:ring-2 focus:ring-blue-200" required>
                                        ...
                                        ${i.map(g=>{let $=a&&(a.locationId===g.id||a.location&&(a.location.split(" - ")[0]===g.name||a.location===g.name));return`<option value="${Utils.escapeHTML(g.id)}" data-site-name="${Utils.escapeHTML(g.name)}" ${$?"selected":""}>${Utils.escapeHTML(g.name)}</option>`}).join("")}
                                    </select>
                                </div>
                            ...
                            <div class="overflow-x-auto bg-white">
                                <table class="w-full ptw-paper-grid-table" style="border-collapse: collapse; border: 1px solid #000;">
                                    ...
                                    <tbody id="manual-team-members-list">
                                        ${(a?.teamMembers&&a.teamMembers.length?a.teamMembers:[{name:"",signature:""}]).map($=>`
...
`))&&j()});const Y=_.querySelector("#manual-paper-permit-number");Y&&Y.addEventListener("input",()=>{Y.style.border="2px solid #90caf9",Y.style.boxShadow="none"});const fe=_.querySelector("#manual-permit-time-from"),se=_.querySelector("#manual-permit-time-to"),te=_.querySelector("#manual-permit-total-time"),we=()=>{const g=fe.value,$=se.value;if(!g||!$){te.value="";return}te.value=this.calculateTotalTime(g,$)};fe?.addEventListener("change",we),se?.addEventListener("change",we),a?.timeFrom&&a?.timeTo&&we();const re=_.querySelector("#manual-permit-location"),be=_.querySelector("#manual-permit-sublocation-wrapper"),ee=_.querySelector("#manual-permit-sublocation"),ve=_.querySelector("#manual-permit-location-entries"),ge=_.querySelector("#manual-selected-sublocations-container"),q=_.querySelector("#manual-selected-sublocations-display"),le=_.querySelector("#manual-selected-sublocations-list"),he=3,oe=[],xe=g=>({locationId:String(g?.locationId||"").trim(),location:String(g?.location||"").trim(),sublocationId:String(g?.sublocationId||"").trim(),sublocation:String(g?.sublocation||"").trim()}),ue=()=>{ve&&(ve.value=JSON.stringify(oe))},me=()=>{if(!ge||!q||!le)return;const g=oe.map(C=>C.sublocation||C.location).filter(Boolean),$=g.length>0;ge.style.display=$?"block":"none",q.value=$?g.join("\u060C "):"",le.innerHTML=oe.map((C,H)=>`
            ...
            `).join(""),le.querySelectorAll(".manual-remove-sublocation-btn").forEach(C=>{C.addEventListener("click",()=>{const H=Number(C.getAttribute("data-index"));Number.isInteger(H)&&H>=0&&(oe.splice(H,1),ue(),me())})})},ke=g=>{if(!ee||!be)return;if(!g){be.style.display="none",ee.innerHTML='<option value="">\u0627\u062E\u062A\u0631 \u0627\u0644\u0645\u0643\u0627\u0646 \u0627\u0644\u0641\u0631\u0639\u064A</option>';return}const $=this.getPlaceOptions(g);if(!Array.isArray($)||$.length===0){be.style.display="none",ee.innerHTML='<option value="">\u0627\u062E\u062A\u0631 \u0627\u0644\u0645\u0643\u0627\u0646 \u0627\u0644\u0641\u0631\u0639\u064A</option>';return}be.style.display="block",ee.innerHTML='<option value="">\u0627\u062E\u062A\u0631 \u0627\u0644\u0645\u0643\u0627\u0646 \u0627\u0644\u0641\u0631\u0639\u064A</option>'+$.map(C=>`
                <option value="${Utils.escapeHTML(C.id)}" data-place-name="${Utils.escapeHTML(C.name)}">${Utils.escapeHTML(C.name)}</option>
            `).join(""),ee.value=""},Ie=()=>{if(!re||!ee)return;const g=String(re.value||"").trim(),$=re.options[re.selectedIndex]?.getAttribute("data-site-name")||re.options[re.selectedIndex]?.textContent||"",C=String(ee.value||"").trim(),H=ee.options[ee.selectedIndex],X=H?.getAttribute("data-place-name")||(H?.value?H.textContent:"")||"";if(!g||!$||!C||!X)return;if(oe.some(F=>F.locationId===g&&F.sublocationId===C)){Notification.warning(this._t("module.ptw.notify.sublocDup","\u062A\u0645 \u0627\u062E\u062A\u064A\u0627\u0631 \u0647\u0630\u0627 \u0627\u0644\u0645\u0643\u0627\u0646 \u0627\u0644\u0641\u0631\u0639\u064A \u0628\u0627\u0644\u0641\u0639\u0644")),ee.value="";return}if(oe.length>=he){Notification.warning(this._t("module.ptw.notify.sublocMax3","\u064A\u0645\u0643\u0646 \u0627\u062E\u062A\u064A\u0627\u0631 3 \u0623\u0645\u0627\u0643\u0646 \u0641\u0631\u0639\u064A\u0629 \u0641\u0642\u0637 \u0643\u062D\u062F \u0623\u0642\u0635\u0649")),ee.value="";return}oe.push(xe({locationId:g,location:$.trim(),sublocationId:C,sublocation:X.trim()})),ue(),me(),ee.value=""},$e=()=>{let g=[];if(ve?.value)try{const $=JSON.parse(ve.value);Array.isArray($)&&(g=$)}catch{g=[]}if((!g||g.length===0)&&(a?.location||a?.sublocation)){const $=String(a?.locationId||re?.value||"").trim(),C=String(re?.options[re?.selectedIndex]?.getAttribute("data-site-name")||"").trim(),H=String(a?.location||"").split("|").map(F=>F.trim()).filter(Boolean),X=String(a?.sublocationId||"").split("|").map(F=>F.trim()).filter(Boolean),J=String(a?.sublocation||"").split("|").map(F=>F.trim()).filter(Boolean);J.length>0?g=J.map((F,ie)=>({locationId:$,location:C||H[0]?.split(" - ")[0]||a?.location||"",sublocationId:X[ie]||"",sublocation:F})):g=H.map((F,ie)=>{const ce=F.indexOf(" - ");return ce===-1?{locationId:$,location:F,sublocationId:X[ie]||"",sublocation:""}:{locationId:$,location:F.slice(0,ce).trim(),sublocationId:X[ie]||"",sublocation:F.slice(ce+3).trim()}})}g.map(xe).filter($=>$.location&&$.sublocation).slice(0,he).forEach($=>oe.push($)),ue(),me()};re?.addEventListener("change",()=>{oe.length=0,ue(),me(),ke(re.value)}),ee?.addEventListener("change",Ie),ke(re?.value),$e(),this.setupManualEquipmentToolsSync(_);const T=(g,$)=>{if(!g||!$)return;const C=()=>{const H=String(g.value||"").trim(),X=String($.value||"").trim(),J=g.dataset.autoCopiedValue||"";g.dataset.knownLoaded!=="1"&&(!X||X===J)&&($.value=H,g.dataset.autoCopiedValue=H)};C(),g.addEventListener("input",C)},G=(g,$,C)=>{const H=_.querySelector(g);if(!H)return;const X=new Set(["\u0645\u0633\u0626\u0648\u0644 \u0627\u0644\u062C\u0647\u0629 \u0627\u0644\u0637\u0627\u0644\u0628\u0629","\u0645\u062F\u064A\u0631 \u0645\u0646\u0637\u0642\u0629 \u0627\u0644\u0623\u0639\u0645\u0627\u0644","\u0645\u062F\u064A\u0631 / \u0645\u0647\u0646\u062F\u0633 \u0627\u0644\u0635\u064A\u0627\u0646\u0629","\u0645\u0633\u0624\u0648\u0644 \u0627\u0644\u062C\u0647\u0629 \u0627\u0644\u0637\u0627\u0644\u0628\u0629"]),J=F=>{const ie=F?.dataset.role;if(!ie)return;const ce=H.querySelector(`${C}[data-role="${ie}"]`);T(F,ce)};H.querySelectorAll($).forEach(F=>{if(F.matches(".ia-approval-select")||F.tagName==="SELECT"){J(F);return}X.has(F.dataset?.role)||J(F)}),H.addEventListener("input",F=>{const ie=F.target?.dataset?.role;F.target.matches($)&&(F.target.matches(".ia-approval-select")||X.has(ie)||J(F.target))}),H.addEventListener("change",F=>{(F.target.matches($)||F.target.matches(".ia-approval-select"))&&J(F.target)})};this._setupIaRolePickerListeners(_),this.setupManualPermitKnownLookups(_,w,I),G("#manual-approvals-list",".manual-approval-name, .ia-approval-select",".manual-approval-sig"),G("#manual-closure-approvals-list",".manual-closure-approval-name, .ia-approval-select",".manual-closure-approval-sig");const ae=_.querySelector("#manual-work-type-panel"),ne=_.querySelector("#manual-work-type-panel-placeholder"),pe=_.querySelector("#manual-work-type-panel-body"),_e=_.querySelector("#manual-work-type-panel-title"),de=_.querySelector("#manual-work-type-select"),Ee=_.querySelector("#manual-work-type-selected-chips"),ze={hot:"manual-panel-hot",confined:"manual-panel-confined",height:"manual-panel-height",excavation:"manual-panel-excavation",electrical:"manual-panel-electrical",cold:"manual-panel-cold",other:"manual-panel-other"},Me={hot:"\u0623\u0639\u0645\u0627\u0644 \u0633\u0627\u062E\u0646\u0629",confined:"\u0623\u0645\u0627\u0643\u0646 \u0645\u063A\u0644\u0642\u0629",height:"\u0639\u0645\u0644 \u0639\u0644\u0649 \u0627\u0631\u062A\u0641\u0627\u0639",excavation:"\u0623\u0639\u0645\u0627\u0644 \u062D\u0641\u0631",electrical:"\u0623\u0639\u0645\u0627\u0644 \u0643\u0647\u0631\u0628\u0627\u0621",cold:"\u0623\u0639\u0645\u0627\u0644 \u0639\u0644\u0649 \u0627\u0644\u0628\u0627\u0631\u062F",other:"\u0623\u0639\u0645\u0627\u0644 \u0623\u062E\u0631\u0649"},ye=[],De=_.querySelector("#manual-work-type-selected-empty"),Ce=_.querySelector("#manual-work-type-selected-hint"),Te=_.querySelector("#manual-work-type-panel-badge"),Ae=()=>{if(!Te||!de)return;const g=ye.some($=>$.typeKey===de.value);Te.style.display=de.value&&g?"inline-block":"none"},qe=()=>{if(!Ee)return;const g=ye.length>0;De&&(De.style.display=g?"none":"block"),Ce&&(Ce.style.display=g?"block":"none"),Ee.innerHTML=ye.map(({typeKey:$,label:C})=>`<span class="manual-selected-type-chip" data-type="${$}" title="\u0627\u0646\u0642\u0631 \u0644\u062A\u062D\u0631\u064A\u0631 \u062A\u0641\u0627\u0635\u064A\u0644: ${C}" role="button" tabindex="0" style="display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; border-radius: 20px; background: #ede9fe; color: #5b21b6; font-size: 0.8rem; font-weight: 500;">${C}</span>`).join(""),Ee.querySelectorAll(".manual-selected-type-chip").forEach($=>{$.addEventListener("click",function(){const C=this.getAttribute("data-type");C&&de&&(de.value=C,de.dispatchEvent(new Event("change")),Ae())}),$.addEventListener("keydown",function(C){(C.key==="Enter"||C.key===" ")&&(C.preventDefault(),this.click())})}),Ae()},Ne=()=>{const g=de?.value,$=g?Me[g]||g:"";!g||!$||ye.some(C=>C.typeKey===g)||(ye.push({typeKey:g,label:$}),qe(),Ae())};if(de&&ae&&pe&&(de.addEventListener("change",function(){const g=this.value,$=Me[g]||g;if(!g){ne&&(ne.style.display="block"),pe.style.display="none",Te&&(Te.style.display="none");return}ne&&(ne.style.display="none"),pe.style.display="block",(ae.querySelectorAll(".manual-type-panel-body")||[]).forEach(H=>{H.style.display="none"});const C=_.querySelector("#"+(ze[g]||""));C&&(C.style.display="block",_e&&(_e.textContent=$)),Ae()}),pe.addEventListener("change",function(g){g.target.matches('input[type="checkbox"], input[type="text"], input[type="number"]')&&Ne()}),pe.addEventListener("input",function(g){g.target.matches('input[type="text"], input[type="number"]')&&Ne()})),a){const g=($,C)=>{$&&!ye.some(H=>H.typeKey===C)&&ye.push({typeKey:C,label:Me[C]})};g(a.hotWorkDetails&&a.hotWorkDetails.length||a.hotWorkOther,"hot"),g(a.confinedSpaceDetails&&a.confinedSpaceDetails.length||a.confinedSpaceOther,"confined"),g(a.heightWorkDetails&&a.heightWorkDetails.length||a.heightWorkOther,"height"),g(a.excavationLength||a.excavationWidth||a.excavationDepth||a.soilType,"excavation"),g(a.electricalWorkType,"electrical"),g(a.coldWorkType,"cold"),g(a.otherWorkType,"other")}qe();const Ue=_.querySelectorAll('input[name="manual-permit-status-radio"]'),je=_.querySelector("#manual-permit-status");a?.status?Ue.forEach(g=>{g.value===a.status&&(g.checked=!0,PTW.updateManualStatusBtnSelection(g))}):Ue.forEach(g=>{g.value==="\u0627\u0643\u062A\u0645\u0644 \u0627\u0644\u0639\u0645\u0644 \u0628\u0634\u0643\u0644 \u0622\u0645\u0646"&&(g.checked=!0,PTW.updateManualStatusBtnSelection(g))}),_.querySelector("#manual-add-team-member-btn")?.addEventListener("click",()=>{const g=_.querySelector("#manual-team-members-list");if(!g)return;const $=document.createElement("tr");$.className="manual-team-member-row",$.innerHTML=`
            ...
            `,g.appendChild($),typeof _._attachManualTeamRowLookup=="function"&&_._attachManualTeamRowLookup($)}),_.querySelectorAll(".manual-risk-cell").forEach(g=>{g.addEventListener("click",()=>{const $=g.dataset.likelihood,C=g.dataset.consequence,H=g.dataset.score,X=g.dataset.level,J=g.dataset.bg||"#22c55e",F=g.dataset.text||"#ffffff";_.querySelectorAll(".manual-risk-cell").forEach(Fe=>{Fe.classList.remove("ring-4","ring-blue-500","ring-blue-600","ring-inset")}),g.classList.add("ring-4","ring-blue-600","ring-inset"),_.querySelector("#manual-risk-likelihood").value=$,_.querySelector("#manual-risk-consequence").value=C,_.querySelector("#manual-risk-score").value=H,_.querySelector("#manual-risk-level").value=X;const ie=_.querySelector("#manual-risk-result");ie&&ie.classList.remove("hidden");const ce=_.querySelector("#manual-risk-score-display"),Be=_.querySelector("#manual-risk-level-display"),Re=_.querySelector("#manual-risk-likelihood-display"),We=_.querySelector("#manual-risk-consequence-display");ce&&(ce.textContent=H),Be&&(Be.textContent=X),Re&&(Re.textContent=$),We&&(We.textContent=C);const Pe=_.querySelector("#manual-risk-result-badge");Pe&&(Pe.style.background=J,Pe.style.color=F,Pe.textContent=H);const Se=_.querySelector("#manual-risk-notes");if(Se){const He=`\u062A\u0642\u064A\u064A\u0645 \u062A\u0644\u0642\u0627\u0626\u064A: \u0645\u0633\u062A\u0648\u0649 \u0627\u0644\u062E\u0637\u0631 ${X} (\u062F\u0631\u062C\u0629 ${H}) \u2014 \u0627\u0644\u0627\u062D\u062A\u0645\u0627\u0644\u064A\u0629 ${$} \xD7 \u0627\u0644\u062E\u0637\u0648\u0631\u0629 ${C}. ${{\u0645\u0646\u062E\u0641\u0636:"\u064A\u0645\u0643\u0646 \u062A\u0646\u0641\u064A\u0630 \u0627\u0644\u0639\u0645\u0644 \u0645\u0639 \u0627\u0644\u0627\u0644\u062A\u0632\u0627\u0645 \u0628\u0627\u0644\u0636\u0648\u0627\u0628\u0637 \u0627\u0644\u0623\u0633\u0627\u0633\u064A\u0629 \u0648\u0627\u0644\u0645\u062A\u0627\u0628\u0639\u0629 \u0627\u0644\u062F\u0648\u0631\u064A\u0629.",\u0645\u062A\u0648\u0633\u0637:"\u064A\u0644\u0632\u0645 \u062A\u0639\u0632\u064A\u0632 \u0627\u0644\u0636\u0648\u0627\u0628\u0637 \u0627\u0644\u0648\u0642\u0627\u0626\u064A\u0629 \u0648\u0627\u0644\u0645\u062A\u0627\u0628\u0639\u0629 \u0627\u0644\u0645\u064A\u062F\u0627\u0646\u064A\u0629 \u0642\u0628\u0644 \u0648\u0623\u062B\u0646\u0627\u0621 \u0627\u0644\u062A\u0646\u0641\u064A\u0630.",\u0645\u0631\u062A\u0641\u0639:"\u0644\u0627 \u064A\u0628\u062F\u0623 \u0627\u0644\u0639\u0645\u0644 \u0642\u0628\u0644 \u0627\u0639\u062A\u0645\u0627\u062F \u0636\u0648\u0627\u0628\u0637 \u0625\u0636\u0627\u0641\u064A\u0629 \u0648\u0627\u0636\u062D\u0629 \u0648\u0645\u062A\u0627\u0628\u0639\u0629 \u0625\u0634\u0631\u0627\u0641\u064A\u0629 \u0645\u0628\u0627\u0634\u0631\u0629.",\u062D\u0631\u062C:"\u0625\u064A\u0642\u0627\u0641 \u0627\u0644\u062A\u0646\u0641\u064A\u0630 \u0641\u0648\u0631\u0627\u064B \u062D\u062A\u0649 \u0625\u0632\u0627\u0644\u0629/\u062E\u0641\u0636 \u0627\u0644\u062E\u0637\u0631 \u0648\u0627\u0639\u062A\u0645\u0627\u062F \u062E\u0637\u0629 \u062A\u062D\u0643\u0645 \u0645\u0634\u062F\u062F\u0629."}[X]||""}`.trim(),Le=String(Se.value||"").trim(),Oe=String(Se.dataset.autoRiskText||"").trim();(!Le||Le===Oe||Le.startsWith("\u062A\u0642\u064A\u064A\u0645 \u062A\u0644\u0642\u0627\u0626\u064A:"))&&(Se.value=He,Se.dataset.autoRiskText=He)}})}),_.querySelector("#manual-add-approval-btn")?.addEventListener("click",()=>{const g=_.querySelector("#manual-approvals-list");if(!g)return;const $=g.querySelectorAll("tr").length+1,C=document.createElement("tr");C.className="manual-approval-row border-b border-gray-100 hover:bg-amber-50 transition-colors",C.innerHTML=`
            ...
            `,g.appendChild(C)}),_.querySelector("#manual-add-closure-approval-btn")?.addEventListener("click",()=>{const g=_.querySelector("#manual-closure-approvals-list");if(!g)return;const $=g.querySelectorAll("tr").length+1,C=document.createElement("tr");C.className="manual-closure-approval-row border-b border-gray-100 hover:bg-cyan-50 transition-colors",C.innerHTML=`
                        ...
                        <div class="ptw-form-section ptw-section-9">
                            ...
                            ${a?`
                                ...
                                ${(()=>{const w=e?.manualClosureApprovals||[];return w.length===0?'<p class="text-gray-500">\u0644\u0627 \u062A\u0648\u062C\u062F \u0627\u0639\u062A\u0645\u0627\u062F\u0627\u062A \u0625\u063A\u0644\u0627\u0642 \u0645\u0633\u062C\u0644\u0629</p>':`
                                    ...
                                    `})()}
                            `:`
                                ${(()=>{const w=o.approvals||[];this.formClosureApprovals=w.map(O=>Object.assign({},O)),this.formClosureCircuitOwnerId=o.circuitOwnerId||"__default__";const I=o.circuitName||"";return this.formClosureCircuitName=I,`
