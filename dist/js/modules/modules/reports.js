...
-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  <table class="report-table">
    ...
    <tbody>
      ${this._msrTableRow([c(e.trainingsConducted),g(l.activities.trainings)])}
      ${this._msrTableRow([c(e.participantsTrained),g(l.activities.participants)])}
      ${this._msrTableRow([c(e.auditsInspections),g(l.activities.inspections)])}
      ${this._msrTableRow([c(e.ptwsIssued),g(l.activities.ptw)])}
      ${this._msrTableRow([c(e.observations),g(l.activities.observations)])}
      ${this._msrTableRow([c(e.toolboxTalks),g(l.activities.toolbox)])}
    </tbody>
  </table>

  <h3 class="section-title ar-text" dir="${e.dir}">${s(e.manpowerStatus)}</h3>
  <table class="report-table">
    <thead><tr><th class="ar-text" dir="${e.dir}">${s(e.metric)}</th><th style="text-align:center" class="ar-text" dir="${e.dir}">${s(f)}</th><th style="text-align:center" class="ar-text" dir="${e.dir}">${s(e.cumulative)}</th></tr></thead>
    <tbody>
      ${this._msrTableRow([c(e.manpowerAvg),g(l.manpower.avgMonth),g(l.manpower.avgCum)])}
      ${this._msrTableRow([c(e.totalManDays),g(l.manpower.manDaysMonth),g(l.manpower.manDaysCum)])}
      ${this._msrTableRow([`<strong class="ar-text" dir="${e.dir}">${s(e.totalManHours)}</strong>`,g(l.manpower.manHoursMonth),g(l.manpower.manHoursCum)],"msr-row-total")}
    </tbody>
  </table>

  <h3 class="section-title ar-text" dir="${e.dir}">${s(e.accidentReport)}</h3>
  <table class="report-table">
    <thead><tr><th class="ar-text" dir="${e.dir}">${s(e.description)}</th><th style="text-align:center" class="ar-text" dir="${e.dir}">${s(f)}</th><th style="text-align:center" class="ar-text" dir="${e.dir}">${s(e.cumulative)}</th></tr></thead>
    <tbody>
      ${this._msrTableRow([`<strong class="ar-text" dir="${e.dir}">${s(e.totalAccidents)}</strong>`,g(l.accidents.totalMonth),g(l.accidents.totalCum)],"msr-row-total")}
      ${this._msrTableRow([c(e.reportableAccidents),g(l.accidents.reportableMonth),g(l.accidents.reportableCum)],"msr-row-danger")}
      ${this._msrTableRow([c(e.minorAccidents),g(l.accidents.minorMonth),g(l.accidents.minorCum)],"msr-row-warn")}
      ${this._msrTableRow([c(e.firstAidCases),g(l.accidents.firstAidMonth),g(l.accidents.firstAidCum)],"msr-row-warn")}
      ${this._msrTableRow([c(e.nearMiss),g(l.accidents.nearMissMonth),g(l.accidents.nearMissCum)],"msr-row-info")}
      ${this._msrTableRow([c(e.manDaysLost),g(l.accidents.daysLostMonth),g(l.accidents.daysLostCum)])}
    </tbody>
  </table>
  ...
  <table class="report-table">
    ...
    <tbody>${l.walkRows.length?l.walkRows.map(y=>this._msrTableRow([s(this._msrFmtDate(y.date,r)),`<span class="ar-text" dir="${e.dir}">${s(y.withWhom)}</span>`])).join(""):this._msrEmptyRow(2,e.dash)}</tbody>
  </table>
  ...
  <table class="report-table">
    ...
    <tbody>${l.inductionRows.length?l.inductionRows.map(y=>this._msrTableRow([s(this._msrFmtDate(y.date,r)),`<span class="msr-num">${s(y.persons)}</span>`])).join(""):this._msrEmptyRow(2,e.dash)}</tbody>
  </table>
  ...
  <table class="report-table">
    ...
    <tbody>${l.toolboxRows.length?l.toolboxRows.map(y=>this._msrTableRow([s(this._msrFmtDate(y.date,r)),`<span class="ar-text" dir="${e.dir}">${s(y.topic)}</span>`,`<span class="msr-num">${s(y.participants)}</span>`])).join(""):this._msrEmptyRow(3,e.dash)}</tbody>
  </table>
  <div class="msr-subsection ar-text" dir="${e.dir}">${s(e.hseTraining)}</div>
  <table class="report-table">
    <thead><tr><th class="ar-text" dir="${e.dir}">${s(e.date)}</th><th class="ar-text" dir="${e.dir}">${s(e.topic)}</th><th style="text-align:center" class="ar-text" dir="${e.dir}">${s(e.participants)}</th></tr></thead>
    <tbody>${l.trainingRows.length?l.trainingRows.map(y=>this._msrTableRow([s(this._msrFmtDate(y.date,r)),`<span class="ar-text" dir="${e.dir}">${s(y.topic)}</span>`,`<span class="msr-num">${s(y.participants)}</span>`])).join(""):this._msrEmptyRow(3,e.dash)}</tbody>
  </table>
  ...
  <table class="report-table">
    ...
    <tbody>${l.momRows.length?l.momRows.map(y=>this._msrTableRow([s(this._msrFmtDate(y.date,r)),`<span class="ar-text" dir="${e.dir}">${s(y.points)}</span>`,this._msrStatusBadge(y.status||e.open,r)])).join(""):this._msrEmptyRow(3,e.dash)}</tbody>
  </table>
...
${this.buildMonthlySafetyReportBody(a,t,r,o)}
</div>`,u=`MSR-${t.label}-${o||"ALL"}`,f=l?`${n.title} \u2014 ${s}`:n.title,v=this._msrBuildPdfMeta(t,r,s,d.preparedBy,o);return typeof FormHeader<"u"&&FormHeader.generatePDFHTML?FormHeader.generatePDFHTML(u,f,c,!1,!1,v,new Date().toISOString(),new Date().toISOString()):`<!DOCTYPE html><html lang="${n.lang}" dir="${n.dir}"><head><meta charset="UTF-8"><title>${e(f)}</title></head><body>${c}</body></html>`},buildMonthlySafetyReportAllSitesHtml(a,t,r="ar"){const o=this.getMonthlySafetyStrings(r),n=b=>Utils.escapeHTML(b==null?"":String(b)),d=this.collectMonthlySafetyReportModel(a,t,null),e=this.getMonthlySafetySites(),l=`
  ...
  </div>`,s=e.map(b=>{const g=this.getMonthlySafetySiteLabel(b,r);return`
  <div class="msr-site-block" style="page-break-before:always">
    <div class="msr-site-banner ar-text" dir="${o.dir}">${n(g)}</div>
...
html,body,.report-wrapper,.report-wrapper *,.msr-report,.msr-report *{font-family:'Cairo','Tahoma','Segoe UI',sans-serif!important;letter-spacing:0!important;word-spacing:normal!important}`,d.head.appendChild(e)}},r=Utils.PdfExport.getOptimalCaptureScale(a.scrollWidth,a.scrollHeight,1.5),o=[{...t,scale:r,useCORS:!0,allowTaint:!1},{...t,scale:r,useCORS:!0,allowTaint:!0},{...t,scale:Math.min(r,1.3),useCORS:!1,allowTaint:!0}];let n=null;for(let d=0;d<o.length;d+=1)try{const e=await html2canvas(a,o[d]);if(e?.width>0&&e?.height>0)return e}catch(e){n=e}if(n)throw n;return null},async downloadMonthlySafetyReport(a,t="ar",r=null){const{t:o}=this.getTranslations();if(!a)return Notification.error(o("msg.invalidDateRange")),!1;if(typeof AppState>"u"||!AppState.appData)return Notification.error(o("msg.noData")),!1;await this.ensureTrainingDataForReport();const n=r==="__all__",d=n?this.buildMonthlySafetyReportAllSitesHtml(AppState.appData,a,t):this.buildMonthlySafetyReportHtml(AppState.appData,a,t,r),e=await this._msrPreparePdfHtml(d,t),l=n?"_ALL_SITES":r?`_${r}`:"",s=`Monthly_Safety_Report_${a.label}${l}_${t}.pdf`,c=document.createElement("iframe");c.setAttribute("aria-hidden","true"),c.style.cssText="position:fixed;left:-100000px;top:0;width:900px;height:1400px;border:0;visibility:hidden;",document.body.appendChild(c);try{if(!await this.ensureMonthlySafetyPdfLibs())return Notification.error(o("msg.allowPopups")),!1;c.srcdoc=e,await new Promise(S=>{c.onload=S,c.onerror=S,setTimeout(S,5e3)});const f=c.contentDocument||c.contentWindow?.document;if(!f)return!1;await this._msrWaitPdfFonts(f),await Promise.all(Array.from(f.images||[]).map(S=>new Promise(y=>{if(S.complete)return y();S.onload=y,S.onerror=y,setTimeout(y,3e3)})));const v=f.querySelector(".report-wrapper")||f.getElementById("monthly-safety-report-root")||f.body,b=await this._msrCaptureReportCanvas(v);if(!b)return!1;const g=Utils.PdfExport.createPdf({orientation:"portrait",unit:"mm",format:"a4"});return g?(Utils.PdfExport.appendCanvasAsPdfPages(g,b,{marginMm:8}),Utils.PdfExport.savePdf(g,s),!0):!1}catch(u){return typeof Utils<"u"&&Utils.safeError&&Utils.safeError("downloadMonthlySafetyReport:",u),Notification.error(u&&u.message||"\u0641\u0634\u0644 \u062A\u062D\u0645\u064A\u0644 \u0627\u0644\u062A\u0642\u0631\u064A\u0631"),!1}finally{c.remove()}},async generateAndExport(a,t={}){let r=null;try{const{t:o}=this.getTranslations();if(typeof AppState>"u"||!AppState.appData){Notification.error(o("msg.noData"));return}if(r=window.open("","_blank"),!r){Notification.error(o("msg.allowPopups"));return}r.document.write('<html dir="rtl"><body style="font-family: Arial; padding: 20px; text-align: center;"><p>\u062C\u0627\u0631\u064A \u062A\u062D\u0636\u064A\u0631 \u0627\u0644\u062A\u0642\u0631\u064A\u0631...</p></body></html>'),r.document.close(),(a==="full"||a==="period")&&await this.ensureTrainingDataForReport();const n=AppState.appData;let d="",e="",l=`REPORT-${String(a).toUpperCase()}-${new Date().toISOString().slice(0,10)}`,s={version:AppState?.companySettings?.formVersion||"1.0"};switch(a){case"incidents":d=o("report.incidents");const u=n.incidents||[];if(!Array.isArray(u)){Notification.error(o("msg.incidentsInvalid")),r&&r.close();return}e=this.generateIncidentsReport(u);break;case"training":d=o("report.training");const f=n.training||[];if(!Array.isArray(f)){Notification.error(o("msg.trainingInvalid")),r&&r.close();return}e=this.generateTrainingReport(f);break;case"period":{const v=t.period||await this._askForPeriod();if(!v){r&&r.close();return}d=`${o("report.periodSummary")} - ${v.label}`,e=this.generatePeriodSummaryReport(n,v);break}case"full":d=o("report.full"),e=this.generateFullReport(n);break;default:throw r&&r.close(),new Error(o("msg.unknownReport"))}const c=typeof FormHeader<"u"&&FormHeader.generatePDFHTML?FormHeader.generatePDFHTML(l,d,e,!1,!0,s,new Date().toISOString(),new Date().toISOString()):`<html><body>${e}</body></html>`;r.document.open(),r.document.write(c),r.document.close(),setTimeout(()=>{try{r.print()}catch(u){typeof Utils<"u"&&Utils.safeWarn&&Utils.safeWarn("\u0637\u0628\u0627\u0639\u0629 \u0627\u0644\u062A\u0642\u0631\u064A\u0631:",u)}},300)}catch(o){r&&typeof r.close=="function"&&r.close();const n=(typeof Notification<"u"&&Notification.error,o&&o.message||"\u062D\u062F\u062B \u062E\u0637\u0623 \u0639\u0646\u062F \u0627\u0633\u062A\u062E\u0631\u0627\u062C \u0627\u0644\u062A\u0642\u0631\u064A\u0631");typeof Notification<"u"&&Notification.error&&Notification.error(n)}},generateIncidentsReport(a){const{t,lang:r}=this.getTranslations(),o=r==="ar"?"ar-SA":"en-GB";return`
        ...
        `},generatePeriodSummaryReport(a,t){const{t:r,lang:o}=this.getTranslations(),n=o==="ar"?"ar-SA":"en-GB",d=t&&t.startDate?new Date(t.startDate):null,e=t&&t.endDate?new Date(t.endDate):null,l=this._filterArrayByDateRange(a.incidents||[],["date","incidentDate","createdAt"],d,e),s=this._filterArrayByDateRange(a.nearmiss||[],["date","createdAt"],d,e),c=this._filterArrayByDateRange(a.ptw||[],["startDate","date","createdAt","endDate"],d,e),u=this._filterArrayByDateRange(a.dailyObservations||[],["date","createdAt"],d,e),f=this._filterArrayByDateRange(a.clinicVisits||[],["visitDate","date","createdAt"],d,e),v=a.training||[],b=a.trainingAttendance||[],g=a.contractorTrainings||[],S=this._filterArrayByDateRange(v,["startDate","date","createdAt"],d,e),y=this._filterArrayByDateRange(b,["date","attendanceDate","createdAt"],d,e),D=this._filterArrayByDateRange(g,["date","trainingDate","startDate","createdAt"],d,e),_=a.violations||[],T=this._filterArrayByDateRange(_,["date","violationDate","createdAt"],d,e),M=[];let A=0;const h=new Set;y.forEach(i=>{i.employeeCode&&h.add(String(i.employeeCode).trim())}),S.forEach(i=>{if(i.participants&&Array.isArray(i.participants)&&i.participants.forEach(m=>{const $=m.code||m.employeeNumber;$&&!M.find(C=>(C.code||C.employeeNumber)===$)&&M.push(m)}),i.hours)A+=Number(parseFloat(i.hours))||0;else if(i.duration)A+=Number(parseFloat(i.duration))||0;else if(i.startTime&&i.endTime)try{const m=new Date(`2000-01-01 ${i.startTime}`),C=(new Date(`2000-01-01 ${i.endTime}`)-m)/(1e3*60*60);A+=Number.isFinite(C)?C:0}catch{}});const F=y.reduce((i,m)=>{const $=parseFloat(m.totalHours);return i+(Number.isFinite($)?$:0)},0);F>0&&(A=F);const L=M.length,H=typeof Training<"u"&&Training.getParticipantsCount?S.reduce((i,m)=>i+Training.getParticipantsCount(m),0):S.reduce((i,m)=>{const $=Number(m.participantsCount)||0;return i+(Number.isFinite($)?$:0)},0),p=h.size>0?h.size:L>0?L:H,x=p>0?(Number(A)/Number(p)).toFixed(2):"0.00",w=S.length,R=new Set;S.forEach(i=>{const m=(i.name||i.programName||"").toString().trim();m&&R.add(m)});const K=R.size||w,B=D.reduce((i,m)=>{const $=Number(m.traineesCount||m.attendees||0);return i+(Number.isFinite($)?$:0)},0),V=D.reduce((i,m)=>{const $=parseFloat(m.totalHours||m.trainingHours||0);return i+(Number.isFinite($)?$:0)},0),U=B>0?(V/B).toFixed(2):"0.00",z=D.length,Y=new Set;D.forEach(i=>{const m=(i.name||i.trainingName||i.topic||"").toString().trim();m&&Y.add(m)});const W=Y.size||z,O=T.filter(i=>i.violationType==="\u0645\u0648\u0638\u0641\u064A\u0646"||i.category==="\u0645\u0648\u0638\u0641\u064A\u0646"||!i.contractorName&&i.employeeName),j=T.filter(i=>i.violationType==="\u0645\u0642\u0627\u0648\u0644\u064A\u0646"||i.category==="\u0645\u0642\u0627\u0648\u0644\u064A\u0646"||i.contractorName),E={},N={};T.forEach(i=>{const m=i.department||i.employeeDepartment||"\u063A\u064A\u0631 \u0645\u062D\u062F\u062F";E[m]=(E[m]||0)+1;const $=(i.violationType||"\u063A\u064A\u0631 \u0645\u062D\u062F\u062F").trim()||"\u063A\u064A\u0631 \u0645\u062D\u062F\u062F";N[$]=(N[$]||0)+1});const G=Object.keys(E).map(i=>`<tr><td>${Utils.escapeHTML(i)}</td><td>${E[i]}</td></tr>`).join(""),k=Object.keys(N).map(i=>`<tr><td>${Utils.escapeHTML(i)}</td><td>${N[i]}</td></tr>`).join(""),I=r("report.hour"),P=t.type==="yearly"?r("report.periodTypeYearly"):r("report.periodTypeMonthly"),J=d&&e?`${d.toLocaleDateString(n)} - ${e.toLocaleDateString(n)}`:t.label;return`
        ...
        `},generateFullReport(a){const t=a.training||[],r=a.trainingAttendance||[],o=a.contractorTrainings||[],n=[];let d=0;const e=new Set;r.forEach(p=>{p.employeeCode&&e.add(String(p.employeeCode).trim())}),t.forEach(p=>{if(p.participants&&Array.isArray(p.participants)&&p.participants.forEach(x=>{const w=x.code||x.employeeNumber;w&&!n.find(R=>(R.code||R.employeeNumber)===w)&&n.push(x)}),p.hours)d+=Number(parseFloat(p.hours))||0;else if(p.duration)d+=Number(parseFloat(p.duration))||0;else if(p.startTime&&p.endTime)try{const x=new Date(`2000-01-01 ${p.startTime}`),R=(new Date(`2000-01-01 ${p.endTime}`)-x)/(1e3*60*60);d+=Number.isFinite(R)?R:0}catch{}});const l=r.reduce((p,x)=>{const w=parseFloat(x.totalHours);return p+(Number.isFinite(w)?w:0)},0);l>0&&(d=l);const s=n.length,c=typeof Training<"u"&&Training.getParticipantsCount?t.reduce((p,x)=>p+Training.getParticipantsCount(x),0):t.reduce((p,x)=>{const w=Number(x.participantsCount)||0;return p+(Number.isFinite(w)?w:0)},0),u=e.size>0?e.size:s>0?s:c,f=u>0?(Number(d)/Number(u)).toFixed(2):"0.00",v=o.reduce((p,x)=>{const w=Number(x.traineesCount||x.attendees||0);return p+(Number.isFinite(w)?w:0)},0),b=o.reduce((p,x)=>{const w=parseFloat(x.totalHours||x.trainingHours||0);return p+(Number.isFinite(w)?w:0)},0),g=v>0?(b/v).toFixed(2):"0.00",S=a.violations||[],y=S.filter(p=>p.violationType==="\u0645\u0648\u0638\u0641\u064A\u0646"||p.category==="\u0645\u0648\u0638\u0641\u064A\u0646"||!p.contractorName&&p.employeeName),D=S.filter(p=>p.violationType==="\u0645\u0642\u0627\u0648\u0644\u064A\u0646"||p.category==="\u0645\u0642\u0627\u0648\u0644\u064A\u0646"||p.contractorName),_={},T={};S.forEach(p=>{const x=p.department||p.employeeDepartment||"\u063A\u064A\u0631 \u0645\u062D\u062F\u062F";_[x]=(_[x]||0)+1;const w=(p.violationType||"\u063A\u064A\u0631 \u0645\u062D\u062F\u062F").trim()||"\u063A\u064A\u0631 \u0645\u062D\u062F\u062F";T[w]=(T[w]||0)+1});const M=Object.keys(_).map(p=>`<tr><td>${Utils.escapeHTML(p)}</td><td>${_[p]}</td></tr>`).join(""),A=Object.keys(T).map(p=>`<tr><td>${Utils.escapeHTML(p)}</td><td>${T[p]}</td></tr>`).join(""),{t:h,lang:F}=this.getTranslations(),L=F==="ar"?"ar-SA":"en-GB",H=h("report.hour");return`
            <div class="section-title">${h("report.generalStats")}</div>
            <p style="margin-bottom: 20px; color: #666;">${h("report.createdDate")}: ${new Date().toLocaleDateString(L)}</p>
